---
title: Adobe Workfront Goalsの基本を学ぶ
...
* 四半期ごとの目標を重ねて表示して調整し、目標にアクティビティを追加します。
...
| オブジェクト | 説明 |
|---|---|
| 目標 | 達成したい全体的な結果。 定義した 1 つの期間に対して複数の目標を作成できます。 各目標には所有者（通常は個人またはチーム）があります（グループに目標を割り当てることもできます）。 複数の目標（子の目標）の進捗状況が別の目標（親の目標）の進捗状況にどのように影響するかを示すために、目標を互いに調整できます。 |
| 結果 | 目標達成を示す指標インジケーター 各目標には 1 つ以上の結果を含めることができ、進行状況は目標の進捗を示します。 |
