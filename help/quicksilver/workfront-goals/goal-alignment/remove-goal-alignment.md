...
title: Adobe Workfront目標の目標の整合を削除
description: 2 つの目標を結び付ける意味がなくなった場合は、2 つの目標間の整列を削除できます。
author: Alina
...
# Adobe Workfront目標の目標の整合を削除

2 つの目標を結び付ける意味がなくなった場合は、2 つの目標間の整列を削除できます。
...
* 1 つ以上の子目標が関連付けられている親目標。 子の目標は、目標の進捗状況インジケーターです。

## 目標の整列の削除に関する考慮事項

2 つの目標間の位置揃えを削除する際は、次の点を考慮してください。
...
1. 次にアクセス： **目標** 」領域に移動し、目標の名前をクリックして、目標のページを開きます。
1. 親目標の目標のページで、 **進行状況インジケーター** をクリックします。

   ![](assets/remove-goal-alignment-from-list-unshimmed.png)

1. 内 **タイプ：目標** グループ化を選択し、目標を選択して、 **切断** アイコン ![](assets/disconnect-goal-to-remove-alignment-icon-unshimmed.png) をクリックします。

   [ 切断 ] ボックスが表示されます。

1. クリック **切断** 選択した目標を親から切断します。

   目標はスタンドアロンの目標になり、元の目標の進捗状況インジケーターとして表示されなくなります。 接続解除された目標の進行状況は、元の目標の進行状況に影響を与えなくなりました。

   成功メッセージがページの右上隅に表示され、目標が切断されたことを確認します。