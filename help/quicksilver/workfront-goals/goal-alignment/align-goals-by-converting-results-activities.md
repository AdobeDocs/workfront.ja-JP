...
2つの目標を手動で調整することも、既存の目標の結果とアクティビティを別の目標に変換することもできます。変換された結果またはアクティビティは、元の目標の子の目標になります。
2つの目標を手動で調整する方法について詳しくは、[Adobe Workfront Goalsで目標を連携させる](../../workfront-goals/goal-alignment/align-goals-by-connecting-them.md)を参照してください。
...
1. 進行状況インジケーターのリストで結果またはアクティビティを選択し、進行状況インジケーターリストの上部にある「**目標に変換**」アイコン「![目標に変換](assets/convert-to-goal-icon-unshimmed.png)」をクリックします。 「目標に変換」ボックスが開きます。

   ![目標ボックスに変換](assets/convert-to-goal-box-unshimmed.png)
1. 次の情報を更新します。
