...
新機能とリリース日について詳しくは、 [マエストロリリースアクティビティ](/help/quicksilver/maestro/release-activity.md).
...
* **リンクされたフィールド** （または参照フィールド）:2 つのレコードタイプ間の接続を確立し、個々のレコードをリンクした後、接続元のレコード上のリンクされたレコードのフィールドを参照できます。
  ...
  例えば、Maestro キャンペーンをWorkfrontプロジェクトと接続する場合、Maestro キャンペーンレコードに、接続されたプロジェクトの「プロジェクト所有者」フィールドを表示できます。
...
組織が Maestro へのアクセス権を受け取り、システムまたはグループ管理者が Maestro 領域をメインメニューに追加したことを確認します。 詳しくは、 [アクセスの概要](/help/quicksilver/maestro/access/access-overview.md) .
