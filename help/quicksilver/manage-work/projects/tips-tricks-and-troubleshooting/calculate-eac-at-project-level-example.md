...
1. **タスク 3 の CPI** = .38 は、次のように計算されます。\
...
1. **プロジェクトの EAC** = 224.49 時間は、次のように計算されます。\
   **プロジェクトの EAC** = *IF* CPI &lt;> 0 *THEN* EAC = 予定時間数/CPI\
   *ELSE* EAC =予定時間数+実際の時間数\
...
1. **タスク 1 の CPI****** = .14 は以下のように計算されます。\
   **CPI** **タスク 1 の場合** = *IF* 実際の労力コスト + OccuredActualExpenseCost &lt;> 0 *THEN*
   ...
   * ELSE* CPI = CPI_Labor\
     ...
     **CPI** **タスク 1 の場合** = 400 / 2900\
     **CPI** **タスク 1 の場合** = .14****

1. **タスク 1 の EAC****** = $13,400.00\
   **CPI 労力** タスク 1 の **** =実際の労力コストが &lt;> 0 の場合
   ...
   ELSE CPI_Labor = 1\
   **CPI 労力** タスク 1 の **** = 100/2500\
   **CPI 労力****タスク 1 の場合** = .04*****EAC 労力 ****タスク 1 の場合**=* IF *CPI_Labor &lt;> 0* THEN *EAC 労力=予定労力コスト/CPI_Labor
...
1. **プロジェクトの EAC** = $28,200.00 は以下のように計算されます。\
   ...
   **EAC****プロジェクト** = 22,500.00 ドル + 5,700.00 ドル\
   **EAC****プロジェクト** = 28,200.00 ドル
...
1. タスク 2 の **CPI** = .17 は以下のように計算されます。\
   **CPI タスク 2** = *IF* 実際の労力コスト + OccuredActualExpenseCost &lt;> 0 *THEN* CPI = （TotalBudgetedCostWorkPerformed + OccuredPlannedExpenseCost） / （ActualLaborCost + OccuredActualExpenseCost）
   * ELSE* CPI = CPI_Labor\
...
1. タスク 3 の CPI = .31 は以下のように計算されます。\
   **CPI****タスク 3** = *IF* 実際の労力コスト + OccuredActualExpenseCost &lt;> 0 *THEN*
   ...
   * ELSE* CPI = CPI_Labor\
     ...
     **CPI****タスク 3** = 1650 / 5400\
     **CPI****タスク 3** = .31 ****** EAC、タスク 3 **= 9,521.74 ドルは次のように計算されます。\
...
1. 以下の計算のとおり、タスク 1 の CPI = .16 になります。\
   **CPI****タスク 1** = *IF* 実際の労力コスト + OccuredActualExpenseCost &lt;> 0 *THEN*
...
1. 以下の計算のとおり、**プロジェクトの EAC** = 32,248.98 ドルになります。\
   ...
   **EAC****プロジェクト** = EAC 労力 + EAC 費用\
   **EAC****プロジェクト** = 22,448.98 + 9,800.00\
   **EAC****プロジェクト** = 32,248.98 ドル
