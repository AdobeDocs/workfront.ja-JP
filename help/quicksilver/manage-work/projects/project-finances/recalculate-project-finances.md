...
>
>プロジェクト（財務データ）レポートを実行すると、財務データの再計算が行われるので、レポートを実行する前に同じ事項を考慮する必要があります。
...
一括編集中に財務を再計算できます。詳しくは、を参照してください [手動による一括の財務の再計算](#manually-recalculate-finances-in-bulk) この記事のセクションです。

1. 財務を再計算するプロジェクトに移動して、 **詳細** アイコン ![](assets/qs-more-icon-on-an-object.png) をプロジェクト名の右側に表示します。
...
1. クリック **財務の再計算**.
   ...
   プロジェクトの財務が正常に再計算されたことを示す確認がブラウザーの上部に表示されます。
ロックされていない既存のコスト値と一部の収益値は、新しいレートを反映するように更新されます。

## 手動による一括の財務の再計算{#manually-recalculate-finances-in-bulk}
...
1. プロジェクトのリストに移動します。
1. リストで複数のプロジェクトを選択し、 **詳細** アイコン ![](assets/qs-more-icon-on-an-object.png) リストの上部。
...
1. クリック **財務の再計算**.
   ...
   選択したプロジェクトのすべての予定コストと収益が、新しい情報があれば再計算されます。
