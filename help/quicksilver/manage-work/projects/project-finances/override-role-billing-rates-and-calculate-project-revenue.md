...
title: プロジェクトでの請求レートの上書きと収益の計算の概要
description: 請求レートを使用して、プロジェクトに費やした時間で乗算したプロジェクトの収益を計算できます。
author: Lisa
...
### 概要 – Workflow Ultimate パッケージ

ユーザーと担当業務に関連付けられた請求レートは、プロジェクトレベルで上書きできます。
...
ユーザーの請求率に基づいてプロジェクトの収益を計算するには、プロジェクトのタスクの収益タイプは次のいずれかである必要があります。
...
### 概要 – その他のすべてのWorkfrontおよびWorkflow パッケージ
...
## 収益を計算する際の請求レートの上書きの階層 – Workflow Ultimate パッケージ

>[!NOTE]
>
>ユーザーと役割の時間単位の収益タイプは、収益計算の請求レートを検索する際に、詳細な階層に従います。 詳しくは、[収益とコスト階層の概要](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)を参照してください。
...
* Workfrontの管理者は、属性と有効日にもとづいて、役割ごとに複数の請求レートを設定するレートカードを定義できます。 レートカードがプロジェクトに添付されると、すべての役割、属性、および関連する請求レートがプロジェクトの請求レート セクションに追加されます。 評価カードを添付すると、プロジェクト上の既存の請求レートが上書きされます。
...
## 収益を計算する際の請求レートの上書きの階層 – その他のすべてのWorkfrontおよびワークフローパッケージ
...
また、レートカードをプロジェクトに添付することもできます。これにより、ジョブロールの請求率がレートカードからプロジェクトにインポートされます。
...
プロジェクトの収益を計算するために使用される担当業務の詳細については、[請求と収益の概要](../../../manage-work/projects/project-finances/billing-and-revenue-overview.md)の「[&#x200B; ユーザーと役割の割り当てに基づくタスクの収益計算](../../../manage-work/projects/project-finances/billing-and-revenue-overview.md#revenue-calculations-for-tasks-based-on-user-and-role-assignments)」セクションを参照してください。
...
## プロジェクトの「請求レート」セクションの概要 – ワークフローUltimate パッケージ

プロジェクトに関連付けられている担当業務の上書き請求レートを指定すると、プロジェクトの「**レート/請求レート**」タブにすべての担当業務とその上書きが表示されます。

### レート グループ化

請求レートは、最初にソース（レートカードまたは上書き）によって&#x200B;**請求レート**&#x200B;領域にグループ化され、次にリソースタイプ（担当業務またはユーザー）によってグループ化されます。

担当業務に対応するグループ化ラインで、**値**&#x200B;列のプロジェクト レベルでその担当業務の請求率に注意してください。 担当業務に日付有効な上書き率がある場合、レートは日付順に表示されます。

担当業務に対して上書きレートまたはレートカードレートが存在する場合、プロジェクトの収益を計算する際に、担当業務のシステムのデフォルトのレートが自動的に適用されません。 収益タイプに基づいて、階層に従って請求レートが決定されます。 詳しくは、[収益とコスト階層の概要](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)を参照してください。

### 担当業務の複数の請求レート値

特定の担当業務に対して上書き請求レートが複数ある場合は、担当業務のグループの下に表示されます。 インライン編集を使用すると、このタブで上書き請求率の上書き率と&#x200B;**開始日**&#x200B;および&#x200B;**終了日**&#x200B;を変更できます。 または、レートを選択し、**編集** アイコンをクリックして、日付有効な上書きレートを設定します。

>[!NOTE]
>
>日付有効率に対して&#x200B;**開始日**&#x200B;または&#x200B;**終了日**&#x200B;が空白の場合、Workfrontは、最初の上書きの&#x200B;**終了日**&#x200B;より古い日付のすべての時間に対して最初の上書き率が適用され、最後の上書きの&#x200B;**開始日**&#x200B;より新しい日付のすべての時間に対して最後の上書き率が適用されることを前提としています。
> プロジェクトの予定開始日より1時間前に記録された場合、最初の請求レートが使用されます。\
>プロジェクトの予定完了日の後に 1 時間が記録された場合は、最後の請求レートが使用されます。

## プロジェクトの「請求レート」セクションの概要 – その他のすべてのWorkfrontおよびワークフローパッケージ
...
**請求レート**&#x200B;エリアでは、請求レートはそれぞれの担当業務別にグループ化されます。
...
特定の担当業務に請求レートの上書きが複数ある場合、その担当業務のグループ化の下に、複数の請求レートがリスト表示されます。 インライン編集を使用すると、このタブで上書きレートを変更したり、上書き請求レートの&#x200B;**開始**、**日付**&#x200B;および&#x200B;**終了日**&#x200B;を変更したりできます。 または、レートを選択し、**編集** アイコンをクリックして、日付有効な上書きレートを設定します。
...
1回限りの請求レートの上書きに基づいて予定収益を計算する場合は、次の点を考慮してください。

* タスクの&#x200B;**収益タイプ**&#x200B;が&#x200B;**役割（毎時）**&#x200B;の場合、Workfront は、タスクの予定時間数に、タスクに関連付けられた担当業務の請求レートを掛け算して、タスクの予定収益を計算します。

* タスクの&#x200B;**収益タイプ**&#x200B;が&#x200B;**User and Role Hourly**&#x200B;の場合、Workfrontは階層を使用して各タスクの請求率を決定します。 詳しくは、[収益とコスト階層の概要](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)を参照してください。
...
複数の請求レートの上書きに基づいて予定収益を計算する場合は、次の点を考慮してください。

* タスクの&#x200B;**収益タイプ**&#x200B;が&#x200B;**役割（毎時）**&#x200B;の場合、Workfront は、タスクの予定時間数に、タスクに関連付けられた担当業務の請求レートを掛け算して、タスクの予定収益を計算します。

  予定収益の計算に使用される担当業務について詳しくは、[請求と収益の概要](../../../manage-work/projects/project-finances/billing-and-revenue-overview.md)の記事の「ユーザーおよび役割の割り当てに基づくタスクの収益計算について」の節を参照してください。

* タスクの&#x200B;**収益タイプ**&#x200B;が&#x200B;**User and Role Hourly**&#x200B;であり、ユーザーが割り当てられていない場合、Workfrontは、タスクの予定時間に次のいずれかを掛けて、タスクの予定収益を計算します。

   * タスクの担当業務の手動上書き率
   * プロジェクトの担当業務レート。レートカードまたはシステムレベルから取得できます。

  予定収益の計算に使用される担当業務の詳細については、記事[請求と収益の概要](/help/quicksilver/manage-work/projects/project-finances/billing-and-revenue-overview.md)の記事[収益とコスト階層の概要](/help/quicksilver/manage-work/projects/project-finances/overview-revenue-cost-hierarchy.md)およびユーザーと役割の割り当てに基づくタスクの[収益計算](/help/quicksilver/manage-work/projects/project-finances/billing-and-revenue-overview.md#revenue-calculations-for-tasks-based-on-user-and-role-assignments) セクションを参照してください。
...
実収益の請求率は、タスクのログ時間の所有者に基づいています。 「所有者」は、タスクに割り当てられていない場合でも、タスクに対して時間が記録される人物です。
...
### 異なるタイムゾーンのユーザーの予定収益の計算 {#calculate-planned-revenue-for-users-in-different-time-zones}
...
異なるタイムゾーンのユーザーの予定収益を計算する場合は、次の点を考慮してください。

* **収益タイプ**&#x200B;の&#x200B;**役割時間**&#x200B;または&#x200B;**ユーザーと役割時間**&#x200B;を持ち、担当業務に割り当てられているタスクの場合、**予定収益**&#x200B;は、タスクの&#x200B;**予定時間**&#x200B;に担当業務の請求率を掛けて計算されます。
...
### 異なるタイムゾーンのユーザーの実際の収益の計算 {#calculate-actual-revenue-for-users-in-different-time-zones}

異なるタイムゾーンのユーザーの実収益を計算する場合は、次の点を考慮してください。

* タスクの&#x200B;**収益タイプ**&#x200B;が&#x200B;**Role Hourly**&#x200B;または&#x200B;**User and Role Hourly**&#x200B;の場合、Workfrontは、タスクの&#x200B;**Actual Hours**&#x200B;と、タスクに割り当てられた担当業務の請求率を掛けて&#x200B;**Actual Revenue**&#x200B;を計算します。 実際の時間数は、タスクに直接記録される時間です。
...
APIを使用して担当業務に新しい請求レートを追加するには、*PUT メソッド*&#x200B;を使用して、**Rate** オブジェクトに対して&#x200B;*setRatesForRole* アクションを実行します。
**Rate** オブジェクトのアクションと日付フィールドは、API バージョン 8.0で使用できます。
プロジェクトの担当業務に対して既に複数の請求レートを定義しており、新しい日付範囲で新しい請求レートを追加する場合は、同じAPI呼び出しに既存のレートと追加するレートの両方を含める必要があります。これは、オブジェクトのコレクションを更新する方法と同様です。
