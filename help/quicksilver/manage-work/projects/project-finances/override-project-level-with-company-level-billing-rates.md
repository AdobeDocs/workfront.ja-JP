...
## 会社レベルの請求レートの上書きオプションをオンにします

会社がプロジェクトに関連付けられていて、このオプションがオンになっている場合、会社レベルの請求レートに対する変更は、プロジェクトに設定された請求レートを上書きします。
...
1. 「**財務**」セクションで、**会社レベルの請求レートを許可して、プロジェクトレベルの請求レート**&#x200B;を上書きします。

   >[!CAUTION]
   >
   >このオプションを有効にすると、請求済みとしてマークされていない限り、収益計算の履歴が上書きされます。 請求記録を作成することで、収益計算の履歴を保持できます。 詳しくは、[請求記録の作成](../../../manage-work/projects/project-finances/create-billing-records.md)の記事を参照してください。
...
1. 1 つ以上のプロジェクトの会社の料金を更新するには、次のいずれかの操作を行います。
   ...
   * 複数のプロジェクト：
     ...
     1. プロジェクトのリストに移動します。
     1. 更新するプロジェクトのチェックボックスをオンにします。
     1. リストの上部にある&#x200B;**詳細** メニュー![詳細メニュー](assets/qs-more-icon-on-an-object.png)をクリックし、**財務の再計算**&#x200B;をクリックします。
