...
Adobe Workfront管理者は、プロジェクトシステムの環境設定の一部として、EACの計算方法を定義できます。 EAC は、次の 2 つの方法のいずれかで計算できます。
...
Workfrontでは、EACの計算は、プロジェクトの選択したパフォーマンス指標法（PIM）によって異なります。 お使いのシステムまたはプロジェクトでの PIM の設定について詳しくは、[パフォーマンスインデックスメソッド（PIM）の設定](../../../manage-work/projects/project-finances/set-pim.md)を参照してください。
...
&#42; コストパフォーマンス指標[ コストパフォーマンス指標（CPI） ](../../../manage-work/projects/project-finances/calculate-cpi.md) = 0の場合、EAC =合計予定時間+実際の時間を計算します。 これは、時間が取り込まれたが、プロジェクト／タスクが 0%完了の場合に発生します。
...
<pre>EAC労力= <em>IF</em> CPI労力&lt;&gt; 0 THEN EAC労力=予定労力コスト / CPI労力</pre><pre><em>ELSE</em>各労力=予定労力コスト +実際の労力コスト</pre><pre>CPI 労力 = IF 実際の労力コスト &lt;&gt; 0 THEN CPI Labor = TotalBudgetedCostWorkPerformed / 実際の労力コスト</pre><pre>それ以外の場合CPI労力= 1 </pre>EAC の計算時には、次のフィールドが考慮されます。

* 予算計上原価計算（BCWP） =計画された作業の予算計上原価（予算計上原価）と、これまで完了したタスクのパーセントを乗算した結果。
   ...
   * **親タスクの場合：**
合計予算計上原価作業実績=すべての直接子タスクの「予算計上原価作業実績」フィールドの合計。

   * **プロジェクト：**
合計予算計上原価作業実行済=すべてのトップレベルのタスク（親タスクとスタンドアロンのタスク）に対する「予算計上原価作業実行済」フィールドの合計。

* EAC費用= 「未発生予定費用」コストに「未発生実績費用」コストを追加した結果。 次の式で計算されます。

  ```
  EAC Expense = Incurred Actual Expense Cost + Not Incurred Planned Expense
  ```

   * 未収実績費用コスト =実績金額フィールド > 0のすべての費用の「予定金額」フィールドの合計。 例えば、タスク 1 の経費を作成し、「予定金額」フィールドに 500.00 ドルと入力し、「実際の金額」フィールドに金額 > 0（つまり 600.00 ドル）と入力した場合、このタスクで発生した予定費用コストは 500.00 ドルになります。
   * 未発生予定費用=「実績金額」フィールド = 0のすべての費用の「予定金額」フィールドの合計。 例えば、タスク 1 に 2 つの費用を作成して、最初の費用の「予定金額」フィールドの値が 500.00 ドルで「実際の金額」の値が 600.00 ドル、2 番目の費用の「予定金額」フィールドの値が 300.00 ドルで「実際の金額」フィールドの値が 0.00 ドルの場合、このタスクの未発生の予定費用の値は 300.00 ドルです。
...
1. 「**財務**」をクリックします。
   ...
   プロジェクト ](assets/eac-highlighted-on-project-350x112.png)の![EAC
