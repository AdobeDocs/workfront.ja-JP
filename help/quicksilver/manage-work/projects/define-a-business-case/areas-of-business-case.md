...
Adobe Workfrontの管理者またはグループ管理者は、「プロジェクト情報」セクションを除き、「ビジネスケース」のすべてのセクションをプロジェクトに表示する前に有効にする必要があります。 「プロジェクト情報」セクションはデフォルトで有効になっています。

ビジネスケースの領域を有効にする方法について詳しくは、記事[&#x200B; システム全体のプロジェクト環境設定](../../../administration-and-setup/set-up-workfront/configure-system-defaults/set-project-preferences.md)の「ビジネスケース」セクションを参照してください。
...
ビジネスケースの「プロジェクト情報」セクションには、プロジェクトが実際に開始される前のプロジェクトの基本情報が含まれています。

すべてのプロジェクトには、事前に設定されたフィールドを含むプロジェクト情報エリアがあります。つまり、Workfront管理者は、このエリアに表示されるフィールドを設定できません。

次のフィールドの編集を検討してください。

* **説明**: プロジェクトの説明を追加します。

* **プロジェクト所有者**: デフォルトでは、プロジェクトを作成するユーザーはプロジェクト所有者でもあります。 このフィールドを編集して、別のアクティブなユーザーをプロジェクトの所有者として選択します。

* **プロジェクト スポンサー**: プロジェクト スポンサーとなるアクティブ ユーザーを選択します。 スポンサーはビジネスケースの承認を受けます。

* **Portfolio**: プロジェクトのPortfolioを選択します。 このドロップダウンメニューで選択する前に、Portfolioを作成し、アクティブステータスに配置する必要があります。
...
* **予定利益**：このプロジェクトが完了したときに、組織が予定する金銭上の利益を見積もります。 任意の金額の通貨を指定でき、正の値（10,000 ドルなど）である必要があります。

* **ステータス**: デフォルトでは、プロジェクトリクエストのステータスはアイデアに設定されています。 ステータスを「アイデア」または「計画中」以外に変更すると、「ビジネスケースの概要」領域から「送信」ボタンが消え、ビジネスケースを承認用に送信できなくなります。
...
* **固定終了日**: プロジェクトを終了する日付を指定します。
  ...
  >[!NOTE]
  >
  >ビジネスケースの「固定開始日」と「終了日」は、プロジェクトの予定開始日と完了日には影響しません。 プロジェクトクリエイターが、プロジェクトの理想的な開発期間として要求する日付を表します。 プロジェクトの予定開始日と予定完了日には、プロジェクトのタスクに基づくプロジェクトの予定タイムラインが表示されます。

## Goals

目標：プロジェクトの目的を定義します。 この領域は、ビジネスケースではデフォルトで有効になっていますが、Workfront管理者は表示しない場合があります。 このフィールドには、目標が優先順に表示されます。
...
>[!TIP]
>
>プロジェクトの個々のビジネスケースに関連していない組織の戦略目標を作成できます。 戦略目標を設定するには、Adobe Workfront Goalsにアクセスする必要があります。 その後、ビジネスケース外のプロジェクトに接続できます。 Workfront Goals を使用した目標の作成について詳しくは、[Adobe Workfront Goals の概要](../../../workfront-goals/goal-management/wf-goals-overview.md)を参照してください。

目標の定義は、プロジェクトがPortfolio Optimizerでスコアを受け取るためのオプションです。 このセクションは、ビジネスケースの唯一のオプションセクションです。 Portfolio Optimizerでプロジェクトをスコアリングする前に、ビジネスケースの他のすべてのセクションを完了する必要があります。 目標を作成する際に、目標の優先度レベルを指定できます。

詳しくは、[&#x200B; ビジネスケース目標の作成](../../../manage-work/projects/define-a-business-case/create-business-case-goals.md)を参照してください。

## 費用

費用は、プロジェクトの存続中に発生する可能性のある役務以外の費用を表します。 この領域は、ビジネスケースではデフォルトで有効になっていますが、Workfront管理者は表示しないように設定できます。

ビジネスケースに入力した費用は、プロジェクトの「費用」タブにも「予定費用」として入力されます。
...
プロジェクトのリソースの予算計上時間は、ビジネスケースのリソース予算計上領域に表示され、プロジェクトの予算計上労力コストが生成されます。 この領域はデフォルトで有効になっています。
...
* リソースの予算計上後、役割が時間当たり単価レートに関連付けられている場合、プロジェクトの予算計上労力コストがリソース予算計上領域に表示されます。 予算計上労力コストは、プロジェクトの通貨で表示されます。
...
リスクとは、プロジェクトが期限や予算を超過せずに完了することを妨げる可能性のある要因のことです。 これらの要素を定義することは、Portfolioの管理者またはプロジェクトスポンサーが、プロジェクトの承認について十分な情報を得た上で意思決定を下すために重要です。 この領域は、ビジネスケースではデフォルトで有効になっていますが、Workfront管理者は表示しないように設定できます。

潜在的なコストは、発生した場合に定義するリスクに関連付けることができます。 リスクのコストは、プロジェクトの純価値に影響します。
...
リスクの作成について詳しくは、[&#x200B; プロジェクトのリスクの作成と編集](../../../manage-work/projects/define-a-business-case/create-edit-risks-on-projects.md)を参照してください。

カスタムリスクタイプの作成と編集について詳しくは、[&#x200B; リスクタイプの編集と作成](../../../administration-and-setup/set-up-workfront/configure-system-defaults/edit-create-risk-types.md)を参照してください。

## スコアカード

スコアカード：プロジェクトの整合性を測定します この領域は、ビジネスケースではデフォルトで有効になっていますが、Workfront管理者は表示しないように設定できます。

スコアカードを適用するには、Workfront管理者が最初にスコアカードを作成する必要があります。 スコアカードを作成しない限り、ビジネスケースの「スコアカード」領域は表示されません。
...
ビジネスケースを定義する際に、カスタム Formsをプロジェクトに添付できます。 この領域は、ビジネスケースではデフォルトで有効になっていません。Workfront管理者は、ビジネスケースに表示するために、この領域を有効にする必要があります。
...
ビジネスケースの右上隅にあるビジネスケースの概要パネルに、メインプロジェクトの財務の概要と、プロジェクトがスコアカードに一致しているかどうかの概要が表示されます。

ビジネスケースの概要は、財務フィールドとスコアカードに関連するプロジェクトの状態を簡単に確認するだけで、編集することはできません。
...
PDF ファイルを印刷する必要がある場合や、より圧縮された形式で電子メールに添付する必要がある場合は、ビジネスケースを書き出すことができます。
