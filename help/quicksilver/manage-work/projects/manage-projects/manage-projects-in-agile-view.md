...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>チーム、プロ、ビジネス、エンタープライズ </p> </td> 
  </tr> 
  <tr> 
   <td> <p>Workfront License Type</p> </td> 
   <td> <p>レビュー、作業、またはプラン </p> </td> 
  </tr> <!--
...
 一般的にアジャイルプラクティスに伴う管理上の課題（チームのバックログの管理や反復の作成など）がない場合。
...
プロジェクトをアジャイルビューで表示する場合、プロジェクト内のタスクを読み取ることはできません。 タスクの順序を変更すると、依存関係を持つ他のタスクに影響を与える可能性があるので、タスクの順序を変更するには、標準ビューでプロジェクトを表示する必要があります。
...
1. 次をクリック： **アジャイル** アイコン\
   ![アジャイルアイコン](assets/agile-icon-nwe.png)\
   デフォルトのアジャイルビューにプロジェクトが表示されます。\
   以前にカスタムアジャイルビューでプロジェクトを表示した場合、プロジェクトはデフォルトのアジャイルビューではなく、そのビューに表示されます。

1. （オプション）カスタムアジャイルビューを作成した場合、または別のユーザーがカスタムアジャイルビューを作成して共有した場合、デフォルトのアジャイルビューの代わりにそのビューを表示できます。\
...
1. （条件付き）プロジェクト内のタスクで、「新規」、「処理中」、「完了」（アジャイルビューのデフォルトステータス）以外のステータスを使用している場合は、それらのステータスのタスクを表示するには、アジャイルビューにステータスを追加する必要があります。\
