...
1. 左側のパネルで「**財務**」をクリックします。
   ...
   ![&#x200B; プロジェクト編集ボックス内の財務領域](assets/nwe-finance-in-edit-project-box-350x183.png)
...
1. プロジェクトの次の財務情報を更新します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr>
      <td><strong><span class="preview">システムの日付の実効為替レートを使用</span></strong></td>
      <td><p><span class="preview">このオプションを選択すると、システムレベルの日付有効な為替レートが常に使用され、為替レートの上書きはプロジェクトで許可されません。</span></p> <p><span class="preview">オプションが選択されている場合は、<strong>為替レート日付</strong>から開始する必要があります。</span></p></td> 
     </tr>
     <tr>
      <td><strong><span class="preview">プロジェクト請求レート情報を保持</span></strong></td>
      <td><p><span class="preview">このオプションを有効にすると、すべての割り当て請求レートは、請求階層から適用できる最も高いレートに永続的に固定されます。 これは1回限りの選択であり、プロジェクトに割り当てまたは時間が設定されている場合は、このオプションを無効にすることはできません。 保存後は、プロジェクトの任意の場所で割り当て請求率を変更、編集、または上書きすることはできません。</span></p></td>
     </tr>
