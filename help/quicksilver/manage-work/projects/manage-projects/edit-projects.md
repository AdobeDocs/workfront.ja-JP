...
1. プロジェクトに関する次の情報を更新します。
   ...
   <table style="table-layout:auto"> 
      ...
      <li>組織でWorkfrontとAdobeの両方のドキュメントストレージタイプを使用している場合、Workfront ストレージプロジェクトをAdobe ストレージプログラムに関連付けることはできません。 その逆も同じです。 Workfront インスタンスには、両方の種類のドキュメントストレージがない可能性があります。 プログラムは、属するポートフォリオからストレージタイプを継承します。 </li></ul>
...
* 異なる種類のドキュメントストレージを持つ複数のプロジェクトを選択し、選択したプロジェクトのPortfolioまたはプログラムを更新する場合、Workfront ストレージプロジェクトをAdobe ストレージポートフォリオに関連付けることはできません。 その逆も同じです。 この場合、一度に同じストレージタイプを持つプロジェクトのみを選択してください。
