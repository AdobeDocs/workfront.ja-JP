...
>[!NOTE]
>
>予定日は、必ずしも両方のシステム間で対応しているわけではありません。 不一致の原因は、スケジュールや Workfront と Microsoft Project の間のシステム設定の違いです。これらの日付の不一致により、作業量、期間、完了率に違いが生じる場合もあります。
...
Workfrontに読み込まれない、または書き出されないプロジェクトフィールドが多数あります。
