...
* メールに添付されたドキュメントがある場合は、そのドキュメントが Workfront のイシューに添付されます。
  ...
  >
  >* 環境が従来のWorkfrontとAdobe エンタープライズ版の両方のドキュメントストレージにアクセスできる場合、Workfront管理者が設定領域のデフォルトのストレージ環境設定に対して選択した内容に関係なく、リクエストはプロジェクトからドキュメントストレージタイプを継承します。 詳しくは、[&#x200B; プロジェクトおよび関連オブジェクトのドキュメント管理の概要](/help/quicksilver/manage-work/projects/manage-projects/manage-documents-on-projects.md)を参照してください。
