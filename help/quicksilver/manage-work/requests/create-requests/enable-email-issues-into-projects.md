...
title: ユーザーが問題をリクエストキュープロジェクトに電子メールで送信できるようにする
description: ユーザーがメールでプロジェクトにイシューを追加できるようにプロジェクトを設定できます。
author: Alina
...
ユーザーがメールでプロジェクトにイシューを追加できるようにプロジェクトを設定できます。 プロジェクトがリクエストキューとして指定されている場合にのみ、問題をプロジェクトに電子メールで送信することを許可できます。 リクエストキュープロジェクトの作成について詳しくは、[リクエストキューを作成](../../../manage-work/requests/create-and-manage-request-queues/create-request-queue.md)を参照してください。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <ul><li>Adobe Workfront</li><li>プランニングリクエストまたはリクエストフォームを表示するには、Adobe Workfront Planningが必要です</td> 
  </tr> 
 </tbody> 
</table>
...
ユーザーが電子メールでプロジェクトに問題を追加できるように、プロジェクトを設定するには、次の条件が必要です。
...
>
>* リクエストキューに複数のキュートピックまたはトピックグループがある場合、Workfrontは、メールされたリクエストが送信されるキュートピックをランダムに選択し、メールされたリクエストの管理が困難になります。
>電子メールを通じてリクエストを受信するように設定したプロジェクトには、複数のキューのトピックを含めないことをお勧めします。送信されたリクエストが異なるリソースまたはプロジェクトを対象としている場合は、送信後に手動でルーティングまたは移動する必要があります。
...
* メールに添付されたドキュメントがある場合は、そのドキュメントが Workfront のイシューに添付されます。
  ...
  >[!NOTE]
  >
  >* MSG ファイルはサポートされておらず、Workfrontの問題に添付されません。
  >
  >* 環境が従来のWorkfrontとAdobe クラウドストレージの両方にアクセスできる場合、Workfront管理者が設定領域のデフォルトのストレージ環境設定に対して選択した内容に関係なく、リクエストはプロジェクトからドキュメントストレージタイプを継承します。 詳しくは、[ プロジェクトおよび関連オブジェクトのドキュメント管理の概要](/help/quicksilver/manage-work/projects/manage-projects/manage-documents-on-projects.md)を参照してください。
