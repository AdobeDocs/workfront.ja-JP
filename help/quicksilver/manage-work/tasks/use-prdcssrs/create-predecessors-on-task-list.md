...
1. 選択したタスクの先行タスクとして指定するタスク番号を入力し、**Enter**&#x200B;を押します。
   ...
   >
   >1. 先行タスクのプロジェクトの参照番号に、コロンとタスクの番号を入力します。 例えば、*765021:12*&#x200B;と入力すると、先行タスクのプロジェクトの参照番号が765021で、先行タスクがプロジェクトのタスク番号12であることを示します。
   >
   ...
   >**重要**
   >
   >タスクリストが手動保存モードで表示されている場合、プロジェクト間の先行タスクを追加することはできません。
...
1. タスクリストで、**先行タスク**&#x200B;列の先行タスク番号にカーソルを合わせます。 先行タスクの詳細が表示されたボックスが表示されます。
   ...
   **後継タスク数：**&#x200B;参照されている先行タスクの後継（または依存）タスク数。
