...
title: Adobe Workfrontでのイシューの変換の概要
...
問題をタスクに変換する方法について詳しくは、[問題をタスクに変換](../../../manage-work/issues/convert-issues/convert-issue-to-task.md)を参照してください。

問題をプロジェクトに変換する方法について詳しくは、[問題をプロジェクトに変換](../../../manage-work/issues/convert-issues/convert-issue-to-project.md)を参照してください。

## イシューを変換する際の考慮事項

* イシューをタスクまたはプロジェクトに変換する場合、この記事で特に指定されていない限り、イシューからほとんどの情報がタスクまたはプロジェクトに転送されます。
* Workfrontの管理者またはグループ管理者は、[ システム全体のタスクとイシューの環境設定](../../../administration-and-setup/set-up-workfront/configure-system-defaults/set-task-issue-preferences.md)で説明されているように、イシューに何が起こるか、その解決、およびプロジェクトまたはタスクに変換されるときのプライマリ連絡先のアクセスに関する環境設定を既に設定しています。
...
* 問題を変換する場合、5分の処理制限があります。 問題に多数のドキュメントが添付されていて、変換できない場合は、一部のドキュメントを削除して、もう一度やり直す必要がある場合があります。
...
詳しくは、[問題をプロジェクトに変換](../../../manage-work/issues/convert-issues/convert-issue-to-project.md)を参照してください。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>優先度</td> 
   <td><p>問題からの転送。</p>
   <p>変換されたプロジェクトにテンプレートを使用する場合は、優先度を手動で変更するオプションがあります。変更しない場合は、イシューの優先度がプロジェクトに転送されます。 
    </td> 
  </tr> 
  <tr> 
   <td>URL</td> 
   <td> <p>イシューのURLが新しいプロジェクトに転送されます。 </p> <p> イシューで URL が指定されていない場合、テンプレートの URL がプロジェクトに転送されます。 </p> <p>イシューとテンプレートの両方の「URL」フィールドが空の場合、プロジェクトのフィールドは空になります。 </p> </td> 
  </tr> 
  <tr> 
   <td>プロジェクト状況のタイプ</td> 
   <td>テンプレートから転送します。</td> 
...
>[!CAUTION]
>
>イシューのプライマリ連絡先が変更された場合、またはイシューの変換後にイシューがプロジェクトまたはタスクからリンク解除された場合、変換済みイシューの発信者名は更新されず、イシューの変換時にイシューの元のプライマリ連絡先が表示されます。

## 元のイシューとリクエストリストから変換されたオブジェクトの表示

元のイシューと変換されたオブジェクトは、リクエスト エリアとマイリクエストウィジェットのホームで表示できます。

詳しくは、次の記事を参照してください。

* [送信されたリクエストを表示する](/help/quicksilver/manage-work/requests/create-requests/locate-submitted-requests.md)
