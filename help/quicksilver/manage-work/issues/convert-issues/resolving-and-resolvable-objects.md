...
* イシューと、プロジェクトまたはタスクの両方で、同じキーを使用してカスタムステータスを作成できます。 プロジェクトまたはタスク（解決中オブジェクトとして）がカスタムステータスに変更されると、その変更はイシューステータスにも反映されます。 ステータスキーは、イシューとプロジェクトまたはタスクのステータスで同じにする必要があります。
  ...
  例えば、3文字のコード LCDを使用して、「Launched」という名前のプロジェクトカスタムステータスを作成できます。これは、Currentと同じです。 次に、「Project Launched」という名前のイシューのカスタムステータスを作成します。この場合、レターコードは「LCD」で、「進行中」と表示されます。 プロジェクトを「ローンチ」としてマークすると、イシューはステータスを自動的に「ローンチ済みプロジェクト」に変更します。 「オブジェクトの解決」設定のステータスが有効になっていない場合に、「解決可能な問題の自動更新」ステータスが「進行中」に変更されます。

* 解決済みオブジェクトが「完了」ステータスにマップされるシステムステータスに設定されている場合、解決可能なイシューは、その状態に対する独自のグループのデフォルトのステータスに設定されます。 3文字のステータスキー一致は、カスタムステータスパスにのみ適用され、Workfrontで設定されたシステムステータスには適用されません。
