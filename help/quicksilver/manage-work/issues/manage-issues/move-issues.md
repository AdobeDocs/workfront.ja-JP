...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <li>プロジェクトの「イシュー」セクションでイシューを移動するライト以上</li></ul>
   または
...
* システム管理者またはグループ管理者は、「ユーザーがタスクを移動できるようにする」および「設定エリアで時間を記録した問題」の環境設定に応じて、時間を記録した問題の移動を禁止できます。 詳しくは、[システム全体のタスクおよびイシューの環境設定の指定](/help/quicksilver/administration-and-setup/set-up-workfront/configure-system-defaults/set-task-issue-preferences.md)を参照してください。
...
* **異なるストレージタイプを持つプロジェクト間でイシューを移動する場合**：イシューをWorkfront ストレージプロジェクトからAdobe ストレージプロジェクトにコピーすることはできません。 その逆も同じです。 すべての顧客が両方のタイプのストレージを持っているわけではありません。
...
1. 移動するイシューに移動し、イシュー名の右側にある&#x200B;**詳細** メニュー![詳細メニュー](assets/more-icon.png)をクリックし、**移動先**&#x200B;をクリックします。

   ![問題レベルで移動](assets/nwe-move-at-issue-level-highlighted-350x579.png)

   **イシューを移動**&#x200B;ボックスが表示されます。

   ![問題ボックスを移動](assets/move-issue-box-nwe-350x280.png)
...
1. （条件付き）宛先プロジェクトのタスクの 1 つにイシューを追加するアクセス権がある場合は、アクセス権をリクエストせずに、引き続き選択した宛先プロジェクトにイシューを移動します。
   ...
   ![問題を移動してアクセスを要求](assets/move-issue-request-access-from-project-nwe-350x118.png)
