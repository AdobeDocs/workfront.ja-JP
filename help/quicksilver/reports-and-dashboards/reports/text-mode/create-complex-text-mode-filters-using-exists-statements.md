...
フィルターの例については、 [例 1：問題をPortfolio所有者名でフィルター](#example-1-filter-for-issues-by-portfolio-owner-name) 」の節を参照してください。
...
1. API エクスプローラーを使用して、 **オブジェクトコード** のリンクオブジェクト (Project)\
...
常に *EXISTS* 文と、このフィルターを作成するためのテキストモードインターフェイス。
...
1. 次のコードを **レポートのフィルタールールを設定する** 上の汎用コードを置き換える領域：
   ...
   >[!NOTE]
   >
   >* 元のオブジェクトは、レポートの対象です。問題
   >* Target オブジェクトは「Portfolio」です。
   >* Linking オブジェクトは Project です。
...
1. 次のコードを **レポートのフィルタールールを設定する** 領域：
   ...
   >[!NOTE]
   >
   >* 元のオブジェクトは、Task というフィルターのオブジェクトです。
   >* Target オブジェクトは「Portfolio」です。
