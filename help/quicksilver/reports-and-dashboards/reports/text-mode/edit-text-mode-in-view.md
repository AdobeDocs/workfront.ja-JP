...
description: テキストモードを使用してリストまたはレポートのビューを編集し、標準インターフェイスで使用できないフィールドにアクセスして、より複雑なビューを作成できます。
author: Courtney
...
テキストモードを使用したビューの編集は、レポートとリストで同じです。 レポートまたはリストからビューにアクセスする方法は異なります。
...
1. 次のいずれかの操作を行います。
   ...
   1. リストからビューにアクセスするには、リストに移動し、**表示** ドロップダウンメニューから、変更するビューにマウスを合わせ、**編集** アイコン ![編集アイコン ](assets/edit-icon.png)をクリックします。
...
1. ビルダーの右上隅にある「**テキストモードに切り替え**」をクリックし、「**テキストモードを編集**」をクリックします。
