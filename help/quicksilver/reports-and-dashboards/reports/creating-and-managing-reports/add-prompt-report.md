...
1. （条件付き）「**カスタムプロンプト**」をクリックして、カスタムプロンプトを作成します。
   ...
   * **ドロップダウン項目ラベル：**&#x200B;これは、レポートを実行する前に表示される、プロンプト内のオプションの1つの名前です。
...
>[!CAUTION]
>
>プロンプト付きレポートを共有すると、公開共有リンクを使用してレポートを表示するログイン済みのユーザーとログインしていないユーザーの両方が、プロンプトを使用してレポートを実行できなくなります。 この場合、レポートの結果はプロンプトを適用せずに表示され、表示される情報は、代わりにユーザーのアクセスレベルと権限、またはレポートの実行ユーザーアクセスレベルと権限（設定されている場合）に基づきます。

Workfront からのプロンプトレポートの共有には次の制限があります。

* レポートを公開する場合、ユーザーは適用されたプロンプトを使用してレポートを実行できません。ただし、Workfrontの資格情報を持ち、最初にログインし、レポートに直接Workfrontで移動します（公開共有リンクを介してではなく）。
...
* プロンプト付きレポートの配信をスケジュールすると、電子メールの添付ファイルに、プロンプトなしレポートのデータが含まれます。 ユーザーがメール内のリンクをクリックしてレポートにアクセスすると、まずログインしてレポートを表示し、プロンプトを自分で実行する必要があります。
...
* 日付ベースのプロンプトを使用してレポートを実行する場合、レポート結果はブラウザーのタイムゾーン設定に基づいてフィルタリングされます。 これにより、1か月の最初または最後の日付に関するプロンプト付きレポートに表示される日付範囲にわずかな差異が生じる可能性があります。 ブラウザーのタイムゾーン設定が特定の場所に関連付けられている場合、その場所の現地時間のバリエーション（夏時間の遵守など）も、プロンプトによるレポートに表示される日付に反映されます。 これにより、同じタイムゾーンで場所の設定が異なるユーザー間で、わずかな日付範囲の相違が生じる可能性があります。
