...
1. 新しい列を追加するには、「**列を追加**」をクリックします。
   ...
   既存の列を変更するには、列をクリックし、レポートビルダーの左上隅にある「**この列フィールドに表示**」領域の現在のフィールドの右側にある「**削除**」アイコン「![列アイコンを削除](assets/remove-column-icon.png)」をクリックし、新しいフィールドを入力し、リストに表示されたらクリックします。
