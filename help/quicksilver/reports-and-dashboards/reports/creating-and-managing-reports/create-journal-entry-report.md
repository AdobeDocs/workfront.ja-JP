...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td><strong>上位の ObjCode</strong> </td> 
...
1. Adobe Analytics の **列（表示）** 「 」タブで、次の列を追加します。
...
1. Adobe Analytics の **フィルター** タブ、クリック **フィルタールールを追加**&#x200B;をクリックし、フィルタールールを追加します。 **フィールド名** > **次と等しい** > **ステータス**.
...
1. （オプション）レポートの焦点を絞り、読み込み時間を短縮するには、プロンプトを追加します。
   ...
   >[!IMPORTANT]
   >
   >修飾子を使用するフィルタールールの作成 **次を含む** は、実際に読み込み時間を増やすことができます。 このため、 **次と等しい** 特定のプロジェクトまたは上位のオブジェクト ID に対してフィルタリングできる場合。

   プロンプトを追加する方法については、 [レポートにプロンプトを追加する](../../../reports-and-dashboards/reports/creating-and-managing-reports/add-prompt-report.md).

1. Adobe Analytics の **グループ化** タブ、クリック **既存のグループの適用**&#x200B;を選択し、「 **プロジェクト**.
...
1. Adobe Analytics の **列（表示）** 「 」タブで、次の列を追加します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td> <p style="font-weight: bold;">変更の種類</p> </td> 
      <td> <p>発生した変更のタイプ。 The <strong>削除</strong> 変更がこの列に表示されます。</p> </td> 
     </tr> 
     <tr> 
      <td> <p style="font-weight: bold;">エントリ日</p> </td> 
...
プロジェクトの進行中に重要なフィールドの変更を追跡できます。 これを行うには、次のように、追跡するジャーナルエントリを設定します。
...
1. Adobe Analytics の **列（表示）** 「 」タブで、次の列を追加します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td> <p style="font-weight: bold;">トップ ObjCode</p> </td> 
      <td> <p>「計画完了日」が変更されたフィールドの最も高い親オブジェクト。</p> </td> 
     </tr> 
     <tr> 
      <td> <p style="font-weight: bold;">範囲</p> </td> 
...
1. Adobe Analytics の **列（表示）** 「 」タブで、次の列を追加します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      <td> <p style="font-weight: bold;">フィールド名</p> </td> 
      <td>影響を受けるフィールドの名前。 The <strong>ownerID</strong> がこの列に表示されます。</td> 
     </tr> 
     <tr> 
      <td> <p style="font-weight: bold;">変更の種類</p> </td> 
