...
title: データの書き出し
description: リスト、レポート、ダッシュボード、検索からAdobe Workfrontデータを書き出すことができます。
author: Courtney
...
リスト、レポート、ダッシュボード、検索からAdobe Workfrontデータを書き出すことができます。
...
>[!NOTE]
>
>ダッシュボードは、PDFファイルにのみ印刷またはエクスポートできます。
...
* **50,000行：** PDFファイルおよびタブ区切りファイルのレポートエクスポートで許可されるデータの行数です。

   * Excelファイルの場合、この制限は&#x200B;**65,000行**&#x200B;です。
...
* **256 列**：Excel が 256 列を超えるドキュメントに対して課される制限です。これらのドキュメントは、手動で書き出したり、配信されたレポートに添付して送信したりすることはできません。この制限は、Excel ファイルにのみ存在し、他のサポートされている形式には該当しません。
  ...
  >[!IMPORTANT]
  >
  >レポート列を含むレポートを書き出すと、それ以外の方法で書き出し制限内にレポートが含まれる場合でも、エラーが発生することがあります。
  >
  >書き出し機能を使用して「レポート」列を含むレポートを他のユーザーと共有する場合は、代わりに公開してレポートを共有することを検討してください。 レポートの公開について詳しくは、[Adobe Workfrontでのレポートの共有](/help/quicksilver/reports-and-dashboards/reports/creating-and-managing-reports/share-report.md)を参照してください。
  >
  >書き出し機能を使用して外部でデータを評価する場合は、代わりにWorkfront Data Connectを使用することをお勧めします。 詳細については、[Workfront Data Connectの概要](/help/quicksilver/reports-and-dashboards/data-lake/data-lake-overview.md)を参照してください。
...
1. 「**書き出し**」をクリックし、形式を選択します。
   ...
   **書き出し**&#x200B;アイコン ![書き出しアイコン](assets/export-icon-nwe.png) をクリックし、形式を選択します。
   ...
   PDFの書き出しに使用できるオプションは、Workfrontのユーザー設定の「電子メールロケール」の設定によって異なります。

   * 北米 – レター – 横長、レター – 縦長、その他のサイズ
...
1. 書き出されたファイル内の情報の表示方法を理解するには、この記事の[書き出されたドキュメントを使用](#use-the-exported-document)のセクションを引き続きお読みください。

### ダッシュボードからのデータの書き出し {#export-data-from-a-dashboard}

ダッシュボードから情報を印刷したり、PDFファイルとしてエクスポートしたりできます。
...
次のセクションでは、書き出したファイルに情報がどのように表示されるかを説明します。
...
プロジェクトをPDFに書き出すと、サブタスクはその親タスクに対してインデントされて表示されます。 書き出したリストは、親タスクを折りたたみません。

レポートに特別な表示がない限り、レポートの送信または配信スケジュールを行うときには、常にレポートのデフォルトタブが表示されます。

Webアプリケーションでレポートに特殊な書式が設定されている場合、PDFファイルとExcelファイルの場合にのみ、[詳細]タブと[マトリックス]タブが表示されたときに、レポートに特殊な書式が設定されます。
...
リンクは、Workfront 内のリンクをサポートする任意のオブジェクトを指すことができます。WorkfrontでリストをPDFに書き出すと、元のドキュメントでサポートされているすべてのリンクが、書き出されたドキュメントに残ります。

>[!TIP]
>
>カスタムフィールド列のテキストモードで`valueformat=HTML`行が表示され、エクスポートされたPDFファイルにリンク値が表示されない場合は、テキストモードで列にコード行を追加する必要があります。
>
...
Workfront管理者がグローバルナビゲーションバーのWorkfrontインスタンスにカスタマイズされたブランディングを追加している場合、書き出されたPDFファイルにはカスタマイズされたロゴも含まれます。
