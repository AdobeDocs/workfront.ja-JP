...
>[!NOTE]
>
>ダッシュボードは、印刷するか、PDF ファイルにのみ書き出すことができます。
...
* **256 列**：Excel が 256 列を超えるドキュメントに対して課される制限です。これらのドキュメントは、手動で書き出したり、配信されたレポートに添付して送信したりすることはできません。この制限は、Excel ファイルにのみ存在し、他のサポートされている形式には該当しません。
  ...
  >[!IMPORTANT]
  >
  >レポート列を含むレポートを書き出すと、レポートがリストされた書き出し制限内にある場合でも、エラーが発生することがあります。
  >
  >書き出し機能を使用して、レポート列を含むレポートを他のユーザーと共有する場合は、レポートを公開して共有することを検討してください。 レポートを公開する方法について詳しくは、[Adobe Workfrontでレポートを共有](/help/quicksilver/reports-and-dashboards/reports/creating-and-managing-reports/share-report.md)を参照してください。
  >
  >書き出し機能を使用してデータを外部で評価する場合は、代わりにWorkfront Data Connectを使用することをお勧めします。 詳しくは、[Workfront Data Connectの概要](/help/quicksilver/reports-and-dashboards/data-lake/data-lake-overview.md)を参照してください。
...
1. 「**書き出し**」をクリックし、形式を選択します。
   ...
   **書き出し**&#x200B;アイコン ![書き出しアイコン](assets/export-icon-nwe.png) をクリックし、形式を選択します。
   ...
   PDFの書き出しに使用できるオプションは、Workfront ユーザー設定の電子メールロケール設定によって異なります。

   * 北米 – 文字 – 横、文字 – 縦、その他のサイズ
...
1. 書き出されたファイルに情報がどのように表示されるかを理解するには、この記事の「[書き出されたドキュメントを使用](#use-the-exported-document)」の節を引き続き読んでください。

### ダッシュボードからのデータの書き出し {#export-data-from-a-dashboard}

ダッシュボードから情報を印刷するか、PDF ファイルとして書き出すことができます。
...
次の節では、書き出されたファイルに情報がどのように表示されるかを説明します。
...
プロジェクトをPDFに書き出すと、親タスクにインデントされたサブタスクが表示されます。 書き出したリストは、親タスクを折りたたみません。

レポートに特別な表示がない限り、レポートの送信または配信スケジュールを行うときには、常にレポートのデフォルトタブが表示されます。

レポートにWeb アプリケーションで特殊な書式設定が含まれている場合は、「詳細」タブと「マトリックス」タブが配信されるときに、PDFおよびExcel ファイルに対してのみ、特殊な書式設定でレポートが配信されます。
...
リンクは、Workfront 内のリンクをサポートする任意のオブジェクトを指すことができます。WorkfrontでリストをPDFに書き出すと、元のドキュメントに存在するサポートされているリンクは、書き出されたドキュメント内に残ります。

>[!TIP]
>
>行`valueformat=HTML`がカスタムフィールド列のテキストモードで表示され、リンク値が書き出されたPDF ファイルに表示されない場合は、テキストモードで列にコードの行を追加する必要があります。
>
...
Workfront管理者がグローバルナビゲーションバー用にWorkfront インスタンスにカスタマイズされたブランディングを追加した場合、書き出されたPDF ファイルにはパーソナライズされたロゴも含まれます。
