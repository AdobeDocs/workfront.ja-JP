...
プロジェクト レポートには、予定時間数から実際の時間数を差し引く計算があります。
...
Workfrontで時間を使用しているほとんどのフィールドは、数分で保存されます。 計算でこれらのフィールドを使用する場合、結果は最も頻繁に数分になります。 時間で結果を取得するには、計算の結果または参照しているフィールドを60で割る必要があります。

予定時間は数分で保存されます。

計算に使用する「実際の時間」フィールドに応じて、次の正しい式を使用します。

* 実際の時間の場合、小数点精度で時間に保存されます。

  `valueexpression=SUB(workRequired/60,actualWorkRequiredDouble)`

* 従来の実際の時間数の場合、分単位で保存されます。

  `valueexpression=SUB(workRequired,actualWorkRequired)/60`

>[!NOTE]
>
>すべてのレポートで`actualWorkRequiredDouble`を使用することを強くお勧めします。
>
>レポートのグラフに`actualWorkRequired`が正しく表示されません。
...
レポートの作成者は、システム管理者のアクセス権、またはデータを表示するアクセス権を持つ標準ユーザーまたはプラン ユーザーでレポートを実行するためにレポートを編集できます。
...
* システム管理者は、レポート行の「編集」オプションを設定して、レポートを作成するためのアクセス権を含めることで、標準またはプランのライセンスを持つユーザーがレポートを編集できるようにすることができます。\
...
時には、レポートの所有者が、レポートの「**次のアクセス権限でこの報告書を作成する**」フィールドで指定されたユーザーにもなっている場合があります。 次のユーザーのアクセス権を持つ&#x200B;**このレポートを実行する：** ユーザーが非アクティブ化されている場合、レポートを共有しているユーザーに対してレポートが表示されなくなります。 このような場合は、「**このレポートを実行する」を空白のままにするか、フィールドにアクティブなユーザーを入力することで、レポートに再度アクセスできるようになります。**
...
* オブジェクトの左側のパネルにあるダッシュボードなど、レポートを含むダッシュボード
