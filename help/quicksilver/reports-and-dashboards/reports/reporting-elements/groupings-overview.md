...
>[!NOTE]
>
>次の例外は、/groupingsの次のフィールドの値を集約する場合に、親オブジェクト（親タスクなど）に適用されます。
>
>* 実際の時間を除く、すべての数値、通貨、日付フィールドは、子タスクとスタンドアロンタスクの値のみを集計します。 親タスクまたは親の親の値を集計しません。 親タスクのみを含むリストの数値、通貨、日付のフィールドを集計しても、グループ化バーに集計値は表示されません。
>
>* 実際の時間数は、メインの親タスクとスタンドアロンタスクの値を集計します。子タスクや親タスクの親タスクの数は集計しません。<!--Examples of Actual hours include Planned/Actual Labor Cost, Planned/Actual Expense Cost, Planned/Actual Cost, and Planned Hours.-->
>
