...
例えば、過去1週間、過去1年、今後2週間などです。この方法では、レポートを1回作成しますが、フィルターでワイルドカードを使用するため、レポートを実行する日に適応するので、読むたびに異なる結果が生成されます。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>フィルター、ビュー、グループへのアクセスを編集</p> <p>ポート、ダッシュボード、カレンダーへの編集アクセスでレポートのレポート要素の編集</p>
   </td> 
  </tr> 
  <tr> 
...
1. 「**相対日付を設定**」トグルをクリックし、表示されるテキストボックスで、レポートの実行日と同じ日に発生する情報を表示する場合は、「`$$TODAY`」と入力します。
