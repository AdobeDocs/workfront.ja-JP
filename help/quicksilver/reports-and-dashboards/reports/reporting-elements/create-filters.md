...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>フィルター、ビューおよびグループへのアクセスを編集</p> <p>注意：まだアクセス権がない場合は、Workfront管理者に、アクセスレベルに追加の制限を設定しているかどうかを問い合わせてください。 Workfront管理者がアクセスレベルを変更する方法について詳しくは、 <a href="../../../administration-and-setup/add-users/configure-and-grant-access/create-modify-access-levels.md" class="MCXref xref">カスタムアクセスレベルの作成または変更</a>.</p> </td> 
  </tr> 
  <tr> 
...
<p>注意：フィルターのベータビルダーは、レポートでは使用できません。
</td>
</tr>
</tbody>
</table>
...
## 標準ビルダーでのフィルターの作成または編集 {#create-filter-in-standard-builder}

次の方法で、リストとレポートでフィルターを作成できます。
...
1. 次のいずれかの操作を行います。
   ...
   * 「 **別のフィルタールールを追加**&#x200B;で、ルールを追加するオプションの名前を **フィールド名の入力を開始** 」ボックスをクリックし、ドロップダウンリストに表示されたらクリックします。
   ...
   * クリック **および** または **または** 新しいフィルタールールを追加する場合。\
      ...
      >
      >ただし、タスク：名前「marketing」を含む 2 つの AND フィルターグループ間で変更はありません。2 番目のグループで繰り返す必要があります。
...
## ベータ版ビルダーでフィルターを作成または編集する

様々なインターフェイスを使用してフィルターを作成する際は、次の点を考慮してください。

* ベータビルダーは、上の表に示す領域の標準フィルターインターフェイスと同じ場所にあります。
* ベータ版オプションが利用可能な標準とベータ版ビルダーインターフェイスを切り替えることができます。
* ある領域でベータビルダーを有効にした後は、利用可能なすべての領域で、ベータビルダーがデフォルトのエクスペリエンスになります。 例えば、プロジェクトリストでベータビルダーを有効にした場合、リストでタスクおよび問題フィルターを作成するデフォルトのエクスペリエンスになります。
* 次の方法で、ベータビルダーインターフェイスを使用してフィルターを作成できます。
...
* 保存済みフィルターは、最初に作成したエクスペリエンスに関係なく、両方のビルダーで使用できます。 例えば、標準ビルダーを使用してフィルターを作成した場合、ベータビルダーインターフェイスでもフィルターを検索および変更できます。

   >[!TIP]
   >
   >フィルターが適用されていない場合はすべてのリスト項目が表示されるので、ベータビルダーには「すべて」フィルターは含まれていません。 クリック **すべてクリア** をクリックして、アクティブなフィルターをクリアし、すべての項目を表示します。 If **すべてクリア** がグレー表示になっている場合、フィルターは適用されません。

* 標準ビルダーとベータビルダーでは、AND および OR 演算子を組み合わせた複数文フィルターを作成する際の構文が少し異なります。 その結果、ビルダー間を切り替えると、これらのフィルターの表示が異なる場合があります。

   >[!INFO]
   >
   >次のシナリオが存在します。
   >
   >1. ベータビルダーを使用して、次の構文のフィルターを作成します。
   >
   >   `(A OR B) AND C`
   >
   >1. 標準ビルダーに戻り、標準ビルダーの構文を使用してフィルターを編集します ( [標準ビルダーでのフィルターの作成または編集](#create-filter-in-standard-builder) 」の節を参照してください。 標準ビルダーの構文は、次のようにフィルター文を表示します。
   >
   >   `A AND C`
   >   `OR`
   >   `B AND C`
   >
   >1. 標準インターフェイスのフィルタを変更します。
   >1. ベータビルダーに戻ります。 filter 文は、前述のように、標準ビルダーでサポートされているロジックに従って表示されます。

   >
   >   このフィルターは、ベータビルダーインターフェイスに次のように表示されます。
   >  
   >   `A AND C`
   >   `OR`
   >   `B AND C`
   > 
   >   これは、標準インターフェイスでフィルターが変更されたためです。

ベータビルダーインターフェイスを使用してフィルターを作成します。
...
1. 次をクリック： **フィルター** アイコン ![フィルターアイコン](assets/filter-nwepng.png)、次にを有効にします。 **ベータ設定** ![ベータ設定](assets/beta-toggle-white-on-existing-filters.png) ベータビルダーにアクセスする。 デフォルトでは無効になっています。
   ...
   次に、必要に応じてベータ版の使用許諾契約に同意します。 同意する必要があるのは 1 回のみで、ベータビルダーは有効なままです。

   ベータ版のフィルタービルダーインターフェイスが開きます。

   >[!TIP]
   >
   >ベータビルダーを有効にすると、フィルタービルダーインターフェイスのヘッダーが青に変わります。 ベータビルダーインターフェイスを有効にした後、Workfrontでは、利用可能なすべての領域でベータビルダーインターフェイスが有効なままになります。
...
1. 次のいずれかの操作を行います。
   ...
   * 管理権限を持つ既存のフィルターの上にマウスポインターを置いて、 **編集** アイコン ![編集アイコン](assets/edit-icon.png) をクリックして編集します。

      または

      表示する権限を持つ既存のフィルターの上にマウスポインターを置いて、 **詳細** メニュー ![その他のメニュー](assets/more-icon-spectrum.png)をクリックし、 **複製** 既存のフィルターをコピーし、コピーを編集します。
...
1. モディファイヤのドロップダウンメニューをクリックして、モディファイヤを選択します。 デフォルトの修飾子は「次と等しい」です。
   ...
   >[!TIP]
   >
   >フィルターを作成すると、結果がリストにすぐに表示されます。 フィルターパネルがリストの上にある場合は、フィルターパネルを閉じて、表示を確認できます。 入力した情報は、パネルを再度開いたときにベータビルダーに残ります。
...
1. （オプション）「 **フィルターグループを追加** 別のフィルター条件のセットを追加する場合。 セット間のデフォルトの演算子は AND です。 演算子をクリックして OR に変更します。
   ...
   >[!INFO]
   >
   >名前に「marketing」を含むプロジェクトをフィルタリングし、名前に「未完了」、「保留中」が含まれない場合、次の複数のフィルターグループを使用できます。
...
1. （オプション）「 **テキストモード** をクリックして、テキストモードを使用したフィルターの作成を続行します。
   ...
   >[!TIP]
   >
   >ベータビルダーインターフェイスを使用し、テキストモードでのみサポートされるフィルターを変更する必要がある場合にのみ、可能な限り多くのフィルターを作成することをお勧めします。
...
1. クリック **テキストモードを終了** ベータビルダーインターフェイスに戻る

   >[!WARNING]
   >
   >一部のテキストモード文は、ベータビルダーまたは標準インターフェイスではサポートされていません。 これらのタイプのステートメントを作成したときにテキストモードを終了すると、警告メッセージが表示される場合があります。
