...
この記事では、リストとレポートの標準ビューを作成および編集する方法について説明します。
...
ビューを作成またはカスタマイズするプロセスは、標準ビューとボードビューのどちらを作成またはカスタマイズするかによって異なります。
...
1. 「**列のプレビュー**」セクションで、以下のいずれかを行います。
   ...
   * **列設定**&#x200B;領域で、**この列を**&#x200B;で要約をクリックし、列にデータを表示する方法を選択します。 このオプションは、次の列タイプで使用できます。
     <table style="table-layout:auto"> 
         <col> 
         <col> 
         <tbody> 
         ...
         <tr>
           ...
           <td><ul><li>カウント</li></ul>
           <p>注意：Workfrontでは、値は常にtrue/falseであるため、通常、ブール値フィールドをカウントで集計することはお勧めしません。</p></td> 
          </tr> 
         </tbody> 
        </table>
     ...
     >[!NOTE]
     >
     >次の例外は、グループ化で次のフィールドの値を集計する際の親オブジェクト（例えば、親タスク）に適用されます。
     >   
     > * 実時間以外のすべての数値フィールドと通貨フィールド（例：計画/実績労力コスト、計画/実績費用コスト、計画/実績費用、予定時間）は、子タスクとスタンドアロンタスクのみの値を集計します。 親タスクまたは親の親の値を要約しません。
     > * 実際の時間数は、メインの親タスクとスタンドアロンタスクの値を要約します。親タスクの親タスクや子タスクの数値を要約しません。
     > * 数値および通貨値のカスタムデータフィールドには、親、子、親の親、スタンドアロンタスクなど、すべてのタスクが要約されます。
     >
        ...
        <table style="table-layout:auto"> 
         ...
         <tbody> 
          <tr> 
           ...
           <td><p>列のカスタムラベルを指定します。 このラベルは、デフォルトのラベルに取って代わります。 互換性の問題を回避するために、UTF-8文字のみを使用することをお勧めします。</p></td> 
          </tr> 
          <tr> 
          ...
          <tr> 
           ...
           <td><p><strong>+この列のルールを追加</strong>をクリックして、列のルールを定義します。 ルールを追加した後、そのルールに一致するフィールドの表示方法について、フィールドとテキストのスタイルを定義できます。 ルールの定義が完了したら、「<strong>ルールを追加</strong>」をクリックします。</p></td> 
          </tr> 
         </tbody> 
        </table>
...
ボードビューは、プロジェクト上のタスクとイシューのリストに対してのみ使用できます。

事前に設定されていますが、特定の設定を変更することもできます。

アジャイルビューまたはボードビューについて詳しくは、[ ボードビューでのアジャイルプロジェクトの管理](../../../manage-work/projects/manage-projects/manage-projects-in-agile-view.md)を参照してください。
