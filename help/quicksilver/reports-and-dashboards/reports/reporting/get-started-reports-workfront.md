...
* 独自のレポートを作成するには、標準ライセンスまたはプランライセンスが必要です。
...
>例えば、カリフォルニア州のユーザーが 2 月 12 日の午後 9:30 （PST）にタスクを完了したとします。 このタスクの完了を含むレポートをニューヨークでユーザーが表示すると、2 月 13 日の午前 12 時（EST）に完了したため、「実際の完了日」が「詳細」タブとチャート詳細の両方に 2 月 13:30 として表示されます。 ただし、グラフでは、グラフ要素を展開するまで、2月12日のグループ化に含まれます。
