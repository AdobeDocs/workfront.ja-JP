...
1. 作成するグラフの種類を選択してください：
   * [棒グラフ、列グラフ、折れ線グラフ](#bar-column-or-line-chart)
   * [円グラフ](#pie-chart)
...
>[!NOTE]
>
>選択したフィールドタイプによっては、追加のフィールドが存在する場合があります。 以下に概説するオプションは、すべてのフィールドタイプの標準です。


1. 左側のパネルで「**グラフを作成**![&#x200B; グラフを作成アイコン &#x200B;](assets/build-chart-icon.png) アイコンをクリックします。

1. 「**グラフのタイプ**」ドロップダウンで、「**棒グラフ**」、「**列**」または「**折れ線グラフ**」を選択します。
1. 2 番目のドロップダウンメニューで、バー、列または行の種類を選択します。
   * **シンプル**
   * **マルチシリーズ**
   * **積み重ね**

1. **下（X）軸** セクションで **フィールドの更新** を選択し、グラフに要約されるデータを含むフィールドを見つけて選択します。
1. **集計タイプ** ドロップダウンで、データをロールアップしてグラフ出力を生成する方法を選択します。
1. <span class="preview"> （オプション）指定されたスペースに軸ラベルを追加します。</span>
1. <span class="preview"> （任意） **軸を非表示** をオンに切り替えます。</span>
1. <span class="preview"> （オプション） **参照線の値** を入力して、グラフのターゲットまたはしきい値を設定します。</span>
1. ドロップダウンメニューから **線種** を選択します。
...
1. **グラフのタイプ** ドロップダウンで、「**棒グラフ**」を選択します。
1. 「**指標**」セクションで「**フィールドを更新**」を選択し、グラフに要約されるデータを含んだフィールドを見つけて選択します。
1. **集計タイプ** ドロップダウンで、データをロールアップしてグラフ出力を生成する方法を選択します。
1. 「**セグメント**」セクションで「**更新フィールド**」を選択し、円グラフに表示するセグメントを含むフィールドを見つけて選択します。
1. <span class="preview"> （任意）「**円グラフ**」セクションで、「**セグメントラベルを表示**」をオンにしてセグメントラベルを表示します。</span>
1. <span class="preview"> （任意） **合計を表示** を切り替えて、グラフの中央に合計を表示します。 有効にすると、中央のラベルを表示して値の形式を選択する追加のオプションが表示されます。</span>

>[!NOTE]
>
>集計タイプは次のように表示されます。
>
>* カウント集計タイプ：表示される中央の値は、グラフのすべてのセグメントのカウントです。
>* Sum 集計タイプ：表示される中央値は、数値または通貨値の集計合計です。
>* 平均、最大、最小の集計タイプ：中央の値は、それに応じて平均、最大、最小の値を表示します。

1. <span class="preview"> （オプション）「凡例」セクションで、「凡例を表示 **オンに切り替えて、グラフの凡例を表示** します。</span>

1. <span class="preview"> （オプション）ドロップダウンメニューから **凡例の位置** を選択します。</span>

## 追加のグラフレポート設定の指定

### フィルター

**フィルター** セクションを設定するには、次の手順に従います。
...
次の手順に従って、「ドリルダウン・グループ設定 **セクションを構成し** す。
