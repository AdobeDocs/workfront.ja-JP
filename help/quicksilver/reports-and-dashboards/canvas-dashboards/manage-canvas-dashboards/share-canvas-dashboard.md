...
* ダッシュボードのレポートが「ユーザーとして実行」オプションで設定されている場合、すべてのビューアには、選択したユーザーの権限に基づくデータが表示されます。この権限は、自分のものではなく、選択したユーザーの権限に基づきます。 そのため、通常はアクセスできないデータを閲覧する可能性があります。 ダッシュボード上の一部のレポートで「ユーザーとして実行」を使用し、その他のレポートを使用しない場合、ビューアはレポート間で一貫性のないデータを表示する可能性があります。
