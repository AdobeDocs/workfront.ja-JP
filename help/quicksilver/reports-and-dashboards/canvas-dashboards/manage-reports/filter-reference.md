...
title: Canvas ダッシュボードのレポートフィルター参照
description: カンバスダッシュボードでレポートをフィルタリングする際に使用できるフィールド、演算子、ワイルドカードおよび特殊ルールに関するリファレンス。
author: Courtney
...
<table>
    ...
    <tr>
       ...
       <tr>
        ...
        <td>「<b>相対日付を設定</b>」オプションを切り替えることで、相対日付のワイルドカードを適用して、レポートをより動的にし、一般的な日付期間に基づいて自己調整することができます。 
         <ul><li>$$TODAY</li>
         <li>$$NOW</li>
         </ul>
        </td>
...
日付ベースのワイルドカードオプションは、任意の日付フィルター属性と組み合わせて使用できます。 日付ベースのワイルドカードをレポートに追加する方法については、[日付ベースのワイルドカードを使用してレポートを一般化する](/help/quicksilver/reports-and-dashboards/reports/reporting-elements/use-date-based-wildcards-generalize-reports.md)を参照してください。
...
例えば、ワイルドカード `$$TODAYb+2w` は、「今週の初めから 2 週間」を意味します。 ワイルドカード `$$NOW+2h`は「今後2時間」を表します。
