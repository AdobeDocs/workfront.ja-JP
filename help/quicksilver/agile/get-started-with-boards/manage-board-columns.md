...
title: 掲示板の列の管理
description: 新しいボードには、デフォルトで 3 つの列が含まれています。 列の追加、列の順序の変更、列の名前の変更、不要な列の削除を行うことができます。 列ポリシーを定義することもできます。
author: Courtney
feature: Agile
...
列ポリシーには、フィールド値の自動更新や、進行中の作業の制限の設定が含まれます。

ステータスを更新するためのポリシーは、カードと列の両方で自動的に機能します。

* カードがポリシーを持つ列に移動されると、カードステータスはポリシーで定義されたステータスに更新されます。 これは、アドホックカードとコネクテッドカードの両方に適用されます。
* アドホックまたは接続されたカードのステータスが、ポリシーの列のステータスに一致するようにカード上で更新されたり、Workfrontの別の場所で接続されたカードのステータスが更新されたりすると、カードはその列に自動的に移動されます。 また、カードのカスタムステータスが、列に割り当てられたシステムステータスと一致した場合、カードはその列に移動されます。

カードのステータスが既存の列ポリシーで設定されているステータスと一致しない場合、カードは配置された列に残ります。
...
1. （オプション）カードのステータスの値を設定します。
   ...
   1. カードがこの列に移動されたときにカードに適用するステータスを選択します。
      ...
      接続されたカードのステータス変換オプションも表示されます。 （ステータスの翻訳は、アドホックカードには適用されません）。 これらのオプションは、接続されたカードがこの列に移動されたときに[!DNL Workfront]のタスクまたは問題に適用されるカスタムステータスを決定します。
