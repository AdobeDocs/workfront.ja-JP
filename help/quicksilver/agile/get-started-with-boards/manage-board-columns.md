...
1. （オプション）カードのステータスの値を設定します。
   ...
   1. 初期の機能オプトインを使用している場合：を選択します。 [!UICONTROL **カスタム**] タスクと問題の両方に対してカードに適用するステータス。

      カードがこの列に移動されたとき、 [!DNL Workfront] 最初に、カスタムステータスの適用を試みます（「解決済み」など）。 そのカードでカスタムステータスが使用できない場合、Workfrontは代わりにシステムステータスを適用します（例：クローズ）。 システムステータスは、上記の手順 b で選択したステータスです。
