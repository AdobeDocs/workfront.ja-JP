...
description: Adobe Workfront Fusion シナリオでは、 [!DNL NetSuite]を使用して、複数のサードパーティのアプリケーションやサービスに接続できます。
author: Becky
...
内 [!DNL Adobe Workfront Fusion] シナリオを使用して、 [!DNL NetSuite]を使用して、複数のサードパーティのアプリケーションやサービスに接続できます。
...
使用する [!DNL NetSuite] モジュールの場合、 [!DNL NetSuite] アカウント



## [!DNL NetSuite] モジュールとそのフィールド

設定時に [!DNL NetSuite] モジュール [!DNL Workfront Fusion] 以下のフィールドが表示されます。 これらに加えて、 [!DNL NetSuite] アプリまたはサービスのアクセスレベルなどの要因に応じて、フィールドが表示される場合があります。 モジュール内の太字のタイトルは、必須フィールドを示します。
...
このアクションモジュールを使用すると、 [!DNL NetSuite] API これにより、他のユーザーが達成できないデータフローの自動化を作成できます [!DNL NetSuite] モジュール。
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>接続方法 [!DNL NetSuite] Workfront Fusion へのアカウントについては、<a href="#create-a-connection-to-netsuite-in-a-workfront-fusion-netsuite-module" class="MCXref xref">への接続の作成 [!DNL NetSuite] (Workfront Fusion NetSuite モジュール )</a> 」を参照してください。</p> </td> 
  </tr> 
  <tr> 
