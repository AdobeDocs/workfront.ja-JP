...
* システム管理者権限を持つ、**[!DNL Anaplan Integration]** という名前の [!DNL Workfront] ユーザープロファイル。
...
* **[!UICONTROL Adobe Workfront の Campaigns.Update キャンペーン]**&#x200B;という名前の [!DNL Anaplan] ビュー。
  ...
  [!UICONTROL [!DNL Workfront] プロジェクト GUID] を持つ項目と、予算配分を Workfront に送信する必要があることを示すインジケーターを表示するには、ビューをフィルタリングする必要があります。

これらのアクションの手順については、[!DNL Anaplan] のドキュメントを参照してください。
...
1. **[!UICONTROL CSV を JSON オブジェクトに変換]**&#x200B;モジュールで、CSV 列を使用可能な JSON オブジェクトにマッピングする新しいデータ構造を追加します。
