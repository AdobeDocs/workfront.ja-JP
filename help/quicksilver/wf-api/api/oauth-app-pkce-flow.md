...
* `client_id`は、アプリケーションの設定時にで作成したOAuth2 アプリケーションのクライアント IDと一致します。

  手順については、Workfront 統合用の OAuth2 アプリケーションの作成にある、PKCE を使用した OAuth2 単一ページ web アプリケーションの作成を参照してください。

* アプリケーションで認証コード付与タイプが使用されているため、`response_type`は`code`です。
* `redirect_uri`は、ユーザーエージェントが`code`と共に送信されるコールバックの場所です。 これは、OAuth2 アプリケーションの作成時に指定したリダイレクト URL の 1 つと一致する必要があります。
* `code_challenge_method`は、チャレンジを生成するために使用されるハッシュ方式です。PKCEを使用するWorkfront Oauth2 アプリケーションでは、常に`S256`です。
* `code_challenge`は、PKCEに使用されるコード チャレンジです。
