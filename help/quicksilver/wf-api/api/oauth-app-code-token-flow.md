...
* `client_id`：これは、WorkfrontでOAuth2 アプリを作成したときに生成されたクライアント IDです。

* `redirect_uri`：このURLは、OAuth2 アプリの作成時にWorkfrontで入力したリダイレクト URLと同じである必要があります。 ユーザーは、アカウントでアプリを認証した後、このページに誘導されます。
