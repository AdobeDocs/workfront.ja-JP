...
* フィルターの構文が正しくないか、ペイロードの&#x200B;**newState**&#x200B;に含まれるデータと一致しない場合、検証メッセージは返されず、エラーが発生したことを示します
...
* 最大10個のフィルターグループ（各5個のフィルター）がサポートされていますが、複雑なフィルターロジックを持つ多数のアクティブなサブスクリプションでは、イベント評価時に遅延が発生する可能性があります。

これらの制限を超えている場合は、ロジックを簡素化するか、サブスクリプションを複数の小さなサブスクリプションに分割することを検討してください。
