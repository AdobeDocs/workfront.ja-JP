...
<pre>/attask/api/v15.0/work/myWork</pre>名前付きクエリは、フィールドパラメーターに対する追加のフィールド取得のリクエストをサポートしています。名前付きクエリの中には、追加のフィルターを受け入れるものもあります。オブジェクトで使用可能な名前付きクエリのリストについては、のオブジェクトの「アクション」タブを参照してください  [API エクスプローラー ] （https://developer.adobe.com/workfront/api-explorer/）。
