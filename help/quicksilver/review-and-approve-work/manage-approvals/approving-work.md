...
1. 右側のパネルで承認を決定する際に、使用可能なオプションの 1 つをクリックします。 承認する項目のタイプに応じて、ページの右上隅に次のオプションが表示されます。
   ...
   * **アクセス：** で付与するアクセスレベルを選択 **アクセスを変更** ドロップダウンメニューで、 **アクセスを許可**. または、 **無視**.
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td>承認プロセスのステージ。 <br>承認待ちの現在のステージは、「保留中」と表示されます。 既に承認済みのステージは、「承認済み」と表示されます。まだ承認されていないステージは、「未開始」と表示されます。</td> 
     </tr> 
    </tbody> 
   </table>
...
通知の設定に応じて、他のユーザーが承認を決定する必要があるドキュメントについて通知する電子メールを受け取る場合があります。 次の情報を含む E メールを受信した場合： **承認を決定** 」ボタンをクリックすると、承認プロセスを電子メールから直接開始できます。

1. 電子メールで、 **承認を決定** をクリックして、配達確認のドキュメントの詳細ページを開きます。
1. ドキュメントを確認するには、次のいずれかの操作を行います。

   * ドキュメントのメタデータを表示します。
   * マークアップとコメントを含むドキュメントのレビュー用に配達確認が作成されている場合は、 **配達確認を開く** ![](assets/open-proof-icon-qs.png) 右上隅近くにある「 」で、配達確認を確認します。
