...
title: 送信済みのプルーフ承認を表示する
description: Adobe Workfront 環境が Workfront Proof Premium アカウントと統合されている場合にのみ、プルーフの承認がホームエリアに表示されます。ここで説明したようにプルーフを使用できない場合は、Workfront 管理者にお問い合わせください。
author: Courtney
...
1. 作業リストで&#x200B;**プルーフ**&#x200B;の承認をクリックします。

   これにより、作業リストの右側にプルーフの承認が開きます。

   >[!NOTE]
   >
   >ゲストプルーフユーザーに承認を送信する場合、その承認はホームには表示されません。

   ![](assets/proof-approval-pending-home-nwe-350x230.png)

1. （オプション）右側のパネルの右上隅にある「**プルーフに移動**」をクリックして、プルーフビューアーを起動します。
