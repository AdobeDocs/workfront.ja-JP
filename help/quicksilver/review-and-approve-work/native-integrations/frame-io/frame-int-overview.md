...
Workfrontにアップロードされたドキュメントは、Adobe エンタープライズストレージに保存され、Workfrontと Frame.io の両方でアクセスできます。 Workfrontでドキュメントをタスクまたは問題にアップロードすると、タスクまたは問題の権限を継承するシステム生成フォルダーがAdobe エンタープライズストレージに作成されます。 そのタスクまたは問題にアップロードされたすべてのドキュメントは、そのフォルダーに保存され、そのフォルダーから権限を継承します。 Workfrontのドキュメントについて詳しくは、[&#x200B; 新しいドキュメントエリアの概要 &#x200B;](/help/quicksilver/documents/managing-documents/documents-area.md) および [Adobe エンタープライズストレージモデルのオブジェクト権限とアクセスレベルの概要 &#x200B;](/help/quicksilver/review-and-approve-work/esm-access-permissions.md) を参照してください。
