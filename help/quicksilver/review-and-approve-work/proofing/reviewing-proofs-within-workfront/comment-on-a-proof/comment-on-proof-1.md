...
1. （条件付き）ビデオプルーフの場合は、ビデオプルーフを再生し、再生ヘッドを使用してビデオ内のコメントを追加したい場所に合わせて「**コメントを追加**」をクリックします

   >[!TIP]
   >
   >1. 使いやすくするには、ビデオを一時停止し、タイムライン内のコメントを追加したい場所をクリックします。
...
1. 静的プルーフでは、同じコメントを複数ページの複数のエリアに対してマークできます。例えば、複数のページで同じコメントを作成する場合は、1 つのページの 1 つのエリアを選択した後で、同じコメントを作成したい次のページに移動して、そこで別の領域を選択します。(これは標準の注釈マークアップツールにのみ適用され、テキストマークアップツールには適用されません。)
...
1. （オプション）コメントにファイルを添付するには、次のいずれかの操作を行います。
   ...
   * コメントの右下隅にあるペーパークリップアイコンをクリックし、アップロードするファイルを参照して選択します。
