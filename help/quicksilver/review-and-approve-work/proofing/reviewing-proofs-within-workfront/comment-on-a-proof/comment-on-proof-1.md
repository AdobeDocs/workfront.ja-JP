...
プルーフのコメントにユーザーをタグ付けする場合、タグ付けできるユーザーは、個々のユーザー権限や組織内のメンバーシップなど、様々な要因によって異なる場合があります。

* プルーフの作成者または所有者である場合、または特定の権限を有効にしている場合は、プルーフワークフロー以外のユーザーにタグ付けして、プルーフを共有できます。
* 外部ユーザーとしてプルーフに追加され、別のプルーフアカウントを持つ別の環境のメンバーである場合は、元の環境からそれらのユーザーのみをタグ付けできます。<!--For more information, see [Proofing collaboration limitations with people outside of your organization](../../../../review-and-approve-work/proofing/tips-tricks-and-troubleshooting/collaboration-with-members-outside-of-your-organization.md)-->
...
1. 必要なプルーフにカーソルを合わせて、**プルーフを開く**&#x200B;をクリックします。

1. プルーフビューアの上部で、「**コメントを追加**」をクリックします。
1. （条件付き）これがビデオプルーフの場合は、ビデオプルーフを再生し、再生ヘッドを使用してコメントを追加するビデオ内のポイントで&#x200B;**コメントを追加**&#x200B;をクリックします。

   >[!TIP]
   >
   >1. 使いやすくするために、ビデオを一時停止し、コメントを追加するタイムライン内をクリックします。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td> <p>シェイプとして開いたままにしたり、閉じたりできる一連の線分セグメントを接続して描画します。 追加した任意の点は移動または削除できます。 </p> <p>このツールは、技術的および建築的なイメージを操作する場合に役立ちます。</p> </td> 
     </tr> 
     <tr> 
      <td role="rowheader"><strong>マークアップの色、太さまたは不透明度を変更</strong> </td> 
      <td> <img src="assets/change-color.png"> </td> 
      <td>マークアップツールの色、太さ、不透明度を変更します。</td> 
     </tr> 
     <tr> 
...
1. （オプションおよび条件付き）静的プルーフでは、1つのコメントに対して複数のページの複数の領域をマークできます。 例えば、複数のページで同じコメントを作成する場合は、1つのページの領域を選択し、同じコメントを作成する次のページに移動して、その領域を選択します。

   >[!NOTE]
   >
   >これは、テキストマークアップツールではなく、通常の注釈マークアップツールにのみ適用されます。
...
1. （オプション）コメントにファイルを添付するには、次のいずれかの操作を行います。
   ...
   * コメントの右上隅にあるペーパークリップアイコンをクリックし、アップロードするファイルを見つけて選択します。
