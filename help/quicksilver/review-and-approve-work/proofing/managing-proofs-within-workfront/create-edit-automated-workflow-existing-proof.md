...
## 自動化ワークフローテンプレートの使用について {#about-using-automated-workflow-templates}

自動ワークフローテンプレートを使用する場合は、次の点を考慮してください。

1. 自動化ワークフローテンプレートの設定により、プルーフの自動化ワークフローで何ができるかが決まります。 例えば、テンプレートで「ステージの追加」ボタンが無効になっている場合、プルーフの自動ワークフロー設定を操作するときにこのボタンは表示されません。
