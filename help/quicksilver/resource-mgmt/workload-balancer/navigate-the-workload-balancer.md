...
ワークロードバランサーの場所の詳細については、 [ワークロード・バランサの検索](https://experienceleague.adobe.com/docs/workfront/using/manage-resources/the-workload-balancer/locate-workload-balancer.html?lang=en).
...
* ワークロード・バランサは、割り当てに応じて、2 つの異なる領域に作業項目を表示します。 作業項目とユーザーは、次の領域に表示されます。
   ...
   * **未割り当ての作業**:割り当てがないアイテム、またはジョブロールまたはチームにのみ割り当てられるアイテム。
   * **割り当てられた作業**:少なくとも 1 人のユーザーに割り当てられている項目。 割り当てられた項目は、割り当てられたユーザーの名前の下に表示されます。

   >[!NOTE]
   >
   >* 主担当者であるユーザーにも割り当てられているジョブロールまたはチームに割り当てられた項目は、「未割り当て作業」領域に表示されません。
   >
   >* 「未割り当て作業」領域に、ジョブロールが項目の主な担当者として選択されているユーザーおよびジョブロールに割り当てられた項目が表示されます。
   >* 複数のユーザーに割り当てられた項目は、「割り当てられた作業」領域のすべてのユーザー名の下に表示されます。
...
* [サマリーを使用してワークロードバランサーの作業項目を更新](../workload-balancer/update-items-in-summary-panel-in-workload-balancer.md)
...
1. クリック **ワークロードバランサー** をクリックします。
   ...
   * この **未割り当ての作業** 領域には、次の作業項目が表示されます。
...
「未割り当て作業」領域には、デフォルトで作業項目は表示されません。 フィルターを使用して、この領域の関連情報を表示することをお勧めします。
   ...
   * この **割り当てられた作業** 領域には、次の作業項目が表示されます。
...
1. 次の手順に従って、ワークロード・バランサをナビゲートします。
   ...
   * [グラフでの配分の表示](#view-allocations-in-a-chart)
...
1. 次をクリック： **前後に** アイコン ![](assets/back-and-forward-icons.png) 左上隅でタイムラインに移動し、 **今日** 現在の週に戻る
...
1. ツールバーで次のいずれかのオプションをクリックすると、異なる時間枠で情報が表示されます。
   * **日**:デフォルトでは、今日の日付から始まる 4 週間の情報を日別に表示します。
   * **週**:4 週間の情報を週別に表示します。
   * **月**:3 か月間の情報を月別に表示します。
...
1. 次をクリック： **右向き矢印** 「未割り当て」領域または「割り当て済み」領域の左側にあるので、プロジェクト名（「未割り当て」領域内）とユーザー名（「割り当て済み」領域内）の下にあるすべてのアイテムを展開できます。
...
1. 次をクリック： **設定** アイコン ![](assets/settings-gear-icon.png).
   ...
   * **プロジェクト別にグループ化**:このオプションを選択すると、[ 未割り当て ] および [ 割り当て ] 作業領域の項目がプロジェクトごとにグループ化されます。 これはデフォルトで選択されています。

   * **問題からの時間を含む**:このオプションを選択すると、ユーザーに割り当てられたタスクが「割り当てられた作業」領域のユーザー名の下に表示され、ユーザーに割り当てられていないタスクが「未割り当ての作業」領域に表示されます。 問題からの計画時間は、プロジェクトの計画時間に対して、また、「割り当てられた作業」領域のユーザーに対してカウントされます。
   * **推定日を表示**:このオプションを選択すると、予定タイムラインに加えて、作業項目の推定タイムラインが表示されます。 次の点に注意してください。
   ...
   * **残り時間を表示**:このオプションを有効にすると、Workfrontには、ユーザーのスケジュールに基づいて、ユーザーが作業できる日次時間と、ユーザーの「割り当てられた作業時間」領域に割り当てられた時間との差が表示されます。 これはデフォルトで無効になっており、割り当てられた時間はデフォルトで表示されます。
   * 内 **カラーテーマを選択** [ ] セクションで、プロジェクトとタスクバーに使用する色を選択します。
      ...
      次の中から選択します。
      * **デフォルト**:すべてのプロジェクトのバーと作業項目が青で表示されます。
      * **プロジェクト**:各プロジェクトに関連付けられたバーとそのタスクは、プロジェクトの名前に応じて変わります。 プロジェクトに属するすべてのタスクは、プロジェクトの色と一致するバーで表示されます。 プロジェクトバーは、タスクと区別するために明るい色で表示されます。 割り当てを表示しないことを選択した場合は、プロジェクトバーにプロジェクトアイコンも表示されます。
      * **プロジェクトステータス**:各プロジェクトとその作業項目に関連付けられたバーは、プロジェクトのステータスの色に変わります。
   ...
   * 内 **でのユーザー割り当ての表示** 「 」セクションで、次のいずれかを選択します。
      * **時間**:割り当てられた時間を時間単位で表示します。 これがデフォルトです。
      * **割合**:割り当てられた時間を合計使用可能時間に対する割合で表示します
   * 内 **並べ替え環境設定** 「 」セクションで、ワークロード・バランサでアイテムを並べ替える方法を選択します。 次のオプションから選択します。
      * **ユーザーの役割でのプライマリの並べ替え**:「割り当てられた作業」領域では、ユーザーのプライマリロールのアルファベット順に表示されます。
      * **ユーザーをアルファベット順に並べ替え**:「割り当てられた作業」領域では、ユーザーの名がアルファベット順に表示されます。
      * **プロジェクトの並べ替え基準**:「未割り当て」または「割り当て済みの作業」領域のそのフィールドでプロジェクトをアルファベット順に並べ替えるには、ドロップダウンメニューからプロジェクトフィールドを選択します。
...
1. 次をクリック： **その他のメニュー** アイコン ![](assets/more-icon.png) タスク名またはイシュー名の右にある下のオプションの 1 つをクリックします。
   ...
   * **割り当てを編集**」をクリックし、そのユーザーの日次または週次の配分を編集します。 ユーザー割り当ての管理の詳細は、 [ワークロードバランサーでのユーザー割り当ての管理](../workload-balancer/manage-user-allocations-workload-balancer.md).

   * **概要を開く**. 右側に「概要」パネルが開き、「割り当て」フィールドをクリックして、 **担当者、役割、またはチームの検索** 項目を割り当てるフィールド。 詳しくは、 [タスクと問題に関する詳細情報を表示](#display-more-information-about-tasks-and-issues) 」を参照してください。
...
1. （オプション）作業項目のバー内でユーザーの日次または週次の配分をダブルクリックして、割り当てられた時間数を編集し、 **保存** アイコン ![](assets/save-allocations-wb.png) 割り当てまたは **キャンセル** アイコン  ![](assets/cancel-allocations-wb.png) 調整した配分を削除します。
   ...
   >[!TIP]
   >
   >保存アイコンとキャンセルアイコンは、タスクの最後またはイシューのタイムラインバーに表示されます。
   >
...
1. クリック **一括割り当て** ：作業項目を一括で割り当てます。
   ...
   詳しくは、 [ワークロードバランサーを使用して作業を一括で割り当て](../workload-balancer/assign-work-in-workload-balancer-in-bulk.md).
...
1. 次をクリック： **グラフアイコン** ![](assets/user-allocation-chart-icon.png) をクリックして、ユーザーの配分をグラフ形式で表示します。
   ...
   ブロックのサイズは、配分の量を示します。ボックスが大きいほど、その日または週の作業項目に割り当てられる時間が長くなります。
...
1. [ 概要 ] パネルに詳細情報を表示するには、次のいずれかの操作を行います。
   ...
   * 次をクリック： **詳細** タスクまたはイシューの右側にあるメニューから、 **概要を開く**.

   ワークロード・バランサのサマリーでタスク情報を更新する方法の詳細は、 [サマリーを使用してワークロードバランサーの作業項目を更新](../workload-balancer/update-items-in-summary-panel-in-workload-balancer.md).
...
1. 次をクリック： **全画面表示を終了** アイコン ![](assets/exit-full-screen.png) をクリックしてデフォルトの画面に戻り、ブラウザ・タブ内にワークロード・バランサを表示します。


## チームのワークロードバランサーのナビゲート

チームのワークロードバランサーのナビゲーションは、複数のプロジェクトのワークロードバランサーのナビゲート方法と似ています。 詳しくは、 [複数のプロジェクトのワークロード・バランサのナビゲート](#navigate-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 」を参照してください。

1. 次をクリック： **メインメニュー** アイコン ![](assets/main-menu-icon.png)を選択し、「 **チーム**.
...
1. クリック **ワークロードバランサー** をクリックします。 チームのワークロードバランサーは、デフォルトで表示される必要があります。
   ...
   * 内 **未割り当ての作業** 領域：チームまたはチームとジョブの役割に割り当てられ、ユーザーに割り当てられていない作業項目です。
   * 内 **割り当てられた作業** 領域：ユーザーに割り当てられた作業項目は、ユーザー名の下に表示されます。
...
1. クリック **ワークロードバランサー** をクリックします。 クリックする必要がある場合があります **さらに表示**&#x200B;を、 **ワークロードバランサー**.

   ![](assets/nwe-balancer-project-350x152.png)

   プロジェクトのワークロード・バランサには、デフォルトで次の情報が表示されます。

   * 内 **未割り当ての作業** 領域：ロールまたはチームに割り当てられ、ユーザーに割り当てられていない、プロジェクトの作業項目。
   * 内 **割り当てられた作業** 領域：少なくとも 1 人のユーザーに割り当てられた、プロジェクト上の作業項目。
...
1. （オプション） **ロール割り当てを表示** アイコン ![](assets/show-role-allocation-icon.png).
...
1. プロジェクトのワークロードバランサーのナビゲーションを続行します。詳しくは、 [複数のプロジェクトのワークロード・バランサのナビゲート](#navigate-the-workload-balancer-for-multiple-projects-in-the-resourcing-area) 」の節を参照してください。
