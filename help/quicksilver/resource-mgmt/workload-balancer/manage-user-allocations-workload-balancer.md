...
* 期間の日数が増加する作業項目の期間を増やすと、調整された割り当て時間は、調整された日数に対して同じままになります。 割り当てられた時間が 0 の作業項目に日数が追加されます。
...
* すべてのタスクとすべてのプロジェクトに対して 1 日または 1 週間に割り当てられた時間は、[ 割り当てを表示する ] 設定を手動で有効にした場合にのみ表示されます。 ワークロードバランサーでの設定の有効化の詳細は、以下を参照してください。 [ワークロード・バランサのナビゲート](../../resource-mgmt/workload-balancer/navigate-the-workload-balancer.md).
