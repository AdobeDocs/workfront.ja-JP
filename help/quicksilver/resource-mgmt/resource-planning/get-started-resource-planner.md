...
プロジェクト別またはロール別に表示する場合、リソース・プランナの正しい機能には、次のタイプの前提条件が必要です。
...
Workfrontでのユーザーおよび役割の時間と工数の計算方法について詳しくは、この記事を参照してください。 [リソース・プランナのユーザーとロールに対する時間と工数の計算の概要](../../resource-mgmt/resource-planning/calculate-hours-fte-for-users-roles-resource-planner.md).
...
時間単価のユーザーとの関連付けの詳細については、「 [ユーザーのプロファイルの編集](../../administration-and-setup/add-users/create-and-manage-users/edit-a-users-profile.md).
