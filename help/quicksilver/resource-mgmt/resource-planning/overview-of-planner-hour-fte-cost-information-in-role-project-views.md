...
title: リソースプランナーのプロジェクトビューおよびロールビューでの時間、工数、コスト情報の概要
...
* 後で非アクティブ化したユーザーに対して、以前に予算計上された時間は表示されません。
  ...
  プロジェクトの予算計上労力コストには、リソースプランナーで非アクティブ化されたユーザーの予算計上時間が含まれることに注意してください。

  例：役割が2人のユーザーに割り当てられ、予算計上時間が追加され（ユーザーあたり20時間、合計40時間）、その役割の合計が手動で設定された場合、リソースプランナーでユーザーの1人を非アクティブ化すると、その時間は計算で考慮されなくなります（合計を20時間に下げます）。 ただし、プロジェクトの予算編成では、手動で設定した役割の合計が正しく保持されるため、非アクティブ化されたユーザーの時間は計算に含まれます（残りは40時間）。

### VAR （Variance）列 {#the-var-variance-column}
...
>[!NOTE]
>
>「NET Hours」、「FTE」または「Cost」が赤で表示されている場合、予算計上済み時間または予定時間または作業に関連するコストをカバーするのに十分な利用可能な時間または予算がありません。 この場合、リソースは割り当て超過になります。
