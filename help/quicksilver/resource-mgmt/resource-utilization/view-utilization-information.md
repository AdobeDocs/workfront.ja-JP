...
Workfront の他のエリアとは異なり、予定収益がキャップを超える場合、キャップ額を超える金額は固定収益と見なされます。予定収益は、固定収益をタスクの予定時間数で均等に割り、キャップ額と役割またはユーザーの時間収益に加算して計算されます。<br></p> <p>次のケースが存在します。</p>
<ul>
<li> <p><strong>役割（毎時）（キャップ付き）：</strong>タスクは役割（毎時）と同様に時間単位で請求されますが、指定できる最大キャップ額があります。Workfront では次の数式が使用されます。</p> <p><code>役割実収益（時間単位）（キャップあり）= [SUM（全タスクおよびイシューの役割からの実際の時間数） * 役割請求レート] + タスクのキャップ額 + SUM（タスクのキャップ額を超えた額 / タスクの実際の時間数）</code></p> </li>
