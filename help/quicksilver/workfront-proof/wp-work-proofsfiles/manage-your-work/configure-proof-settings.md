...
* **[!UICONTROL 購読者の役割]** – これは、自分自身をプルーフに登録するすべてのレビュー担当者に割り当てられるデフォルトのプルーフの役割です。
* **[!UICONTROL 購読者のメールアラート設定]** - プルーフを購読するすべてのレビュー担当者に割り当てられるデフォルトのメールアラートです。
* **[!UICONTROL 次の操作が必要です：メールリンクを使用したプルーフへのアクセス]** – 購読者がプルーフにアクセスするには、メール内のリンクをクリックする必要があります\
