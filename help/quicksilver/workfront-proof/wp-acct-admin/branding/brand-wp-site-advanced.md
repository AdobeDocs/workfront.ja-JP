...
[!DNL Workfront Proof] の以下のエリアでブランディングが可能です。
...
Web アプリケーションヘッダー（2）は 3 つのブランディングオプションから選択できます。

* ヘッダーのブランディング画像
* ヘッダー内のアカウント名
* ブランディングを無効にした場合、[!DNL Workfront Proof] ロゴは web アプリのヘッダーに残ります

ブランディング画像 - サイズが最大の 550 x 90 px に変更されます。JPG、GIF、PNG を使用でき、透明な背景がサポートされています。
...
カラー - ここでは、カラースキーム設定の他のすべてのブランディングフィールドと同様に、任意の 16 進数カラー値を入力するか、便利なカラーピッカー（テキストフィールドをクリックするとポップアップが開きます）を使用できます。ヘッダーのデフォルトの背景色は #232d2e です。

![Branding_-_color_picker.png](assets/branding---color-picker-350x131.png)

背景画像 - ブランディング画像と組み合わせることができます。

* JPG、GIF、または PNG を使用できます。透明なファイルの場合は、背景色が白で表示されます。
* ヘッダーの高さは 96 ピクセルであり、「[!UICONTROL リピートなし]」オプションを選択するとアップロード画像はサイズ変更されません。
* 背景画像は左上に配置されます。
...
メニューバー（5）とメニューフォント（6）のカラーを選択して、サイドバーをカスタマイズします

![Menu_bar.png](assets/menu-bar-350x490.png)

>[!NOTE]
>
>選択したメニューバーのカラーに定数の 16 進数値を追加すると、ポイント時のカラーが自動的に調整されます。

「新しいプルーフ」ボタンのカラーはカスタマイズできません。
...
>[!NOTE]
>
>フッターのテキストエディターでは HTML を編集できませんが、コピーしたデザイン（すべてのリンクと画像を含む）を貼り付けることができます。
...
>[!NOTE]
>
>お気に入りアイコンは、アカウントで作成されたすべてのプルーフの [!DNL Workfront Proof] ビューア画面のヘッダーにも表示されます。

**ヘルプリンク**

独自のヘルプリンクをカスタマイズして、ユーザーやレビュー担当者が独自のコンテンツを参照できるようにすることもできます。これを有効にするには、[!UICONTROL ヘルプリンク]オプション（13）を有効に設定し、次のフィールド（14）にリンクを追加します。最大 4 つのリンクを設定でき、以下で使用できます。

* [!UICONTROL ヘルプ]リンクの[!UICONTROL ヘッダー]メニュー内
  ![Help_links1.png](assets/help-links1-350x170.png)

* [!DNL Workfront Proof] [!UICONTROL ビューア]サイドバーの[!UICONTROL ヘルプ]パネル
  ![Help_links2.png](assets/help-links2-350x532.png)

エンタープライズプランのお客様には、追加料金で以下を完全にカスタマイズするオプションもあります。

* ランディングページの完全なカスタマイズ（ログインページ、ログアウトページ、パスワードを忘れた場合のページなど）
* ドメインの完全なカスタマイズ

その他のブランディングオプションについて詳しくは、sales.team@workfront.com までお問い合わせください。
