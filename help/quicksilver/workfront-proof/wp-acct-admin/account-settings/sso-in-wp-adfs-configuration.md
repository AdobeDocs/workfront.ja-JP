...
1. Microsoft AD FS 2.0をコンピューターにダウンロードします。
...
1. フェデレーションメタデータは、AD FS 2.0 スナップイン／サービス／エンドポイントフォルダーにあります。 「メタデータ」セクションで、フェデレーションメタデータタイプのメタデータを探します。 メタデータを表示するには、このエンドポイントをブラウザーに貼り付けます。 また、{adfs.your-company.com}を独自の詳細に置き換えた後、https://*&lt;adfs.your-company.com>*/FederationMetadata/2007-06/FederationMetadata.xmlのリンクに直接移動することもできます。
...
これにより、信頼者信頼のほとんどが設定されます。
   ...
   >* 完全なカスタムドメイン （例：www.your-proofing.com）が[!DNL ProofHQ] アカウントに設定されている場合は、「{yoursubdomain}.proofhq.com」の部分の全体を独自のドメインに置き換えて、[!DNL ProofHQ] メタデータリンクを作成します。
