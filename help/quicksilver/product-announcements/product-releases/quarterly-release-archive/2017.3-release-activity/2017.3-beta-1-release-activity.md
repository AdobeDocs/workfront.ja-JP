...
description: このページでは、2017.3 リリースのプレビュー環境で最近使用されたすべての変更について説明します。 このページの機能は、2017 年 8 月 10 日にプレビュー環境で使用できるようになりました。 2017 年 11 月初旬に、実稼動環境で利用可能になる予定です。
author: Luke
...
このページでは、2017.3 リリースのプレビュー環境で最近使用されたすべての変更について説明します。 このページの機能は、2017 年 8 月 10 日にプレビュー環境で使用できるようになりました。 2017 年 11 月初旬に、実稼動環境で利用可能になる予定です。
...
* [Workfrontのデフォルトの電子メールアドレスの変更](#workfront-default-email-address-change)
...
* [プロジェクトの使用率レポートにユーザー割り当て情報を含める](#include-user-allocation-information-in-the-utilization-report-on-a-project)
...
タスクおよびイシューの削除に対してシステム設定を有効にする方法の詳細については、「 [システム全体のタスクと問題の環境設定を構成する](../../../../administration-and-setup/set-up-workfront/configure-system-defaults/set-task-issue-preferences.md).
...
プロジェクトの稼働状況レポートでは、タスクの期間中に計画時間が再割り当てされたかどうかが考慮されるようになりました。 時間に対するユーザー割り当てが変更された場合（「スケジューリング領域でのユーザー割り当ての管理」で説明されているように）、稼働状況レポートで選択した日付にタスクの一部しか含まれていない場合は、稼働状況レポートのデータが影響を受けます。
