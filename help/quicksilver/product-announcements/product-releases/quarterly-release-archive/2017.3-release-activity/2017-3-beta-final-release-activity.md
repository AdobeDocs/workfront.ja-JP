...
アジャイルチームは、既にサポートされているスクラムアジャイル手法に加えて、Workfront内でカンバン手法を使用できるようになりました。

Workfrontのスクラムとカンバンのアジャイル手法には、次のような違いがあります。

**Workfront でかんばんを使用するメリット**

* バックログをカンバンアジャイルストーリーボードに表示します。

  詳細情報は、次を参照してください。

* バックログ上のアイテムを設定して、ほかのアイテムが「完了」と同じステータスに移動したときに、カンバンアジャイルストーリーボードに自動的に追加されるようにします。
...
* カンバンアジャイルストーリーボードに表示するWIP （進行中の作業）制限を設定します。
...
* アジャイルイテレーションに一連のストーリーを追加し、そのイテレーション用のストーリーボードを作成します。
* スクラムストーリーボードにイシューを追加できます。
* アジャイルチームのバックログに問題を記載する。
...
スクラムアジャイル手法を使用する場合、アジャイルチームのバックログにイシューを含めることができるようになりました（カンバン手法を使用する場合、アジャイルチームのバックログにイシューが表示されません）。 既存のスクラムアジャイルチームが問題を含めるには、この機能を有効にする必要があります。 問題は、2017.3 リリース以降に作成されたスクラムアジャイルチームのバックログに自動的に含まれます。
...
アジャイルバックログで「グループ化」および「フィルター」オプションを使用できるようになりました。これにより、バックログをグループ化して整理したり、特定のタスクや問題をフィルタリングしたりできます。
