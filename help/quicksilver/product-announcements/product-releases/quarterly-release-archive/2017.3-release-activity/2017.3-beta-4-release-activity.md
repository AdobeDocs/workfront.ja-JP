...
description: このページでは、2017.3 ベータ 4 リリースでプレビュー環境で最近使用されたすべての変更について説明します。 このページの機能は、2017 年 9 月 25 日の週にプレビュー環境で使用できるようになりました。 2017 年 11 月初旬に、実稼動環境で利用可能になる予定です。
author: Luke
...
このページでは、2017.3 ベータ 4 リリースでプレビュー環境で最近使用されたすべての変更について説明します。 このページの機能は、2017 年 9 月 25 日の週にプレビュー環境で使用できるようになりました。 2017 年 11 月初旬に、実稼動環境で利用可能になる予定です。
...
* [リソースのスケジュール時に複数のタスクの割り当てを変更する](#modify-assignments-for-multiple-tasks-when-scheduling-resources)
...
たとえば、プライマリのスケジュールが 1 か月で 100 時間の作業が可能で、プライマリの役割の FTE 可用性の割合が 75%、他の役割の FTE 可用性の割合が 25%である場合、リソース・プランナは、75 時間のユーザーを「の役割」にリストし、他の役割に属する 5 時間。
...
リソース・プランナのユーザーおよびロールに対して使用可能な時間と使用可能な FTE を計算する方法の詳細は、次を参照してください： [リソース・プランナのユーザーとロールに対する時間と工数の計算の概要](../../../../resource-mgmt/resource-planning/calculate-hours-fte-for-users-roles-resource-planner.md).
...
元の問題から情報を取り込むカスタムテキストモードビューの作成について詳しくは、 [表示：タスクとプロジェクトの一覧に元の問題情報を表示します](../../../../reports-and-dashboards/reports/custom-view-filter-grouping-samples/view-display-original-issue-info-task-project-list.md).
...
この変更以前は、次の権限プロファイルのみ使用可能でした。スーパーバイザ、マネージャ、管理者。

## イベント購読に追加された時間リソース {#hour-resource-added-to-event-subscriptions}

新しい Hour リソースを使用して、イベントの購読を作成し、請求アプリケーションをWorkfrontと同期させることができるようになりました。
