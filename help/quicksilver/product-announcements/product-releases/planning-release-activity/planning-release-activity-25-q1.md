...
これで、レコードタイプにフィールドを追加する際に、既存のWorkfrontのカスタムフィールドまたはネイティブフィールドを読み込んで、選択したレコードタイプに関連付けることができます。
...
## 「要求」エリアの「送信済み」セクションに新しい「計画」タブが追加されました

>[!NOTE]
>
>プレビューリリース：2024 年 11 月 27 日（PT）、高速リリースの実稼動：24.12 リリース（2024 年 12 月）の場合、四半期リリースの実稼動：25.1 リリース（2025 年 1 月）の場合

Workfrontの「リクエスト」エリアの「送信済み」セクションで、Workfront Planning リクエストを確認できるようになりました。 「送信済み」セクションには、2 つのタブが表示されます。1 つは「Workfront」タブのWorkfrontで送信された要求であり、もう 1 つは「計画」タブのWorkfront計画の要求フォームを使用して送信された要求です。
