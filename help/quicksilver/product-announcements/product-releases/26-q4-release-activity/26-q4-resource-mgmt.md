---
title: 2026年第4四半期のリソース管理の強化
description: 2026年第4四半期のリソース管理の強化
author: Becky
feature: Product Announcements
recommendations: noDisplay, noCatalog
source-git-commit: 1dd8ab20d11b2b4471308ac5402b31e20359a04c
workflow-type: tm+mt
source-wordcount: '116'
ht-degree: 20%

---

# 2026年第4四半期のリソース管理の強化

このページでは、2026年第4四半期リリースで行われたリソース管理の機能強化をプレビュー環境に対して説明します。 これらの機能強化は、前述のように本番環境で利用できるようになります。

2026年第4四半期リリースサイクルのこの時点で利用可能なすべての変更のリストについては、[2026年第4四半期リリースの概要](/help/quicksilver/product-announcements/product-releases/26-q4-release-activity/26-q4-release-overview.md)を参照してください。

## Workload Balancerの一括割り当て機能の強化

>[!NOTE]
>
>プレビュー：2026年7月30日>実稼動用の高速リリース：2026年7月30日>すべてのユーザー向けの実稼動：2026年7月30日>[!BADGE  スケジュールをオフ ]{type=Neutral}

ワークロードバランサーの一括割り当てダイアログで、**置換** アクションの名前が&#x200B;**割り当て**&#x200B;に変更され、アクションの動作をより詳細に説明できるようになりました。 機能は変更されていません。

詳しくは、[ワークロードバランサーを使用した作業の一括割り当て](/help/quicksilver/resource-mgmt/workload-balancer/assign-work-in-workload-balancer-in-bulk.md)を参照してください。
