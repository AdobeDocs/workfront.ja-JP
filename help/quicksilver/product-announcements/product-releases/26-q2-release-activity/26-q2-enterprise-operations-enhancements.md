...
* 再設計された&#x200B;**高度な割り当て**&#x200B;のエクスペリエンスと、拡張された設定可能性により、レート、時間枠、プロパティ全体の可視性と制御が向上しました。
  ...
  >[!NOTE]
  >
  >4月16日の実稼動リリースの前に、古い高度な割り当てエクスペリエンスと新しい高度な割り当てエクスペリエンスのどちらかを選択できるスイッチが追加されます。 古いエクスペリエンスはデフォルトのままになります。
...
>
>この機能は、すべてのWorkfrontおよびワークフローパッケージの組織で使用できます。
>レートカードは、Workflow Ultimate パッケージの組織でのみ使用できます。

Workfrontでは、請求とコスト率の日付範囲にギャップを残すことができますが、意図的であることを確認する警告が表示されます。

レートが存在するあらゆる場所で、日付範囲にギャップがある可能性があります。
