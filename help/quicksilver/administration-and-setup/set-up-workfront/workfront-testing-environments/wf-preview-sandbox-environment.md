...
[!DNL Workfront] には、[!DNL Workfront] 本番環境のレプリカである次の 2 つのテスト環境があります。
...
プレビューサンドボックスは、組織のユーザーが本番環境に影響を与えることなく、本番環境のデータを安全にテストし、操作できる環境の役割を果たします。

プレビューサンドボックスには、実際の本番データが含まれていますが、毎週末に更新されるので、データは本番環境より最大 1 週間遅れている可能性があります。 最後の更新時間以降に作成された項目は、次の更新が行われるまで、プレビューサンドボックス環境にあります。
...
>[!IMPORTANT]
>
>このセクションで説明する手順は、[!DNL Adobe Admin Console]にオンボーディングされていない組織にのみ適用されます。 すべての組織が[!DNL Adobe Admin Console]にオンボーディングされているため、操作は必要ありません。
>
...
プレビューサンドボックスでシングルサインオンを使用する場合は、本番環境とは別にその設定を行う必要があります。 プレビューサンドボックスの SSO 設定は、本番環境の SSO 設定とは独立しています。

プレビューサンドボックスが（週末ごとに）更新されると、SSO 情報は本番環境からコピーされず、プレビューサンドボックス設定が上書きされます。

プレビューサンドボックスでシングルサインオンを設定する手順は、本番環境での手順と同様です。
...
* プルーフはプレビュー環境では使用できません。

* Frame.io ビューアはプレビュー環境では使用できません。 Adobeクラウドストレージは利用可能なので、サンドボックスでテストできますが、統一されたレビューと承認のエクスペリエンス全体を本番環境で検証する必要があります。 詳しくは、[Adobe クラウドストレージの概要](/help/quicksilver/review-and-approve-work/esm-overview.md)を参照してください。
