...
ユーザーが次のいずれかの操作を行うと、システムはビジネスプロファイル監査ログエントリを生成します。
...
ビジネスルールは、Ultimate Workfront プランを購入したお客様のみが利用できます。 詳しくは、[&#x200B; ビジネスルールの作成と編集](/help/quicksilver/administration-and-setup/set-up-workfront/configure-system-defaults/business-rules.md)を参照してください。
...
* カスタムフィールドの変更
   ...
   * フィールドサイズの変更
     ...
     これは、フィールドが次のいずれかのタイプである場合にのみ使用できます。単一行、段落、リッチテキスト
...
>[!NOTE]
>
>すべての組織がAdobe Admin Consoleにオンボーディングされているため、この機能は使用できなくなりました。詳細情報が必要な場合は、ネットワークまたは IT 管理者にお問い合わせください。
