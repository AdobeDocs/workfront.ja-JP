...
1. 「**追加の制限事項を設定**」をクリックし、次にアクセスレベルに対して次のいずれかの制限を設定します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr>
      <td role="rowheader">ユーザーによるブランドへのアクセスを許可</td> 
...
1. <span class="preview"> （オプション）このアクセスレベルのユーザーが特定のフィールドを表示できないように制限するには、「制限されたフィールドを追加」セクションにフィールドを追加します。</span>

   <span class="preview">このセクションでは、ネイティブ フィールドとカスタム フィールドの両方を検索できます。 制限されたフィールドは20個までです。</span>

   <span class="preview"> アクセス レベルがユーザーにプライマリ アクセス レベルとして割り当てられている場合、またはビジネス プロファイルを通じて割り当てられている場合、フィールドは制限されます。 ビジネスプロファイルについて詳しくは、[ ビジネスプロファイルの概要](/help/quicksilver/administration-and-setup/add-users/create-and-manage-users/business-profiles.md)を参照してください。</span>

   <span class="preview">制限付きフィールドは、引き続きシステム内のユーザーに表示されますが、空白として表示されるか、実際のデータではなくN/Aが表示されます。</span>
