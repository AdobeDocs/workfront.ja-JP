...
* 解決オブジェクト
  ...
  解決中のオブジェクトは、「**元の問題を保持する」オプションで構成された変換済みの問題であり、その解決策をこの** &lt;**（プロジェクト**&#x200B;または&#x200B;**タスク）**>に関連付けます。 親プロジェクトまたはタスクを削除すると、イシューはそのプロジェクトまたはタスクにリンクされなくなるので、解決オブジェクトとして識別されなくなります。 親を復元しても、リンクは復元されません。
