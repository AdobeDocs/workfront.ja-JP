...
title: キックスタートシナリオ：複数オプションのカスタムフィールドをWorkfrontにインポートする
...
1. 左側のメニューで「**システム**」を展開し、「**データを書き出し（キックスタート）**」をクリックします。
...
1. 「**ダウンロード**」をクリックします。 .xlsx ファイルがお使いのコンピューターにダウンロードされます。 そこに移動して開きます。

   ![Excelでデータを書き出しました](assets/existing-data-excel-parameter-sheet.png)
...
Excel スプレッドシートを入力する前に、上記の節で説明したように、キックスタートテンプレートをダウンロードします。
...
1. 前のセクションでダウンロードしたExcel スプレッドシートを開き、シートを確認します。 各シートは、アプリケーション内の 1 つのオブジェクトを表します。
...
1. 「**キックスタートスプレッドシートを使用してデータをアップロード**」セクションの「**ファイルを選択**」をクリックします。

1. コンピューターで準備したExcel スプレッドシートを参照し、見つけたら選択します。
