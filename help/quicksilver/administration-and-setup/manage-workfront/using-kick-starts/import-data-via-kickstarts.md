...
keywords: キックスタート,キックスタート,キックスタート,キックスタート
...
1. 含める情報のタイプを選択します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      <td scope="col"> <p>ダッシュボード</p> <p>システム内のすべての公開されている共有ダッシュボードを書き出すことができます。 システム全体で共有されていないダッシュボードは書き出すことができません。 1 回の書き出しで最大 100 個の特定のダッシュボードを選択できます。</p> </td> 
...
1. （オプション）カスタムデータを追加するには、次の手順に従います。
   ...
   * カスタムフィールドに複数のデータ値（ラジオボタン、チェックボックス、リストなど）を追加する必要がある場合は、「環境設定」タブに表示される縦棒のカスタムデータ区切り記号「|」を使用して値を区切ります。
     ...
     **例：** DE:Departments 列の下に A|D と入力すると、カスタムフォームに部門 A と部門 D が入力されます。
     ...
     >[!NOTE]
     >
     >区切り「|」のみを使用して、カスタムフィールドの値を区切ります。 **setCategoryID** など、他のスプレッドシート列では使用できません。
