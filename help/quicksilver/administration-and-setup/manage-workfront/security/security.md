---
user-type: administrator
content-type: reference
product-area: system-administration
navigation-topic: manage-workfront
title: セキュリティ
description: Adobe Workfront管理者は、Workfrontシステムのセキュリティ環境設定を設定できます。
author: Caroline
feature: System Setup and Administration
role: Admin
recommendations: noDisplay, noCatalog
exl-id: a8be3e69-0e1d-4a9e-9da8-4cde9643ca25
source-git-commit: 86a0a9ddf5be1515ed8f5c2a537b0e2ccd5a1b63
workflow-type: tm+mt
source-wordcount: '69'
ht-degree: 2%

---

# セキュリティ

この節では、次の記事を扱います。

* [システムセキュリティの環境設定の構成](../../../administration-and-setup/manage-workfront/security/configure-security-preferences.md)
* [認証用のパスワードポリシーの設定](../../../administration-and-setup/manage-workfront/security/configure-password-policies-authentication.md)
* [様々なシングルサインオンオプションの削除](../../../administration-and-setup/manage-workfront/security/removal-various-sso-options.md)
* [Adobe Workfront SAML 2.0 メタデータ証明書を更新します。](../../../administration-and-setup/manage-workfront/security/renew-wf-saml-2-certificate.md)
* [重複するユーザーを防ぐ](../../../administration-and-setup/manage-workfront/security/prevent-duplicate-users.md)
* [IP アドレスでAdobe Workfrontへのアクセスを制限する](../../../administration-and-setup/manage-workfront/security/restrict-access-workfront-ip-address.md)
* [ファイルの暗号化](../../../administration-and-setup/manage-workfront/security/file-encryption.md)
* [Adobe Workfrontドメインの変更](../../../administration-and-setup/manage-workfront/security/change-your-workfront-domain.md)
* [API キーの管理](../../../administration-and-setup/manage-workfront/security/manage-api-keys.md)
* [Adobe Workfrontプライバシーポリシー](../../../administration-and-setup/manage-workfront/security/workfront-privacy-policy.md)
