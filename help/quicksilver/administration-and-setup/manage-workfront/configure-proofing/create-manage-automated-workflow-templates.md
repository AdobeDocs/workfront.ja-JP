...
1. 人が自動ワークフローテンプレートのステージに追加され、プルーフのレビュアーとしてすでに存在している場合、テンプレートを適用すると、レビュアーはステージから削除されます。 ステージに別のレビュアーを追加しない場合は、レビュアーを追加するように求めるメッセージが表示されます。
...
1. **ステージ**&#x200B;セクションで、自動化ワークフローテンプレートの各ステージを構成します。
...
1. 「**テンプレートを次と共有する**」セクションで、オプション（テンプレートがまだ組織全体で共有されていない場合）をクリックして、テンプレートを使用できるユーザーを指定します。
   ...
   デフォルトでは、新しい自動化ワークフローテンプレートは組織の全員と共有されます。

1. 「**作成**」をクリックします。

## 自動化ワークフローテンプレートの変更

Workfront のプルーフ管理者は、自動化ワークフローテンプレートを変更できます。 変更内容は、変更時に自動的に保存されます。
...
1. **ワークフロー** セクションで、任意のステージの名前を変更し、そのステージの設定を展開します![必要な変更を行うには、ボタン ](assets/arrow-button.png)を展開します。
...
1. **共有** セクションで、ユーザーを削除する場合は、右側の「その他![個のアイコン ](assets/more-icon.png)」ボタンをクリックし、**削除**&#x200B;をクリックします。
