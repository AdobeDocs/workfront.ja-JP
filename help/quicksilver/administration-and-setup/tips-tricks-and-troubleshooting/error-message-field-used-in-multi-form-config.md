...
>[!NOTE]
>
>このエラーが表示されても、カスタムフィールドを含む複数のオブジェクトが見つからない場合は、ごみ箱を確認します。 ごみ箱のオブジェクトに、フィールドが含まれている場合があります。 ごみ箱内のオブジェクトはいつでも復元できるので、競合が解決されるまで、複数形式の競合エラーが引き続き発生します。
>
>別の式を持つフィールドを追加するには、次のいずれかの操作を実行してこの競合を解決する必要があります。
>
>* 競合するカスタムフォームからフィールドを削除
>* 削除したオブジェクトを復元し、競合するカスタム フォームの 1 つを分離してから、必要に応じてオブジェクトを再度削除します。
