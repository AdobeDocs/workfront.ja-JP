...
title: エラーメッセージ：問題が少し発生しました。 このフィールドは、マルチフォームの設定で使用されます。
...
いずれかのフォームのカスタムフィールドの計算を変更することはできません。これは、別のフォームの同じフィールドの数式と競合するからです。
この競合を解決するには、同じ計算カスタムフィールドを持つ複数のフォームが添付されているオブジェクトを見つけてから、次のいずれかの操作を行う必要があります。
...
>[!NOTE]
>
>エラーが表示されたが、カスタムフィールドを含む複数のオブジェクトが見つからない場合は、「ごみ箱」を確認してください。 ごみ箱のオブジェクトにフィールドが含まれている可能性があります。 ごみ箱のオブジェクトはいつでも復元できるため、競合が解決されるまで、多形式の競合エラーが引き続き発生します。
>
>別の式を持つフィールドを追加するには、次のいずれかの方法でこの競合を解決する必要があります。
>
>* 競合するカスタムフォームからフィールドを削除します
>* 削除したオブジェクトを復元し、競合するカスタムフォームのいずれかを切り離してから、必要に応じてオブジェクトを再度削除します。
...
1. 画面の右上隅にある&#x200B;**[!UICONTROL 検索アイコン]** ![検索アイコン ](assets/search-icon.png)で、**[!UICONTROL 詳細検索]**&#x200B;をクリックします。
