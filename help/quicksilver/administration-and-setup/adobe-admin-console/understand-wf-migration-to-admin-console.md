...
description: この記事では、組織をAdobe Admin Consoleに移行するプロセスについて一般的に説明します。これにより、Workfront管理者は何を期待できるのかを把握できます。
author: Becky
...
この記事では、組織をAdobe Admin Consoleに移行するプロセスについて一般的に説明します。これにより、Workfront管理者は何を期待できるのかを把握できます。
...
* **ユーザーはAdobe IDでログインします。** Adobeが既存のユーザーをAdobe Admin Consoleに移行すると、ユーザーは新しいAdobe ID （Adobe IDまたはAdobe Federated ID（SSO）を使用してWorkfront インスタンスにログインします。
...
* **Adobeは、最初にWorkfront管理者を自動的に移行します。** Workfront管理者がAdobe IDに移行されると、Adobe Product Profile Administrator （Workfront System Administrator）ロールが割り当てられます。 移行前にユーザーが持っていた既存の役割には影響しません。
...
* **ユーザーが移行されると、Workfrontへのログイン方法の変更を通知するメールがAdobeから送信されます。** ユーザーは、既存のAdobeでログインするか、同じメールアドレスを使用して新しいAdobe IDを設定することで、初めてAdobe ID IDを使用してログインするための招待を受け入れることができます。
