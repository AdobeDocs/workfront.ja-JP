...
title: 特定のステータスを使用した保留中の承認プロセスを持つオブジェクトのリスト
...
# 特定のステータスを使用して、保留中の承認プロセスを持つオブジェクトを一覧表示する

ステータスを削除しようとすると、システム内の少なくとも1つの保留中の承認プロセスにあるため、ステータスを削除できないというエラーメッセージが表示されることがあります。 レポートを実行して、承認待ちプロセスのオブジェクトのリストを表示し、各オブジェクトに対して何を実行する必要があるかを決定できます。
