...
1. 「**フィールドを追加**」タブを開きます。
   ...
   ![「フィールドを追加」タブ](assets/add-a-field.jpg)

1. 「**新しいフィールド**」![新しいフィールドアイコン](assets/new-field.jpg) を選択したうえで、次のいずれかのフィールドタイプを選択します。
...
1. 「**フィールド設定**」タブで、追加するカスタムフィールドのタイプに使用できるオプションを設定します。
   ...
   <p><strong>メモ：</strong>数値フィールドの文字数制限は 16 文字です。テキストフィールドを使用して数値を入力し、制限を回避することもできます。</p>
     ...
     <tr data-mc-conditions=""> 
      ...
      <td> <p>（先行入力フィールドのみ）オブジェクトタイプにフィルターを追加して、ユーザーがフィールドを使用する際に選択できるオブジェクトを制限します。 </p> <p>例えば、以下の条件を満たすユーザー名のみを選択できるようにフィールドを制限することができます。</p> 
       ...
       </ul> <p>テキストモードの構文を使用して、選択したオブジェクトタイプのフィルターを定義する必要があります。テキストモードを使用したフィルターの作成について詳しくは、<a href="/help/quicksilver/reports-and-dashboards/reports/text-mode/edit-text-mode-in-filter.md" class="MCXref xref">テキストモードを使用したフィルターの編集</a>を参照してください。</p>
       <p><b>メモ</b>：   
        <ul> 
     ...
     <tr> 
      ...
      <ul> 
      <li>費用、会社、イテレーション、請求記録、ドキュメント、グループのオブジェクトタイプに関連付けられたカスタムフォーム。</li> 
      <li>計算済み、説明テキストおよびセクション区切りフィールドタイプ</li> 
      </ul>
...
1. （オプション）手順 3～5 を繰り返して、他のカスタムフィールドを追加します。
