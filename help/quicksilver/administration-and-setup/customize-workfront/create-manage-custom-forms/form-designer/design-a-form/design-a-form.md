...
![オブジェクトタイプを選択](assets/new-custom-form-select-objects.png)
...
1. <span class="preview"> （オプション）より多くのオブジェクトに添付できるようにフォームにオブジェクトタイプを追加する場合は、フォームデザイナーのヘッダーで「**オブジェクトタイプ**」をクリックします。 追加するオブジェクトタイプを選択し、フォームから削除するオブジェクトタイプをすべて選択解除します。</span>
1. （オプション）フォームに他のオブジェクトタイプを追加して、より多くのオブジェクトに添付できるようにする場合は、**オブジェクトタイプ**&#x200B;の横にある「**追加**」アイコン ![オブジェクトを追加アイコン](assets/add-objects-icon.png) をクリックし、表示されるメニューから必要なタイプを選択します。この操作を繰り返して、必要な数のオブジェクトタイプを追加できます。

   フォームに複数のオブジェクトを追加したら、オブジェクトタイプで「X」をクリックしてフォームから削除できます。
...
1. 画面の右側で、追加するカスタムフィールドのタイプに使用できるオプションを設定します。
   ...
   <table>
    ...
    <tr>
      <td><span class="preview">財務権限タイプ</span></td>
      <td><p><span class="preview">ユーザーがこのカスタムフィールドを表示または編集する前に必要な財務権限タイプを選択します。 計算フィールドには任意の形式を使用できます。</span></p>
      ...
      <p><span class="preview">詳しくは、<a href="/help/quicksilver/administration-and-setup/customize-workfront/create-manage-custom-forms/form-designer/design-a-form/restrict-access-to-financial-data.md"> カスタムフィールド内の財務データへのアクセスを制限</a>を参照してください。</span></p>
      </td>
...
>[!NOTE]
>
>* Outlook プラグインでは、外部ルックアップフィールドはサポートされていません。
