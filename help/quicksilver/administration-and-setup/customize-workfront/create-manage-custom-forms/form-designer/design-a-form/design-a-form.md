...
* API名はデフォルトでは読み取り専用です。 Workfrontでカスタムフォームを使用した後は、このAPI名を変更しないことをお勧めします。 変更してしまうと、Workfront の他の領域で参照される可能性のあるカスタムフィールドが認識されなくなります。

  例えば、カスタムフィールドをレポートに追加した後でAPI名を変更した場合、Workfrontはレポート内でカスタムフィールドを認識せず、新しい名前を使用してレポートに追加しない限り、レポート内で正しく機能しなくなります。

  また、フィールドのAPI名に依存する既存の統合や計算を解除することもできます。
