...
1. カスタムフォームを添付するオブジェクトタイプを選択し、「**続行**」をクリックします。
   ...
   ![オブジェクトタイプを選択](assets/new-custom-form-select-objects-new-spectrum-icons.png)
...
### フィールド API名とラベルに関する注意事項 {#notes-on-field-names-and-labels}
...
* すべてのフィールドにはAPI名が必要です。 この名前は、Workfront 全体の様々なエリア（レポート、ホーム、API の操作など）にカスタムフィールドを追加した場合に、システムがそれを識別できるようにします。 初めてフィールドまたはウィジェットを設定し、ラベルを入力すると、API名フィールドが自動的に入力されます。 「ラベル」フィールドと「API名」フィールドは同期されません。 これにより、システムで表示される名前を変更することなく、ユーザーに表示されるラベルを変更するオプションが提供されます。

* カスタムフィールドのAPI名は、組織のWorkfront インスタンス内で一意である必要があります。 これにより、別のカスタムフォームで既に作成されたものを再利用できます。

* 可能ですが、Workfrontでカスタムフォームを使用し始めた後で、このAPI名を変更しないことをお勧めします。 変更してしまうと、Workfront の他の領域で参照される可能性のあるカスタムフィールドが認識されなくなります。

  例えば、カスタムフィールドをレポートに追加した後でAPI名を変更した場合、Workfrontはレポート内でカスタムフィールドを認識せず、新しい名前を使用してレポートに追加しない限り、レポート内で正しく機能しなくなります。

* API名はデフォルトでは読み取り専用です。 API名を編集可能にするには、**API名**&#x200B;の横にある&#x200B;**編集** アイコンをクリックします。 次に、確認ボックスに「**confirm**」と入力し、**フィールド名の変更**」をクリックします。

* 組み込みのWorkfront フィールドに既に使用されているAPI名を入力しないことをお勧めします。

* Workfrontの別の領域でフィールドを使用する際のエラーを防ぐために、カスタムフィールド API名にピリオド/ドット文字を使用しないことをお勧めします。
...
* **リッチテキスト**: フィールドに複数の行のテキストを入力し、太字、斜体、下線、箇条書き、番号付け、下付き文字と上付き文字、ハイパーリンク、ブロック引用符、見出し、表を使用してテキストを書式設定できます。 15,000文字の制限は、テキストと書式設定に大きなスペースを提供します。

  >[!NOTE]
  >
  >リッチテキストフィールドタイプは、テキストを書式設定フィールドタイプに置き換えます。 右側のフィールドオプションの「**リッチテキストに変換**」ボタンをクリックすると、書式設定フィールドを含む既存のテキストをリッチテキストにすばやく変換できます。
...
1. 画面の右側で、追加するカスタムフィールドのタイプに使用できるオプションを設定します。
   ...
   <table>
    ...
    <tr>
     <td>API 名</td>
    <td><p>（必須）この名前は、システムによるフィールドの識別方法です。 初めてウィジェットを設定し、ラベルを入力すると、API名フィールドが自動的に入力され、それに一致します。 「ラベル」フィールドと「API名」フィールドは同期されません。 これにより、システムで表示される名前を変更することなく、ユーザーに表示されるラベルを変更するオプションが提供されます。</p>
...
日付フィールドには、ユーザーが日時を選択できるカレンダーが表示されます。

日付フィールドを追加するには：

1. 画面の左側にある「**新しいフィールド**」タブで、**日付**&#x200B;を見つけて、キャンバス上のセクションにドラッグします。
...
1. 画面右側で、カスタムフィールドのオプションを設定します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 名</td> 
      <td> <p>（必須）この名前は、システムによるフィールドの識別方法です。 初めてウィジェットを設定し、ラベルを入力すると、API名フィールドが自動的に入力され、それに一致します。 「ラベル」フィールドと「API名」フィールドは同期されません。 これにより、システムで表示される名前を変更することなく、ユーザーに表示されるラベルを変更するオプションが提供されます。</p> <p>詳しくは、<a href="design-a-form.md#notes-on-field-names-and-labels">フィールド名とラベルに関するメモ</a>を参照してください。</p> </td> 
     </tr>
     <tr> 
      <td role="rowheader">手順</td> 
     ...
     <tr> 
      ...
      <td><p>日付とともに時刻をフィールドに表示する場合は、このオプションを選択します。</p></td>
     </tr> 
     <tr>
      <td>アクティブ</td>
      <td><p>このオプションは、デフォルトでオンになっています。<p><p>フィールドを非アクティブに設定すると、そのフィールドはレポート、フィルター、ビューから除外され、カスタムフォームフィールドライブラリでは使用できなくなります。</p></td>
...
1. 画面右側で、カスタムフィールドのオプションを設定します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 名</td> 
      <td> <p>（必須）この名前は、システムによるフィールドの識別方法です。 初めてフィールドを設定し、ラベルを入力すると、API名フィールドが自動的に入力され、一致します。 ただし、「ラベル」フィールドと「API名」フィールドは同期されません。これにより、システムに表示される名前を変更することなく、ユーザーに表示されるラベルを変更するオプションを使用できます。</p>
      <p>詳しくは、<a href="design-a-form.md#notes-on-field-names-and-labels">フィールド名とラベルに関するメモ</a>を参照してください。</p> </td>
     </tr> 
      <td role="rowheader">手順</td> 
      <td> <p>カスタムフィールドに関する追加情報を入力します。 ユーザーがカスタムフォームに入力する際に、疑問符アイコンの上にポインタを合わせると、ここに入力した情報を含むツールチップが表示されます。</p> </td> 
     ...
     <tr> 
      ...
      <ul><li>フォームの保存後にフォーマットタイプを変更できますが、制限が 1 つあり、オブジェクト上の既存の値はすべて新しいタイプに変換できる必要があります。 （例えば、書式タイプがテキストで、オブジェクトが「abc」という値を格納している場合、フィールドを変換できず、システムが「abc」を数値/通貨に変換できないというエラーが表示されます）。 数学計算でフィールドを使用する場合は、必ず「数値」または「通貨」フォーマットを選択してください。</li>
      <li>数値または通貨を選択すると、0 から始まる数値が自動的に切り捨てられます。</li>
      <li>数値フィールドの文字数制限は 16 です。 テキストフィールドを使用して数値を入力し、制限を回避することもできます。</li>
      </ul></td>
     </tr> 
     ...
     <tr> 
      ...
      <li>{referenceObject}.{fieldName} - フィールドがオブジェクトの一部である場所。 この構文はカスタム式に似ています。 （例：portfolioID={project}.{portfolioID}）</li></ul>
...
1. 画面の左側にある&#x200B;**新しいフィールド** タブで、**内部参照**&#x200B;または&#x200B;**複数選択の内部参照**&#x200B;を見つけて、キャンバス上のセクションにドラッグします。
1. 画面右側で、カスタムフィールドのオプションを設定します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td><p>フィールドに関連付けるオブジェクトタイプを選択します。</p> <p>「<strong>適用</strong>」または「<strong>保存して閉じる</strong>」をクリックした後は、フィールドのオブジェクトタイプを変更できません。</p>
      <p><strong>メモ：</strong></p>
      <ul><li>Workfront 管理者が Workfront ユーザーインターフェイスでポートフォリオ、プログラム、またはプロジェクトの名前をカスタマイズした場合、このドロップダウンリストには、カスタマイズされた名前ではなく、オブジェクトのデフォルトの Workfront 名が表示されます。 これについてサポートが必要な場合は、Workfront 管理者にお問い合わせください。</li>
     ...
     <tr>
      ...
      <td><p>オブジェクトタイプにフィルターを追加して、ユーザーがフィールドを使用する際に選択できるオブジェクトを制限します。 </p> <p>例えば、以下の条件を満たすユーザー名のみを選択できるようにフィールドを制限できます。</p> 
       ...
       <p><b> ヒント：</b> フィルターを内部検索フィールドに直接追加する前に、フィルターをテストするレポートを作成できます。 これにより、フィルターが正しいオブジェクトを返すことを確認できます。 次に、レポートでテキストモードに切り替え、テキストモードステートメントをコピーして、内部参照フィルターに追加できます。</p>
       <p><b>メモ</b>：
       <ul> 
        <li>既存のカスタムフォームを編集する場合、内部検索フィールドにフィルターを追加しても、ユーザーがフィールドを使用して既に追加したオブジェクト（フィルターの範囲外）は削除されません。</li> 
        <li>このフィルターは、モバイルデバイスでは使用できません。 内部検索フィールドにフィルターを使用すると、そのフィールドは、フィルターの影響を受けないユーザーのモバイルデバイスに表示されます。</li> 
        </ul></p>
      </td>  
     </tr>
     <tr> 
...
1. ウィジェットの次のいずれかのプロパティを入力または編集します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 名</td> 
      <td> <p>（必須）この名前は、システムによるウィジェットの識別方法です。 初めてウィジェットを設定し、ラベルを入力すると、API名フィールドが自動的に入力され、それに一致します。 「ラベル」フィールドと「API名」フィールドは同期されません。 これにより、システムで表示される名前を変更することなく、ユーザーに表示されるラベルを変更するオプションが提供されます。</p> <p>詳しくは、<a href="design-a-form.md#notes-on-field-names-and-labels">フィールド名とラベルに関するメモ</a>を参照してください。</p> </td> 
     </tr> 
     <tr> 
...
このリストには、カスタムフォームで使用できるネイティブフィールドと、各フィールドで使用できるオブジェクトタイプが表示されます。

* **実際のメリット**: プロジェクト
* **実際の完了日**：プロジェクト、タスク、イシュー
* **実際のコスト**：プロジェクト、タスク、イシュー
* **実際の期間**: プロジェクト、タスク
* **実際の時間数**：プロジェクト、タスク、イシュー
* **実際の開始日**：プロジェクト、タスク、イシュー
* **予算**: プロジェクト、テンプレート、Portfolio
* **コミット日**：タスク、イシュー
* **会社**: プロジェクト、テンプレート、レートカード
* **条件**：プロジェクト、タスク、イシュー
* **条件タイプ**: プロジェクト、テンプレート
* **変換済みイシューの発信元**: プロジェクト
* **通貨**: プロジェクト、テンプレート
* **説明**：プロジェクト、タスク、イシュー、テンプレート、テンプレートタスク、Portfolio、プログラム、グループ、ドキュメント、レートカード、担当業務
* **期間**: タスク、テンプレートタスク
* **期間タイプ**：タスク、テンプレートタスク
* **期間単位**: タスク、テンプレートタスク
* **入力者**：プロジェクト、タスク、イシュー、テンプレート、テンプレートタスク、グループ
* **エントリ日**：プロジェクト、タスク、イシュー、テンプレート、テンプレートタスク、グループ
* **為替レート日付**: プロジェクト
* **固定コスト**: プロジェクト、テンプレート
* **固定収益**: プロジェクト、テンプレート、請求記録
* **グループ**：プロジェクト、テンプレート、Portfolio、プログラム、会社、レートカード
* **問題の種類**：問題
* **反復**: タスク
* **最終更新者**：プロジェクト、タスク、イシュー、テンプレート、テンプレートタスク
* **最終更新日**：プロジェクト、タスク、イシュー、テンプレート、テンプレートタスク
* **名前**：プロジェクト、タスク、イシュー、テンプレート、テンプレート、Portfolio、プログラム、グループ、ドキュメント、会社、レートカード、担当業務
* **Owner**: プロジェクト、テンプレート、Portfolio、プログラム
* **パフォーマンス インデックス メソッド**: プロジェクト、テンプレート
* **予定利益**: プロジェクト、テンプレート
* **予定完了日**：プロジェクト、タスク、イシュー
* **予定期間**: プロジェクト、テンプレート
* **予定時間**：プロジェクト、タスク、イシュー、テンプレート タスク
* **予定開始日**：プロジェクト、タスク、イシュー
* **Portfolio**：プロジェクト、テンプレート、プログラム
* **優先度**：プロジェクト、タスク、イシュー、テンプレート、テンプレートタスク
* **プログラム**：プロジェクト、テンプレート
* **予定完了日**: プロジェクト、タスク
* **予定期間（分）**: タスク
* **予定開始日**: プロジェクト、タスク
* **キューのトピック**：問題
* **準備状況**: タスク
* **参照番号**：プロジェクト、タスク、イシュー、テンプレート、テンプレートタスク
* **リソース管理者**: プロジェクト
* **スケジュール モード**: プロジェクト、テンプレート
* **重要度**：問題
* **スポンサー**：プロジェクト、テンプレート
* **ステータス**：プロジェクト、タスク、イシュー
* **ストーリーポイント**：タスク
* **タスクの制約**: タスク
* **テンプレート**: プロジェクト
* **解決済み**: プロジェクト、タスク
* **見積もり合計費用**: プロジェクト、テンプレート
* **合計推定収益**: プロジェクト、テンプレート
* **URL**：プロジェクト、タスク、イシュー、テンプレート、テンプレートタスク
* **作業量**: タスク
...
1. 画面右側で、カスタムフィールドのオプションを設定します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 名</td>
      <td> <p>（必須）この名前は、システムによるフィールドの識別方法です。 初めてフィールドを設定し、ラベルを入力すると、API名フィールドが自動的に入力され、一致します。 「ラベル」フィールドと「API名」フィールドは同期されません。 これにより、システムで表示される名前を変更することなく、ユーザーに表示されるラベルを変更するオプションが提供されます。</p>
      <p>詳しくは、<a href="design-a-form.md#notes-on-field-names-and-labels">フィールド名とラベルに関するメモ</a>を参照してください。</p></td> 
     </tr> 
     <tr> 
      <td role="rowheader">手順</td> 
     ...
     <tr>
      ...
      <td><p>参照フィールドにフィルターを追加して、ユーザーがフィールドを使用する際に選択できるアイテムのリストを制限します。 </p> <p>例えば、以下の条件を満たすユーザー名のみを選択できるようにフィールドを制限できます。</p> 
       ...
       <p>フィールドにシステムフィルターが存在する場合、カスタムフィルターを追加しない場合は、デフォルトで適用されます。 カスタムフィルターは、システムフィルターを上書きします。</p>
...
1. ウィジェットの次のいずれかのプロパティを入力または編集します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      <td role="rowheader">API 名</td> 
      <td> <p>（必須）この名前は、システムによるウィジェットの識別方法です。 初めてウィジェットを設定し、ラベルを入力すると、API名フィールドが自動的に入力され、それに一致します。 「ラベル」フィールドと「API名」フィールドは同期されません。 これにより、システムで表示される名前を変更することなく、ユーザーに表示されるラベルを変更するオプションが提供されます。</p>
    <p>詳しくは、<a href="design-a-form.md#notes-on-field-names-and-labels">フィールド名とラベルに関するメモ</a>を参照してください。</p></td> 
     </tr> 
     <tr> 
      <td role="rowheader">URL</td> 
...
1. 画面右側で、カスタムフィールドのオプションを設定します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tr> 
     ...
     <tr> 
      <td role="rowheader">API 名</td>
      <td> <p>（必須）この名前は、システムによるフィールドの識別方法です。 初めてフィールドを設定し、ラベルを入力すると、API名フィールドが自動的に入力され、一致します。 「ラベル」フィールドと「API名」フィールドは同期されません。 これにより、システムで表示される名前を変更することなく、ユーザーに表示されるラベルを変更するオプションが提供されます。</p></td> 
     </tr> 
     <tr> 
      <td role="rowheader">手順</td> 
