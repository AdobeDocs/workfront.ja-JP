...
>[!NOTE]
>
>Workfrontでカスタムフォームを使用し始めた後で、この名前を変更しないことをお勧めします。これを行うと、Workfrontの他の領域で参照されるカスタムフィールドが認識されなくなります。
>例えば、カスタムフィールドをレポートに追加し、後でその名前を変更した場合、Workfrontはレポート内でカスタムフィールドを認識せず、新しい名前を使用してレポートに追加しない限り、レポート内で正しく機能しなくなります。
>
