   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr>
      ...
      <td><span class="preview"><p>このオプションをオンにすると、数式で使用されるフィールドから財務権限が自動的に取得されます。 権限定義は、以下の「財務権限タイプ」フィールドに表示されます。</p><p>数式内のすべてのフィールドにアクセスする必要があります。 例えば、2つのフィールドが計算フィールドで使用され、そのうちの1つが請求権限を適用しており、2番目のフィールドにコスト権限が適用されている場合、ユーザーは請求率とコスト率の両方を表示して計算値を表示する権限を持っている必要があります。</p></span></td>
     </tr>
     <tr>
      <td><span class="preview">財務権限タイプ</span></td>
      <td><p><span class="preview">自動権限フィールドがオフになっている場合、ユーザーがこのカスタムフィールドを表示または編集する前に必要な財務権限タイプを選択できます。 計算フィールドには任意の形式を使用できます。 数式で使用されるフィールドによって、このフィールドがアクティブかどうかが決まります。 権限フィールドが空白の場合、数式内のフィールドは財務権限をサポートしません。</span></p>
...
計算は、通常、式で始まり、カスタムフォームがオブジェクトに添付される際に参照するフィールドを含む括弧が続きます。
   ...
   1. 大きいテキストボックスをクリックし、計算に追加できる&#x200B;**式**&#x200B;および&#x200B;**フィールド**&#x200B;をクリックします。
      ...
      **フィールド**&#x200B;の下にあるオブジェクト名を展開すると、そのオブジェクトで使用可能なすべてのフィールドが表示されます。 リストは200項目に制限されています。 フィールド名を知っていれば、検索できます。
      ...
      >  フィールド式![の](assets/colors-fields-expressions.jpg)色
