...
* 単一のビルトインのフィールドへのシンプルな参照。
  ...
  >[!INFO]
  >
  >**例：** プロジェクトとタスクによって生成された収益を計算するには、ビルトインフィールドであるActual Revenueを含む計算カスタムフィールドを作成できます。 誰かがカスタムフォームをプロジェクトまたはタスクに添付すると、プロジェクトまたはタスクの収益がフィールドに表示されます。

* 1 つ以上のフィールドを参照する式。 これには、カスタムフィールド、その他の計算カスタムフィールド、組み込みフィールドなどがあります。
  ...
  >
  >次の手順では、次の例のような式を作成する方法を確認できます。

>[!NOTE]
>
>ダイレクト フィールドに変更を加えると、計算フィールドの値が自動的にトリガーされます。 （ダイレクトフィールドは、Workfront API エクスプローラーで使用できるフィールド、またはオブジェクトに添付されたカスタムフォームのカスタムフィールドです）。 参照または数式を変更するには、フィールド値を手動で再計算する必要があります。
...
既存の計算カスタムフィールドを使用する場合、計算は新しいフォームに転送されません。 新しいカスタムフォームの同じフィールドに、計算を再度追加する必要があります。
...
1. （条件付き）この計算フィールドの数式を変更することで影響を受ける可能性のあるすべてのオブジェクトを確認するには、**競合の確認**&#x200B;をクリックします。
...
>[!IMPORTANT]
>
>新しい計算カスタムフィールドを作成する前に、計算に必要なデータがWorkfrontに存在することを確認するために、含める既存のフィールドを特定します。
   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      ...
      <td>フィールドのラベルを入力します。 これは、ユーザーがカスタムフォームを使用するときに表示される内容です。 自動的に入力されるフィールド <b>API Name</b>は、レポートでWorkfrontによって参照されます。</td> 
     </tr> 
     <tr> 
     ...
     <tr> 
      ...
      <td> <p>フィールドの結果を保存および表示する形式です。</p> <p>フィールドを数学計算で使用する場合は、常に<strong>数字</strong>または<strong>通貨</strong>形式を使用します。 <strong>数値</strong>または<strong>通貨</strong>を選択すると、0で始まる数値が自動的に切り捨てられます。</p> 
      <p><b>重要</b>：形式を選択する前に、新しいフィールドに適した形式を検討してください。 カスタムフォームを保存した後は、書式フィールドを編集できません。 また、誤った形式を選択すると、今後の計算や集計値がレポートやリストのグループ化に影響を受ける可能性があります。</p>
      <p><strong> メモ </strong>: <strong>通貨</strong>形式の計算フィールドには、引用符を含めないでください。 例えば、"800.00" ではなく、800.00 を使用してください。 引用符を使用すると、通貨タイプの言語形式の微妙な違いにより、予期しない結果が生じる可能性があります。</p></td>
     </tr> 
     <tr>
      <td>アクティブ</td>
     ...
     <tr>
      <td>自動許可</td>
      <td><p>このオプションをオンにすると、数式で使用されるフィールドから財務権限が自動的に取得されます。 権限定義は、以下の「財務権限タイプ」フィールドに表示されます。</p><p>数式内のすべてのフィールドにアクセスする必要があります。 例えば、計算フィールドで請求権限とコスト権限の2つのフィールドを使用する場合、ユーザーは両方を表示して計算値を表示する権限を持っている必要があります。</p></td>
     </tr>
     <tr>
      <td>財務権限タイプ</td>
      <td><p>自動権限フィールドがオフになっている場合、ユーザーがこのカスタムフィールドを表示または編集する前に必要な財務権限タイプを選択できます。 計算フィールドには任意の形式を使用できます。 数式で使用されるフィールドによって、このフィールドがアクティブかどうかが決まります。 権限フィールドが空白の場合、数式内のフィールドは財務権限をサポートしません。</p>
      <ul>
...
1. 「**計算**」ボックスで、次の計算の作成を開始します。
   1. 「**最大化**」をクリックすると計算エディターが開くので、計算を作成します。
      ...
      * 計算でカスタムフィールドを参照する場合は、フィールド名を中括弧で囲み、括弧内の先頭に `DE:` を指定します。
        ...
        **カスタムフィールドを参照するための名前の構文**
        ...
        >[!NOTE]
        >
        >内部参照フィールドの構文は、最後に`:name`を追加する必要があるため、他のタイプのフィールドの構文とは少し異なります。
        >
        >例えば、「エグゼクティブスポンサー」という名前のカスタム内部ルックアップフィールドで選択したオプションを参照するには、次のように入力します。
        >
        >`{DE:Executive sponsor}.{name}`
        >
        >同じ例でtypeahead フィールドを使用する場合は、次のように入力します。
        >
        >`{DE:Executive sponsor:name}`
        ...
        **複数オブジェクトカスタムフォームの計算カスタムフィールド**
        ...
        マルチオブジェクトカスタムフォームでは、選択したオブジェクトタイプは、フォームの計算カスタムフィールドで参照される少なくとも1つのフィールドと互換性がある必要があります。 オブジェクトと互換性のないフィールドでは、フォームにN/Aが表示されます。
   ...
   1. 大きいテキストボックスをクリックし、計算に追加できる&#x200B;**式**&#x200B;および&#x200B;**フィールド**&#x200B;をクリックします。
      ...
      **フィールド**&#x200B;の下にあるオブジェクト名を展開すると、そのオブジェクトで使用可能なすべてのフィールドが表示されます。 リストは200項目に制限されています。 フィールド名を知っていれば、検索できます。
