   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     <tr> 
      ...
      <td> <p>オブジェクトでカスタムフォームを表示し、入力する機能。</p> <p><b>メモ</b>:Light およびコントリビューターライセンス （またはワーク、レビュー、リクエストのライセンス）を持つユーザーの場合、これが最も利用可能なオプションです。</p> <p>「<strong>詳細設定</strong>」をクリックして、次のことを許可するかどうかを指定します。</p> 
       <ul> 
     ...
     <tr> 
      ...
      <td> <p>Standard ライセンスまたは Plan ライセンスを持つユーザーのみが使用できます。 </p> <p>ユーザーは、編集権限を持つオブジェクトにフォームを追加できるだけでなく、フィールドの追加、編集、削除など、カスタムフォームを完全に編集することもできます。</p> <p>「<strong>詳細設定</strong>」をクリックして、次のことを許可するかどうかを指定します。</p> 
       <ul> 
...
1. （オプション）カスタムフォーム（添付先のオブジェクト）へのアクセスを、前の手順で指定したアクセスに制限する場合は、共有ボックスの右上隅にある歯車アイコン ![](assets/gear-icon-settings-with-dn-arrow.jpg) をクリックし、「**システム全体のアクセスを削除**」をクリックします。
   ...
   >* ほとんどの組織では、作業対象のオブジェクトにフォームを添付し、そのデータをレポートに表示する際に、システム内のすべてのユーザーがカスタムフォームに必ず記入できるようにしたいと考えています。組織にこれが当てはまる場合は、を使用することをお勧めします **システム全体で表示する**. このようにオプションを設定すると、ダイアログボックスに「システム全体の表示」が表示されます。
   >   
...
1. クリック **共有** フォームを共有する準備ができたら、フォームデザイナーの右上に表示されます。
1. 表示されるボックスで、 **カスタムフォームアクセス権の付与先**&#x200B;で、カスタムフォームを共有するユーザー、チーム、担当業務、グループまたは会社の名前の入力を開始し、を押します **Enter** 名前が表示されます。
1. 先ほど追加したユーザー、チーム、担当業務、グループまたは会社のアクセスを調整するには、名前の右にあるドロップダウンメニューをクリックし、次の使用可能なオプションのいずれかと、その詳細設定を指定します。
...
1. （オプション）カスタムフォーム（フォームが添付されたオブジェクト）へのアクセスを前の手順で指定したものに制限するには、の下のドロップダウン矢印をクリックします **アクセスできるユーザー**&#x200B;を選択してから、 **招待されたユーザーのみがアクセスできます**.

   変更する場合は、次を選択できます **システム内の全員が表示できます**.
   ...
   >* ほとんどの組織では、作業対象のオブジェクトにフォームを添付し、そのデータをレポートに表示する際に、システム内のすべてのユーザーがカスタムフォームに必ず記入できるようにしたいと考えています。組織にこれが当てはまる場合は、を使用することをお勧めします **システム内の全員が表示できます**. このようにオプションを設定すると、ダイアログボックスに「システム全体の表示」が表示されます。
   >   
