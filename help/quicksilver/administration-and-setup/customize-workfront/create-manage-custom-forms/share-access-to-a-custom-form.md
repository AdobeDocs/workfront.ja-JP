...
ただし、カスタムフォームがまだ添付されていないオブジェクトでは、次のいずれかに該当する場合を除き、ユーザーは（標準またはプランナーのアクセスレベルを持っている場合でも）カスタムForms ドロップダウンメニューからカスタムフォームを添付することはできません。

* 誰かが「システム内の全員が表示して添付できる」というカスタムフォームを共有しました
