...
1. 左側のパネルで、**システム**／**顧客情報**&#x200B;をクリックします。
   ...
   購入したWorkfront パッケージによっては、お客様情報ページに一部のセクションが表示されない場合があります。 自社でどのWorkfrontパッケージを使用しているのかを把握する必要がある場合は、アカウント担当者にお問い合わせください。
...
* <span class="preview">**ストレージの概要**

  ドキュメントのストレージの使用状況と割り当て量の確認について詳しくは、[ ドキュメントのストレージ制限の確認](../../documents/managing-documents/check-document-storage.md)を参照してください。</span>
...
* **メール許可リスト**
  ...
  メールをメール許可リストに追加する方法について詳しくは、[ メールを設定するを参照してください](/help/quicksilver/administration-and-setup/get-started-wf-administration/configure-your-email-allowlist.md)。
