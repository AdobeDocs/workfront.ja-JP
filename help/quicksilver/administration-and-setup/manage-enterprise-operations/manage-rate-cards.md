...
1. 有効な日付レートのリスト内の特定の場所にレートを追加するには、レートにカーソルを合わせて、レート名の右側にある&#x200B;**More** メニューを選択します。 次に、**行を上に挿入**&#x200B;または&#x200B;**行を下に挿入**&#x200B;を選択します。

   「請求レートを編集」ボックスが表示され、新しい請求レートの行が配置された場所に追加されます。 行の上または下に追加するために選択したレートは、青色の線で強調表示されます。 新しい請求レートを追加し、必要に応じて既存の請求レートの有効日を調整して、**保存**&#x200B;をクリックします。
