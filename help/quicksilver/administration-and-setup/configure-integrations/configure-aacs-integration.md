...
ワークフローが Adobe Experience Manager 全体に対して有効になり、設定されます。これらのワークフローは、プロジェクトテンプレートに適用できます。 これらのテンプレートは、テンプレートレベルで調整またはカスタマイズすることも、そのテンプレートからプロジェクトを作成する際にプロジェクトレベルで調整することもできます。
...
フォルダーツリーごとに最大100個のリンクされたフォルダーを作成できます。
...
>[!NOTE]
>
>* Workfront AEM Native Integrationでは、フォルダーツリーの数に関係なく、1つの統合ごとに&#x200B;**100個を超えるフォルダーを作成することはできません**。
>   * 例：単一のプロジェクトで4つのフォルダーツリーを使用する統合では、400個ではなく、最大100個の合計フォルダーを作成できます。
>* 各フォルダーツリーの最初のフォルダーは、Workfrontにリンクされているとして自動的にマークされます。 このフォルダーをリンクしない場合は、リンクを解除できます。
