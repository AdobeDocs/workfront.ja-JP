   ...
   <table style="table-layout:auto"> 
    ...
    <tbody> 
     ...
     <tr> 
      ...
      <td> <p>（サブグループではなく、グループの詳細を表示している場合にのみ使用できます）。 オプション <strong>このグループとサブグループをプライベートにする</strong>を有効または無効にします。</p> <p>パブリックグループの場合、編集ユーザー権限のあるユーザー（グループの内外を問わない）は、他のユーザーのプロファイルにグループを追加できます。 これは、プライベートグループに対しては実行できません。</p> <p>このオプションは、複数のレベルを持つグループの最上位階層の親グループでのみ編集できます。 すべてのサブグループは親グループの設定を継承します。</p> </td> 
     </tr> 
     <tr> 
