...
1. Workfront で新しいドキュメントまたはドキュメントフォルダーのリンクを作成し、新しい外部 ID を介して新しい場所のリソースを指すようにします。
   ...
   1. **フォルダー**：同じ場所に同じ名前で新しいフォルダーを作成します。
...
>[!IMPORTANT]
>
>フォルダーリンクの場合は、ドキュメントリンクの場合と異なり、新しいリンクを配置するWorkfront内のフォルダーの「documentFolderId」が必要です。 コピーするリンク先のフォルダーとして最も考えられるのは、同じ親フォルダーです。

>[!CAUTION]
>
>フォルダーの一時保存はありません。古いフォルダーを削除しないでください。 古いフォルダーを削除するには、設定エリアでカスタムドキュメント統合を無効にします。
...
* **外部ドキュメント ID**:Workfront外でアセットを保存する際に割り当てられる ID。 Workfront では、この「外部ドキュメント識別子」フィールドを使用して、Workfront の内部識別子を、外部システム内のアセットを見つけるための識別子にマッピングします。したがって、新しい外部ストアからドキュメントまたはフォルダーをリンクする場合は、外部ドキュメントプロバイダーが新しいリポジトリまたはストア内のドキュメントを識別できるように、適切な形式で新しい外部ドキュメント識別子を作成する必要があります。
