...
* このモジュールの接続を設定するには、Workfrontで OAuth アプリケーションを作成している必要があります。
  ...
  手順については、を参照してください [OAuth アプリケーションの作成](#create-an-oauth-application) この記事の内容です。

## モジュール設定

Workfront Fusion でAdobe Experience Manager ワークフローを含むプロジェクトを作成する場合は、Workfront/その他のアクション モジュールを使用する必要があります。

1. を追加 **Workfront** > **その他のアクション** モジュールをシナリオに追加します。
1. が含まれる **接続** フィールドで、このモジュールが使用するアカウントに接続するWorkfront接続を選択します。
   ...
   接続を作成する必要があるクライアント ID およびクライアント秘密鍵の作成方法については、を参照してください。 [OAuth アプリケーションの作成](#create-an-oauth-application) この記事の内容です。

1. が含まれる **レコードタイプ** フィールド、選択 `Issue`.
1. が含まれる **アクション** フィールド、選択 `convertToProject`.
1. が含まれる **ID** フィールドに、プロジェクトに変換するイシューの ID を入力またはマッピングします。
1. Enable （有効） **詳細設定を表示**.
1. モジュールの下部までスクロールし、を見つけます。 **プロジェクト （詳細コレクション）** フィールド。
1. 次のテキストをに貼り付けます。 **プロジェクト （詳細コレクション）** フィールド。
...
1. 置換 `Folder tree ID here` （フォルダー ID を使用）。

   フォルダーツリー ID を検索するには、を参照してください [フォルダーツリー ID の検索](#locate-folder-tree-ids) この記事の内容です。

   複数のフォルダーツリーを使用するには、ID をコンマで区切ります。

   `"aemNativeFolderTreeIDs": ["Folder tree ID here","Second folder tree ID here"],`
1. 置換 `New project name here` 新しいプロジェクトに付ける名前。
1. 置換 `Template ID here` を、新しいプロジェクトに使用するテンプレートの ID に置き換えます。
...
1. を開きます **ネットワーク** デベロッパーツールの「」タブ
1. が含まれる **フィルター** ボックス、を入力 `object-workflow`.
1. 名前列で、表示される英数字の ID をクリックします。

   ![フォルダー ID 1 の検索中](assets/finding-folder-id-1.png)

1. 「」をクリックします **プレビュー** 英数字 ID の右側にある Tab キー。
...
1. 新規プロジェクトに使用するフォルダーツリーを開きます。 をメモします `_id` フィールド値。 複数のフォルダーツリーを使用する場合は、をメモしておきます `_id` 使用するフォルダーツリーのフィールド値。

   ![フォルダー ID 2 を検索しています](assets/finding-folder-id-2.png)

   次のとおりです `aemNativeFolderTreeIDs`  に入力する値 **プロジェクト （詳細コレクション）** のフィールド **Workfront** > **その他のアクション** Fusion モジュール。

## OAuth アプリケーションの作成

このモジュールの接続用に、Workfrontで OAuth アプリケーションを設定する必要があります。 Fusion の特定のWorkfront接続に対して、これを行う必要があるのは 1 回だけです。

1. Workfrontで、OAuth アプリケーションの作成を開始します（「」を参照） [ユーザー資格情報を使用して OAuth2 アプリケーションを作成します（認証コードフロー）](/help/quicksilver/administration-and-setup/configure-integrations/create-oauth-application.md#create-an-oauth2-application-using-user-credentials-authorization-code-flow) （の OAuth2 アプリケーションの作成）を参照してください。 [!DNL Workfront] 統合。
1. クライアント ID とクライアント秘密鍵を安全な場所にコピーします。
1. が含まれる **リダイレクト URI** フィールドに、以下を入力します。
