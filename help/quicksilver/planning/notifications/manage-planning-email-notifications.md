...
1. （条件付き、オプション）誰かがPlanning オブジェクトへのアクセス権を要求または付与した後、権限リクエストを通知するメールに移動します。 メールの送信者はAdobe Experience Cloudです。
...
1. （オプション）リクエストを送信した後、または送信したリクエストが承認または拒否された後にWorkfrontが送信するメールに移動します。 メールの送信者はAdobe Workfrontです。
