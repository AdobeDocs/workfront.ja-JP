---
title: 戦略的ホーム：30日間のランチパッド
description: Workfront計画を、心理的安全性を優先し、混乱を最小限に抑え、迅速に成果を上げる戦略を用いて展開する方法を説明します。
...
このガイドでは、心理的安全性を優先し、混乱を最小限に抑え、迅速に成果を上げる戦略を設計するために、Adobe Workfront計画を展開する方法を説明します。

このガイドは、Workfront Planningを導入しているWorkfront管理者を対象としています。

## 短期間で開始してスマートに拡張する方法の概要

普及を成功させるための最も一般的な障害はテクノロジーではありません。 それは“技術の責任”です。 新しいメンバー、新しいプロセス、新しいツールを一度に使用するという課題に直面すると、そのツールは不満の対象となります。

30日間のランチパッドは、「チームファースト」の導入モデルにもとづいて構築されています。 通常は数カ月にわたる調整を必要とする全社的な導入を試みる代わりに、特定の機能部門内でROIを即座に達成する、分散型のチーム主導のアプローチに重点を置いています。

## 段階的な移行を達成するためにガイド付きの自律性を利用する

製品を効果的に展開するには、ガイド付き自動化機能を利用するのが賢明です。 これは、管理者が、管理されたデフォルトとテンプレートの構造化されたシステムで、チームがどのように作業すべきかを定義することを意味します。

道路のレーン（または管理されたデフォルトとテンプレート）を定義し、その中でチームが独自の作業パスを柔軟に選択できるようにします。

最も重要なことは、ガイド付き自律性の進化です。 初日に完全なエンタープライズ分類法を適用する代わりに、最初のパイロットチームのニーズに基づいて、最小限のグローバル定義から開始できます。

オンボーディングを行うチームが増えるにつれて、一般的なパターンを観察し、グローバル基準を繰り返し適用することができます。

管理者として、あなたはコントロールを提供するのではなく、実際に機能しているものに基づいてインテリジェントに拡大・縮小します。

この進化的シフトをアクティブ化するには、次の操作を行います。

1. イノベーションに取り組む準備ができているパイロットチームを選びましょう。
2. 最初の15日間で、次のふたつの重要な経路のいずれかを通じてモデルを証明します。

   * **並行パス（またはデュアルトラッキングアプローチ）:** チームは現在のワークフローを維持しながら、Workfront Planningで次の計画サイクルを同時にモデル化します。

     これにより、1 ポイントの失敗のリスクなしに、筋肉の記憶と自信を構築します。

   * **クリーンなブレーク：** サンプルデータを使用してモデルを検証する最初の設計演習の後、チームはライブの開始点を次のサイクルのWorkfront Planningに直接移動します。

   どちらのアプローチでも、理論の枠を超え、可能な限り迅速に実際のアプリケーションに移行することを目標としています。これにより、ツールがサンドボックスのエクササイズではなく、実際の戦略的作業に使用されるようになります。


## ブレイクスルーの瞬間までの30日間のパスの概要

導入の成功は、勢いに乗るものです。 この30日間のスケジュールにより、設定から最初の自動勝利へと移行します。

### 1～10日目：チーム基盤

一元化された構造を構築することで、戦略的な基盤を構築できる。

複雑なエンタープライズ全体の分類法はまだ必要ではありませんが、すぐに2つの環境を作成する必要があります。 例えば、ハブアンドスポーク型アーキテクチャアプローチを採用する場合、次のように作成できます。

* **グローバル分類ワークスペース** （ハブ）：これは、システムの一般的な構造と命名規則である必要があります。
* 最初の&#x200B;**チームワークスペース** （スポーク）：これは、個々のチームの作業に対するアプローチと一致している必要があります。

詳しくは、[ワークスペースの作成](/help/quicksilver/planning/architecture/create-workspaces.md)を参照してください。

このチームファーストのアーキテクチャにより、次のことが可能になります。
...
* **デザインに関する共同作業：**&#x200B;試験運用チームと直接作業して、抽象的な企業の理想ではなく、実際のニーズを反映するレコードタイプとフィールドを定義します。

* **戦略を作業に接続：**&#x200B;計画レコードとWorkfront オブジェクト間のリンクを有効にします。

  詳しくは、[接続されたレコードタイプの概要](/help/quicksilver/planning/architecture/connect-record-types-overview.md)を参照してください。

* **現在のデータをミラーリング：** チームの既存のツール（Excelなどのコアアプリケーション）を使用して、Workfront Planningにデータを入力し、親しみやすく関連性の高いものにします。

* **柔軟な設計：** Workfront Planningは進化するように設計されていることを忘れないでください。 チームレベルのプロセスから始め、ビジネスの準備が整ったらエンタープライズレベルのガバナンスに容易に強化または移行することができます。

### 15日目：最初のブレイクスルーの瞬間

15日目までに最初の大きな勝利を目指すことをお勧めします。 自動化されたプロジェクト作成です。

これを実現するには、次の機能を実装する必要があります。

1. 「実行準備完了」ステータスを識別するネイティブオートメーションを設定します。
...
これにより、チームは戦略的な意図を自動的にトリガー作業に反映させることができます。 この画期的な瞬間は、戦略と作業の橋渡しの価値を証明し、賛同を得るのに役立ちます。

### 16～30日目：企業規模への段階的な移行

稲妻が生まれた瞬間に、チームの出発点をWorkfrontプランニングに移すことができます。

このチームファーストのモデルが成功するにつれ、企業の拡大に向けた道筋を始めるには、うまくいくパターンを特定することから始めます。

チームファーストからエンタープライズ規模に移行するには、次の点を考慮する必要があります。

* **受注フォームの紹介：** リクエストフォームを戦略的リクエストの新しいゲートウェイとして使用して、データの整合性を確保します。

  詳しくは、[Adobe Workfront Planning](/help/quicksilver/planning/requests/create-request-form.md)でのリクエストフォームの作成と管理を参照してください。

* **観察と標準化：** グローバル分類ワークスペースを使用して、パイロットチームがツールをどのように使用しているかを観察します。 これらの実例は、最終的な企業設計を形成します。

* **従来のスプレッドシートを廃止：** プロセスの成熟度が高まるにつれ、Workfront Planningに完全に移行します。

* **シフト電源ユーザー：** レコード テーブル ビューを使用して、直接かつ高速なプラン管理を行います。

  詳しくは、[テーブルビューの管理](/help/quicksilver/planning/views/manage-the-table-view.md)を参照してください。

## ランチパッドの成功指標の概要

成功のマーカーを常に探し、結果に基づいて調整することをお勧めします。

これらの健康状態の早期指標を探してください：

* **導入：**&#x200B;最初の5～10件の戦略的記録は実行プロジェクトに正常にリンクされ、リンクされたプロジェクトの作成と自動化のプロセスはシームレスで簡単に繰り返すことができます。

* **支持：** パイロットチームは、ツールを隣接するチームに伝道し始めます。

* **効率性：**&#x200B;自動化により、戦略的な承認からプロジェクトの開始までの時間が短縮されます。

* **信頼：**&#x200B;関係者は、レビュー用の手作業によるステータスデッキではなく、Workfront計画タイムラインを使用しています。

## ベストプラクティスとヒント

CDPの導入を成功に導くには、導入方法と期待値を明確にする必要があります。

Adobe Workfrontの導入を成功に導くための推奨事項を、いくつか紹介します。

### 導入の相談：

* **パイロットチームの準備を開始：** グローバル展開が大幅に向上したことを確認します。 イノベーションに熱心で、他の人々に青写真を提供できるチームと協力して、概念実証に集中しましょう。

* **壁ではなく車線を定義：** ガイド付き自律性を使用して、探索を妨げることなく構造を提供します。

* **心理的安全性の優先順位付け：**&#x200B;新しいワークフローに慣れるまで、レガシーツールが適切に配置されていることをチームに伝えます。

* **短期間の成功を祝う：**&#x200B;最初の自動プロジェクト作成をチーム全体でハイライト表示します。

* **チャンピオンを設定：**&#x200B;移行をリードする各チームのパワーユーザーを特定します。

### 悪い例：

* **大規模な変更を強制しません：**&#x200B;すべてのユーザーに初日に切り替えることを強制すると、テクノロジーの問題が発生します。

* **理由を無視しない：**&#x200B;すべてのチームメンバーが、戦略的スレッドによって作業がより可視化される方法を理解していることを確認します。

* **完璧を待ってはいけません：**&#x200B;分類法は進化します。 最善の推測から始め、必要に応じて調整しましょう。
