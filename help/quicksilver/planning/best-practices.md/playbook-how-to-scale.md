...
description: 導入が成功した後にAdobe Workfront Planningを展開する方法について説明します。
...
Adobe Workfront計画のユースケースを最初に成功させることは、需要の急増や新しいチームのアクセスの機会の増加など、画期的な出来事です。経営陣は、全社的な可視性の可能性を見出しています。

しかし、これは導入の最も脆弱な段階でもあります。厳格なガバナンスに素早く移行しすぎると、チームがスプレッドシートに戻ってしまうリスクがあります。緩やかに移動しすぎると、解決しようとした乱雑な状態が再現されます。

このプレイブックでは、最初に可視性を優先し、ローカルのイノベーションを可能にし、エンタープライズ標準を早急に課するのではなく、実際の運用上の成功から浮上させる調整エンジンとしてWorkfront計画を使用するバランスの取れたアプローチの概要を説明します。

以下は、アドビの経験から得られた知見と、注目すべき点に関するヒントです。

## &#x200B;1. サクセストラップ

Workfront計画の導入において最も危険な段階は、最初の成功事例や概念実証の直後だと感じる場合があります。 熱意は高いが、技術的負債と行政上の無秩序な拡大に対する恐れは、同様に有害な2つの対応につながる可能性がある。

* **ガバナンスを超える**：新しいチームがスプレッドシートに戻るように、システムを非常に厳密にロックします。

* **ガバナンスをゼロ**：すべてのチームが独自のフィールドとレコードタイプを作成できるようにして、レガシー環境で見つかった断片化されたメタデータの拡散を再作成します。

## &#x200B;2. 基本理念：和解エンジンとしてのWorkfront計画

Workfront計画では、チーム間の分断を解消するために、それらの相違点を可視化し、調整できるようにします。

この取り組みを支援するために、次の要素を検討してください。

* **導入速度の管理**：既存の環境を最適化する前に、新しいツールに拡大することについて慎重に感じるのは自然なことです。 最初に簡素化することを選択すると、高度に管理された基盤が提供されますが、調整の準備ができているチームの価値創出までの時間が遅れる可能性があります。 この変化をリードする最も効果的な方法は、可視性がステップ 1であることを認識することです。企業向けの共有されたツールに向けた勢い（プレゼンテーションやスプレッドシートの広がりから離れる）は、最終的に長期的な目標の妨げになります。

  まずクリーンアップを実行する権限を与えるのではなく、リソースの一部を継続的なメンテナンスに割り当て、より大きな部分を差し迫ったビジネスニーズの解決に割り当てることをお勧めします。

  例えば、分類法の「クリーンアップ」だけに1年を費やしても、付加価値はほとんど生まれません。 しかし、部門をまたいで可視性を確保することは、関係者が必要とする変革的な価値をもたらすと同時に、長期的に環境を管理するために必要な統合データ構造を提供します。

  統合されたエンタープライズカレンダーや統合された市場投入ロードマップにより、部門をまたいだ可視性を実現できます。

* **現実を認識**：独立したチームが独自のプロセスを自然に開発します。多くの場合、分断されたスプレッドシートには隠れています。 これらのプロセスを共有環境で可視化することで、プロセスをひとつの場所に集め、最終的に対処し、改善することができます。

  >[!IMPORTANT]
  >
  >Workfront計画に移行しても、混乱は生じません。既に存在しているものに光を当てることです。


* **進行状況の兆候を確認**: チームが独自のフィールドを要求しても、「無秩序」にはなりません。 これは導入の兆候だと考えてください。 つまり、Workfront Planningを作業スペースとして捉えているのです。

* **借金を管理します。隠さないでください**：後で異なる分類をクリーンアップするために必要な労力について懸念するのは自然なことです。 その一方で、厳格な基準を早期に課すという代替案は、プロセス（そして負債）を隠したままのスプレッドシートにチームを戻すことがよくあります。 現在の分類を使用してWorkfront Planningを開始できるようにすることで、債務を目に見える管理された環境に移行します。 これにより、最終的な調整は、単一の圧倒的な移行プロジェクトではなく、反復的なタスクになります。

## &#x200B;3. ガイド付き自律型ガバナンスモデル

道路のレーンとローカルのプレイグラウンド（または管理されたデフォルトとテンプレート）を定義すると同時に、チームはその中で独自のパスを柔軟に選択できます。

ガイド付き自律型ガバナンスモデルの次のコンポーネントを検討します。

* [グローバルレーン](#the-global-lanes)
* [地元の遊び](#the-local-playgrounds-or-the-spokes)

詳細については、次も参照してください。

* [戦略的ホーム：30日間のランチパッドを展開](/help/quicksilver/planning/best-practices.md/30-day-launchpad.md)
* [成功を設計する：キャンペーン階層のモデル化](/help/quicksilver/planning/best-practices.md/model-campaign-hierarchy.md)
...
* **によって管理**：センターオブエクセレンスまたはマーケティングオペレーション管理者。
...
* **実験オブジェクト**: チームの戦術的ニーズに固有のフィールドまたはレコードタイプ（ソーシャルチームの`Influencer Handle`やweb チームの`URL Slug`など）。

* **によって管理**: チームリード （軽いガイダンス付き）。

* **ルール**：ここではチームがイノベーションを実行できます。 3つ以上のチームでローカルフィールドを導入した場合、グローバルレーンに昇格できます。

## &#x200B;4. ガバナンスのパラドックスを考慮する：チームが最初で、基準は次のとおりです

Workfront計画の規模を拡大する際の一般的な課題は、まず最初に来るもの、すなわちエンタープライズガバナンスとチームの運用調整を決定することです。

これを乗り切る最も効果的な方法は、企業価値が双方向の通りに構築されていることを認識することです。

* **チームには関連性が必要**：企業は、チームが積極的に実行されている場合にのみ価値を実現します。 そのため、ガバナンスは、チームの既知の業務ニーズをサポートする構造を提供することで、チームにサービスを提供する必要があります。

* **企業は可視性を必要としています**：経営陣は、データが十分に整理され、十分に整理されている場合にのみ、十分な情報にもとづいた意思決定をおこなうことができます。 そのため、ポートフォリオを可視化するために必要な実行可能な最低限のメタデータを提供することで、企業にサービスを提供する必要があります。

管理された拡張の目的は、可視性を提供するために十分に標準化することによってこれら2つのニーズの交差点を見つけることですが、チームの実行を遅らせることにはなりません。

ガバナンスモデルの構築では、次の段階を考慮する必要があります。

* [優先順位付け：データとビジュアライゼーションの違い](#align-priorities-data-vs-visualization)
* [サービス主導の観察フェーズ](#the-service-led-observation-phase)
* [レトロアクティブ調和](#retroactive-harmonization)

### 優先順位付け：データとビジュアライゼーションの違い

規模拡大を行う際には、次のペルソナによって価値の定義が異なることを認識してください。

* **管理者または製品所有者**&#x200B;は、統一された分類と分類を値とします。 同社の目標は、長期的な拡張性をサポートするクリーンなデータアーキテクチャです。

* **関係者またはリーダー**&#x200B;は、グローバル カレンダーまたはポートフォリオ タイムラインでキャプチャできるビジュアライゼーションとinsightを重視します。 目標は、データを実用的なものにする稲妻の瞬間を特定することです。

>[!NOTE]
>
>関係者が必要とする可視化を、チームが管理者がデータ基準に関するニーズに対応するためのインセンティブとして活用することが重要です。 経営陣が求めるカレンダーを提供することで、統合された分類基準を実現できます。


### サービス主導の観察フェーズ

初期のスケールアップの間、管理者の役割は、チームと企業との間のこの交換を促進することです。

管理者は、次の操作を行うことで、この交換を促進できます。

* **標準化よりも運用を優先する**: グローバル定義の欠如によって停止するよりも、チームがサイロ化されたワークスペースで積極的に計画を立てることをお勧めします。 このアクティビティは、健全な現実世界の基準を構築するために必要な生データです。

* **可視性の最小値を特定**：経営陣と協力して、エンタープライズレポート用にクリーンにする必要がある3 ～ 5のフィールド（`Strategic Alignment`、`Start Date`、`Budget`など）を特定します。 適用エネルギーは、可視性の最小フィールドのみに集中させましょう。

### サービスとしてのガバナンス

チーム間で既知のニーズのパターンが見つかれば、そのパターンをグローバルサービスに統合することができます。 これがサービスとしてのガバナンスです。

またはサービスとしてのガバナンスを達成するには、次の手順を実行します。

* **成功したパターンを観察**: チームが既に構築および導入した勝利の分類法を特定します。

* **共同作業を促進**: チーム チャンピオンを集めて、ローカルでの成功を企業標準に絞り込みます。

* **サービスとしてのデプロイ**：新しいグローバル レーンを制限としてではなく、作業を行うユーザーのレポートとチーム間の調整を簡素化する方法としてロールアウトします。

>[!IMPORTANT]
>
>ガバナンスは、業務上の成功に対する対応であるべきであり、その前提条件であってはならないことを忘れないでください。

## &#x200B;5. フィールド管理のスケーリング機構

### パターンベースのフィールド成長モデル

この概念を適用するには、データ構造に対して慎重なアプローチが必要です。 ガバナンスの乱立を回避するには、個々のリクエストごとにグローバルフィールドを構築する必要があります。

代わりに、次のフィールド成熟度パスを使用して、実際の使用で企業標準を導くことができます。

* **レベル 1: ローカル実験**: チーム Aがワークスペースにカスタムフィールドを作成します。

* **レベル 2: パターン認識**：管理者は、チーム BとCが類似のフィールドを使用または要求していることに気付きます。
...
フィールドの関連性が失われ、廃止する必要がある時点に到達します。

Workfront Planningには現在、フィールドのネイティブアーカイブ機能がないため、ローカルフィールドを廃止するには、UIを混乱させることなく履歴データを保存するための意図的なソフト廃止プロセスが必要です。

フィールドを削除するには：

1. データの移行を行います。 テーブルビュー（またはFusion）を使用して、ローカルシャドウフィールドから新しいグローバルレーンフィールドに値を一括コピーします。 この移動中に、データが検証され、クリーニングされていることを確認します。

2. 非推奨フィールドの名前を変更：`[DEPRECATED]`または`z_`などのプレフィックスを使用して、ローカルフィールドの名前を変更します（例：`z_Language (Old)`）。 これにより、フィールドがフィールドピッカーの下部にプッシュされ、信頼できる唯一の情報源ではなくなることをユーザーに知らせます。

3. すべてのレコードフォームから非推奨フィールドを削除します。 これは最も重要なステップです。 これにより、必要に応じて、既存のテーブルビューやレポートに古いデータを表示したまま、新しいデータを入力しないようにします。

4. 非推奨フィールド（接頭辞が付けられ、フォームから削除されたフィールド）を30～60日間保持して、移行中にデータが見落とされないようにすることで、日没期間を開始します。 この期間が経過した後、データがグローバルレーンで完全に紐付けられた場合、ローカルフィールドをワークスペースから削除できます。

## &#x200B;6. Workfrontドリフトを避ける

プランニングが乱雑になるのを防ぐには、次の操作を行います。

* **抽象化レイヤーについて**：プランニングのすべてのフィールドは、戦略的な質問に答える必要があります。 フィールドが戦術的なトラッキングにのみ使用される場合（たとえば、フィールドが「このプルーフは承認されましたか？」などの質問に回答する場合）、Workfrontに保持します。

* **最初に統合を実現**: チームが新しいメタデータフィールドを必要とする場合は、最初にグローバル分類を確認するように招待します。 これには、グローバルタクソノミーワークスペースへの読み取り専用アクセス権をチームリードに付与する必要があります（セクション 7を参照）。 戦術的なニーズを既存の戦略的フィールドにマッピングすることで、不要な重複を防ぎ、レポートの統合性を維持できます。

## &#x200B;7. 読み取り専用アクセス表示モデル

チームがグローバル分類管理ワークスペースに読み取り専用でアクセスし、自社のワークフローにどのような概念が適用されるのかを確認できるようにすることで、分断された作業のノイズを回避しながら分断されたフィーリングを解決できます。

次の点に注意してください。

* **問題**: チーム （スポーク内）は、自分のレコードしか表示しないため、孤立しているように感じます。

* **修正**：共有レコードタイプのプライマリとして指定されたワークスペースへの読み取り専用アクセス権をチームに付与します。

  詳しくは、[ワークスペースの共有](/help/quicksilver/planning/access/share-workspaces.md)を参照してください。

* **結果**：チームは、インスピレーションと調整のために、より広範な企業コンテキストを確認できますが、ローカルのワークスペースはクリーンであり、特定のタスクに集中しています。

## &#x200B;8. ワークショップを通じた成長管理

Workfront計画の拡大は、技術的な課題と同様に、文化的な課題でもあります。 ガバナンスのギャップを埋めるために、ターゲットを絞ったワークショップを実施する。

次に、ワークショップのアイデアを示します。

* [「必要な混乱」ディスカバリーワークショップ](#the-necessary-mess-discovery-workshop)
* [「戦略的可視性」の調整セッション](#the-strategic-visibility-alignment-session)
* [「Spoke-to-global」のショーケース](#the-spoke-to-global-showcase)
* [「継続的サポート」オフィスアワー](#the-ongoing-support-office-hours)

### 「必要な混乱」ディスカバリーワークショップ

* **Audience**：地域マーケティングのリードとオペレーションのチャンピオン

* **目標**：現在のサイロ化された現実、または実際の断片化された運用データを文書化します。

* **メッセージ**:「フィールドを削除する予定はありません。 彼らがグローバル戦略とどのようにリンクしているかを理解するためにここにいます。」

* **結果**：ローカル戦術フィールドとグローバル戦略レーンのドラフトマッピング。

### 「戦略的可視性」の調整セッション

* **オーディエンス**：マーケティングに関する高度な関係者（経営陣の役割の人など）。

* **目標**：単純ファーストの不安を再構築します。

* **メッセージ**:「開始するには完全な分類は必要ありません。 Workfront計画を活用して、最適な分類法を構築しています。

* **結果**: Workfrontが現在の状態を維持している間に、Workfront Planningを紐付けエンジンとして前進させる承認。

### 「Spoke-to-global」のショーケース

* **Audience**: Workfront計画を検討している新しいチーム。

* **目標**：分断した感情を減らします。

* **メッセージ**:「指定されたWorkspaceにTeam Aのローカル作業を自動的にフィードする方法を確認してください。 同じように作業を可視化できます」。

* **成果**：新しい部門からオプトインし、地域の独立性を失うことなく、接続のメリットを享受できます。

### 「継続的サポート」オフィスアワー

* **Audience**：すべてのWorkfront Planning ユーザー（現在および見込み）。

* **目標**: トラブルシューティングと戦術的なガイダンスのために、繰り返し低い賭け金の環境を提供します。

* **メッセージ**:「間違った質問はありません。 アドビは、お客様の特定の計画の課題をリアルタイムで解決するお手伝いをします。

* **結果**：ユーザーの信頼の向上、技術的な摩擦の迅速な解決、およびグローバル標準化が必要な新しいパターンの特定。

## &#x200B;9. 大規模な人員配置：役割と責任

マネージドスケーリングモデルで成功するには、単なるツール設定だけでは不十分です。 グローバルチームとスポーク部門をまたいで役割を明確に分担する必要があります。

以下のサブセクションでは、スケーリングモデルの管理における主なプレーヤーのアイデアを見つけることができます。

### エンタープライズアーキテクト（センターオブエクセレンスまたはマーケティングオペレーション）

* **フォーカス**：企業の整合性、システムパフォーマンス、統合データ分類基準。
...
* **フォーカス**：戦略的整合、OKRの可視化、ポートフォリオの可視化（グローバル カレンダーなど）。
...
実装が成功した場合に得られるすべての学習により、さらなる実装に使用できるチェックリストが生成されます。

チェックリストに含めるべき事項の例を次に示します。

* [ ] **チャンピオンの特定**：この新しいチームのプロセス所有者またはチャンピオンは誰ですか？

* [ ] **ローカルの差分を定義**：このチームが現在グローバル標準で提供していない2 ～ 3個のフィールドが必要ですか？

* [ ] **グローバルレーンへのマッピング**：将来のチームのニーズの80%を満たすことができる既存のグローバルフィールドはどれですか？

* [ ] **グローバル可視性を付与**:1日目に、関連するプライマリワークスペースおよびグローバル分類ワークスペースへの読み取り専用アクセス権を付与します。
