...
Workfront Planning のドキュメントが含まれている記事の完全なリストについて詳しくは、[Adobe Workfront Planning に関する一般的な情報](/help/quicksilver/planning/planning-information.md)を参照してください。
...
* **接続されているレコードタイプ**、**接続されているレコード**、**接続されているフィールド**：Workfront Planning では、次のエンティティ間で接続を作成できます。
  ...
  レコードタイプ間の接続を確立すると、各タイプの個別のレコードやオブジェクトを相互に接続できます。レコード間の接続は、接続済みレコードフィールドまたは接続として表示されます。
...
* **ルックアップフィールド**（またはリンク済みフィールド）：2 つのレコードタイプ間の接続を確立し、個別のレコードを接続すると、接続元のレコードから接続済みレコードのフィールドを参照できます。
...
* [Adobe Workfront Planning に関する一般情報](/help/quicksilver/planning/planning-information.md)：Workfront Planning に関するドキュメントを含むすべての記事のインデックスは、関心エリア別にグループ化されています。
