...
リクエストフォームを作成するには、次の情報を入力する必要があります。

* 新しいフォームを追加し、そのフィールドとコンテンツ要素を設定する
* フォームの設定を構成して、今後のリクエストの承認と完了オプションを追加します
* フォームを公開
...
1. 「**リクエストフォームを作成**」ボックスで、リクエストフォームの名前を更新します。 デフォルトでは、フォームの名前は&#x200B;**名称未設定のフォーム**&#x200B;です。<!--check this; you logged a bug to rename it to 'Untitled request form' but was it fixed?-->
...
1. 「**作成**」をクリックします。
   ...
   * 選択したレコードタイプのテーブルビューで使用できるレコードフィールド。
...
1. （オプション）フォームから&#x200B;**デフォルトセクション**&#x200B;を削除するには、次の操作を行います。
   ...
   1. **デフォルトセクション**&#x200B;からすべてのフィールドを削除します。
...
1. （オプション）フォームの左側にある「**コンテンツ要素**」タブをクリックし、次のいずれかの要素を追加します。
   ...
   * **説明テキスト**：例えば、新しいセクションの手順を追加します。
   * **セクション区切り**：複数のフィールドを含むフォーム上の領域です。
...
1. 次のオプションから選択します。
   ...
   * **リクエストされたオブジェクトが作成されたときにリクエストが完了しました**：これにより、レコードが作成されたときにリクエストが完了します。
   * **リクエストされたオブジェクトが完了すると、リクエストが完了します**：これにより、レコードが完了としてマークされたときにリクエストが完了します。
...
1. フォームを作成して保存したら、**公開**&#x200B;をクリックしてフォームを公開し、一意のリンクを取得します。
   ...
   * **非公開** ボタンは、**公開** ボタンに置き換わります。 これをクリックすると、フォームにアクセスできなくなります。
   * **共有** ボタンがフォームに追加されました。
...
1. リクエストフォームのリストに移動し、次のいずれかの操作を行います。

   * レコードタイプのページのリクエストフォーム名の右側にある&#x200B;**詳細** メニュー![詳細メニュー](assets/more-menu.png)をクリックします。
   * リスト内のリクエストフォームの名前をクリックして開きます。
...
1. （条件付き）フォームを開いた場合、「**フォーム**」タブの右下隅にある「**保存**」をクリックして、フォームを保存します。
