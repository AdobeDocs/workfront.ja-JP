...
1. 任意のフィールドをクリックし、フォームの右側のパネルにあるコントロールを使用して、フィールドのサイズまたは次の情報を定義します。
   ...
   * **ロジックの追加**：フィールドを表示または非表示にするには、どの条件を満たす必要があるかを定義します。
   ...
   フォームでフィールドを選択すると、各フィールドのフィールドタイプが右側のパネルの上部に表示されます。 リクエストフォームのフィールドタイプの名前は、テーブルビューの名前とは異なります。 次の表は、テーブルビューのフィールドタイプの名前と、レコードタイプのリクエストフォームの同じフィールドの名前の違いを示しています。

   | Workfront計画フィールドタイプ | リクエストフォームフィールドタイプ |
   |-------------------------------|-------------------------|
   | 1 行テキスト | 1 行テキスト |
   | 段落 | 書式付きテキスト |
   | 通貨、数値、パーセンテージ | 1 行テキスト |
   | 単一選択 | 単一選択ドロップダウン |
   | 複数選択 | 複数選択ドロップダウン |
   | チェックボックス | チェックボックスグループ |

   >[!NOTE]
   >
   >   フィールドの形式は保持されます。 例えば、通貨とパーセンテージは 1 行のテキストフィールドとして表示されますが、フィールド内の情報は通貨とパーセンテージ値として表示されます。
