...
* リクエストフォームには、1 人または複数の承認者を追加できます。 承認者としてユーザーとチームを追加できます。
...
1. （任意）承認ルールの追加を開始します。 カスタム承認ルールごとに、次の操作を行います。
   ...
   >   承認ルールを追加する際は、次の点を考慮してください。
   >
   >   * デフォルトのルールのみが設定されている場合は、送信されたすべてのリクエストに適用されます。
   >   * カスタムルールが満たされた場合、リクエストの承認ワークフローにデフォルトは適用されません。 一致したカスタムルールのみが承認に適用され、デフォルトルールは無視されます。
   >   * 複数のカスタムルールが満たされた場合は、順序の最初のカスタムルールが適用されます。 この場合、デフォルトの承認は（存在する場合）適用されません。
