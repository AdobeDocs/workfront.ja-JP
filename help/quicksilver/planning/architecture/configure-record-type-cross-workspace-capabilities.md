...
1. 「**管理対象のワークスペースにこのレコードタイプを追加できるユーザーを選択**」フィールドで、管理対象のワークスペースにこのレコードタイプを追加するユーザーを追加します。

   あなたの名前が自動的にフィールドに追加されます。

   管理するワークスペースに、このレコードタイプを追加するユーザーを持つ個々のユーザー、グループ、チーム、担当業務または会社を追加できます。
...
1. **レコードタイプを編集** ボックスで、「**詳細設定**」タブを選択します。
1. **他のワークスペースからこのレコードタイプへの接続を許可** 設定を有効にします。<!-- check the setting name, this is the suggested UI text to be edited by Lilit and team-->
