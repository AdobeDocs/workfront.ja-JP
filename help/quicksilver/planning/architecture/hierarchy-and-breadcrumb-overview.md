...
* ワークスペース階層では、次のオブジェクトタイプのみを接続できます。
   ...
   * Workfront プロジェクト。 Workfront プロジェクトを他のレコードタイプの親として追加することはできません。 彼らは常に階層内の最後の子です。
* 以下のオブジェクトタイプを階層内に追加することはできません。
   * 接続可能またはグローバル レコード タイプとして設定されている場合でも、他のワークスペースからのレコード タイプ。 グローバルレコードタイプを階層に追加できるのは、階層を構築するワークスペースに追加されている場合のみです。
...
* 階層に含めるレコードとオブジェクトの接続されたフィールドに対して、[ リンクされたレコード タイプに対応するフィールドを作成 **] 設定をオンにする必要があります**。
* 階層設定のルールを次に示します。
