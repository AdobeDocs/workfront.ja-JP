...
1. <span class="preview"> 「**デフォルトの選択肢**」フィールドで、このフィールドにデフォルトで表示する選択肢を選択します。 複数の選択肢を選択できます。 ユーザーは、このフィールドを更新する際に、既定の選択肢を削除できます。</span>
...
1. <span class="preview"> 「**デフォルトの選択肢**」フィールドで、このフィールドにデフォルトで表示する選択肢を選択します。 選べる選択肢は1つだけです。 ユーザーは、このフィールドを更新する際に、デフォルトの選択肢を削除できます。</span>
...
1. 「**新しいフィールド**」タブで次の情報を追加します。
   ...
   * **日付形式**：このフィールドに表示する日付形式のタイプ。<!--update this casing - submitted bug for it-->
      ...
      * **時刻を含める**: タイムスタンプを含める場合は、このオプションを選択します。 これはデフォルトでは選択されていません。 フィールドを保存した後は、時間を含めることはできません。
         ...
         * **24hr**：例：18:00
         * **12hr**：例：午後6:00
      * **すべての共同作業者**&#x200B;に同じタイムゾーンを使用し、**タイムゾーン** <!--(************** logged bug to change this to "Show the same time zone to all users" ****************)-->を選択します。ユーザーのタイムゾーンに関係なく、フィールドを表示しているすべてのユーザーに同じタイムスタンプを表示する場合は、これを選択します。 選択したタイムゾーンがこのフィールドの列ヘッダーに追加されます。
...
>[!TIP]
>
>* 「人物」フィールドにユーザーの名前を追加すると、そのユーザーのプライマリジョブロールと電子メールもフィールドに表示されます。 ユーザーの電子メールを表示するには、アクセスレベルのUsers オブジェクトで「連絡先情報を表示」設定を有効にする必要があります。
>
>* Adobe Admin Consoleに追加されたユーザーのみが、「人物」フィールドに表示されます。 Workfront ユーザーは、「人物」フィールドに追加しようとすると表示されません。
>
...
1. <span class="preview"> デフォルトユーザー&#x200B;**または** デフォルトユーザー&#x200B;**フィールドで、このフィールドにデフォルトで表示するユーザーを選択します。</span>**

   <span class="preview">複数の値を許可&#x200B;**設定も選択した場合は、複数のユーザーを選択できます。</span>**

   <span class="preview"> ユーザーは、このフィールドを更新する際に、既定のユーザーを削除できます。</span>

   >[!TIP]
   >
   ><span class="preview">この設定で既定のユーザーとして選択したユーザーを非アクティブ化すると、ユーザーの名前が既定として保持されます。 非アクティブ化されたユーザーはフィールド値から削除されず、ユーザーが非アクティブ化されたことを他のユーザーに示すものはありません。</span>
