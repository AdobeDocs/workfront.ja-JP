...
1. （オプション）テーブルビューのフィールド名にポインタを合わせ、フィールド名の右側にあるドロップダウンメニューをクリックしてから、「**フィールドを編集**」をクリックします。
1. **値を表示** 設定をオンにして、各選択肢の値を表示します。 値は、Workfront データベースに表示される各選択肢の名前です。

   >[!NOTE]
   >
   >* Workfrontでは、各選択肢に一意の値を割り当てます。
   >
   >* 値は、小文字の形式で選択名に一致します。 複数の単語はアンダースコアで区切られます。
   >
   >* 複数のフィールド間で値を繰り返すことはできますが、1 つのフィールドで一意にする必要があります。
   >
   > ![ 値を表示切替スイッチ ](assets/show-values-toggle-and-choices-with-values.png)

1. （オプション） API 呼び出しやその他の統合で、選択値を使用します。
...
1. 「**新しいフィールド**」タブで次の情報を追加します。
   ...
   * **日付形式**：このフィールドに表示する日付形式のタイプ。<!--update this casing - submitted bug for it-->
      ...
      * **ヨーロッパ式**：例：16/05/2023
      * **ISO**：例：2023-05-16
   * **時間を含める**：タイムスタンプを含める場合は、このオプションを選択します。 デフォルトでは選択されていません。 フィールドを保存した後に時間を含めることはできません。
...
>[!TIP]
>
>* ユーザー名を「ユーザー」フィールドに追加すると、プライマリ担当業務 <span class="preview"> およびメール </span> もフィールドに表示されます。 ユーザーのメールを表示するには、アクセスレベルでユーザーオブジェクトに対して「連絡先情報を表示」設定を有効にする必要があります。
>
...
レコード ID フィールドタイプを使用して、各レコードのシステム生成の読み取り専用識別子を表示できます。

作成時に、レコード ID フィールドには英数字の値が自動入力されます。 式フィールドや API 統合で使用して、各レコードを一意に識別できます。

レコード ID フィールドの値を手動で変更することはできません。

1. この記事の「最初からフィールドを作成 [ の説明に従ってフィールドの作成を開始し、](#create-fields-from-scratch) レコード ID **フィールドタイプを選択** ます。

   ![ レコード ID フィールドタイプ ](assets/record-id-field-type.png)
...
1. 「**作成**」をクリックします。
   ...
   新しいレコード ID タイプのフィールドがレコードタイプに列として追加され、その値には英数字の値が事前入力されます。
