---
title: レコードを作成
description: Adobe Workfront Planningを使用する場合、レコードはレコードタイプのインスタンスです。 Workfront Planningでは、レコードタイプごとに一意のレコードを作成できます。その場合、レコードタイプを手動でテーブルビューに追加したり、リストから取り込んだり、複製したり、他のレコードに接続するときに作成したりします。 各レコードは、サムネール、カバー画像、固有の色に関連付けることができます。
