...
または
<p>スタンドアロンパッケージとして購入された場合の任意のPlanning パッケージ</p>
...
1. （条件付き） **フィールド値の変更**&#x200B;を選択した場合、**設定** セクションで次の操作を行います。

   1. ドロップダウンメニューからフィールドを選択します。 選択したレコードタイプに関連付けられているフィールドです。
   1. 選択したフィールドの条件の定義を続行します。
   1. 「**条件を追加**」をクリックして、最大5つのフィールドを追加し、その条件を定義します。

      次のいずれかのタイプのフィールドを追加できます。
      ...
      * 日付
      ...
      >* 各条件は、1つ以上の値を参照できます。 すべての条件で、作成する条件の数に関係なく、最大5つの合計値を参照できます。 例えば、合計6つの値を参照する3つの条件を持つ設定は、制限を超えています。 条件をネストすることはできません。 設定が5値の制限を超えると、条件の下にインラインエラーが表示され、参照値の数を減らさない限り、自動化を保存できません。
      >
...
1. （オプション）オートメーションを表示、無効化、または削除するには、オートメーションのリストから、オートメーションの名前の右側にある&#x200B;**詳細** メニュー![詳細メニュー](assets/more-menu.png)をクリックし、次のいずれかの操作を行います。

   * フィールド値の変更の自動処理の場合は、**表示**&#x200B;をクリックして、自動処理の設定を表示します。

     >[!TIP]
     >
     >オートメーションの設定を保存した後、フィールド値の変更によってオートメーションがトリガーされた場合は、その設定を編集できません。

   * ボタンクリックによる自動処理の場合は、**編集**&#x200B;をクリックして、次の情報を更新します。

     * 自動化名は、自動化名の右側にある&#x200B;**詳細** メニュー![詳細メニュー](assets/more-menu.png)をクリックし、**編集**&#x200B;をクリックします。
   ...
   * 「**無効化**」をクリックして、自動化をレコードのテーブルビューから削除し、ユーザーがレコードまたはオブジェクトの作成に使用できないようにします。
   ...
   * 「**削除**」をクリックして、自動処理を削除します。 削除されたオートメーションは復元できません。
