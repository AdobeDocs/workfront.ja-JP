...
* オートメーションで作成されたオブジェクトまたはレコードの名前は、作成元のレコード名と同じです。
...
1. （条件付き）レコードの作成を選択し、元のレコードタイプと「**アクション**」領域で選択されたレコードタイプの間に接続フィールドがない場合は、**現在のレコードが表示される接続されたレコードタイプのフィールド** フィールドの右側にある疑問符アイコンをクリックしてから、**追加** アイコン ![ 接続フィールドアイコンを作成 ](assets/create-a-connection-field-icon.png) をクリックして接続フィールドを追加します。

   「**アクション**」領域で選択したレコードタイプに対して、新しいフィールドが自動的に作成され、「**接続されたレコード**」という名前が付けられます。
