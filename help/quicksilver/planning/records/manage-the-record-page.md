...
description: Adobe Workfront Planningでは、レコードのプレビューとページのレイアウトを編集できます。
...
Adobe Workfront Planningでは、レコードのプレビューとページのレイアウトを編集できます。

レコードプレビューは、レコードタイプのビューに表示されるレコードページの小さいビューです。
...
<p>各Workfront計画パッケージに含まれる内容について詳しくは、Workfrontの担当者にお問い合わせください。 </p> 
  ...
  <tr>
   ...
   <td>
   <p>ワークスペースとレコードタイプに対する権限以上の権限を付与し、<span class="preview"> レコードに対する権限を管理</span> </p>  
...
* デフォルトでは、レコードの詳細ページには、レコードに関連付けられたすべてのフィールドが表示されます。

* プレビューページまたは詳細ページでは、レコードの新しいフィールドを追加できません。 プレビューページと詳細ページに表示するには、テーブルビューに新しいフィールドを追加する必要があります。

* レコードプレビューまたは詳細ページにセクションを追加して、共通の基準で情報を整理し、見つけやすくすることができます。

* 次の変更は、同じタイプのすべてのレコードに影響し、これらのレコードにアクセスするすべてのユーザーに表示されます。
...
* レコードのページには、次のタイプのページを追加できます。
   ...
   * 接続されたレコードページ
     ...
     詳しくは、「[接続されたレコードの追加」の節を参照してください。](#add-a-connected-records-page-to-a-record)

## レコードプレビューまたはページへのセクションの追加

レコードページにセクションを追加する際には、次の点を考慮してください。
...
1. 任意のタイプのビューで、レコードの名前をクリックします
   ...
   <span class="preview"> レコードのプレビューがビューで開きます。</span>
...
1. （オプション）レコードプレビューの右上隅にある「**新しいタブで開く**」アイコン「![新しいタブアイコンで詳細を開く](assets/open-details-in-a-new-tab-icon.png)」をクリックして、新しいタブでレコードのページを開きます。
   ...
   <span class="preview"> レコードページが開きます。 「詳細」タブがデフォルトで開きます。</span>
...
1. （オプション）次のいずれかの操作を行って、セクションを編集します。
   ...
   * 「**名前変更**」をクリックして、セクションの名前を変更します
     ...
     >[!TIP]
     >
     > セクションの名前をインラインで変更するには、名前をクリックします。

   * **上に移動**&#x200B;をクリックして、セクションを1つ上に移動します
...
1. （オプション）「**接続**」をクリックして、接続されたレコードタイプにさらにレコードを追加します。 詳しくは、[レコードの接続](/help/quicksilver/planning/records/connect-records.md)を参照してください。

1. （オプション）レコードカードにカーソルを合わせ、レコードの切断アイコン **-**&#x200B;をクリックしてから、**切断**&#x200B;をクリックします。 <!--this is copied to the Manage the Planning page in Workfront article; update in both articles-->
次のことが発生します。
...
1. （オプションおよび条件付き）実稼動環境を使用している場合、詳細ページに表示するフィールドに応じて、次のいずれかの操作を行います。
   ...
   * レコードのテーブルビューで非表示になっているレコードのフィールドを&#x200B;**詳細** ページから除外するには、「**すべてのフィールドを表示**」設定の選択を解除します。
...
1. <span class="preview"> （オプションおよび条件付き）プレビュー環境を使用している場合、詳細領域はデフォルトで&#x200B;**選択したテーブルビューフィールド** ビューで開きます。 これにより、テーブルビューに表示されるフィールドのみが表示されます。</span>
   ...
   >[!TIP]
   >
   > <span class="preview">選択したテーブル表示フィールド **ビューは、レコードの詳細プレビューページのテーブル表示でレコードを表示する場合にのみ使用できます。**   >**選択したテーブルビューフィールド**&#x200B;は、他のレコードタイプビューや、ブラウザーウィンドウで開くレコードの詳細ページでは使用できません。</span>

1. <span class="preview"> （条件付き）プレビュー環境で、次の操作を続行します。</span>
...
1. （オプション）接続されたレコードに移動する場合は、詳細ページの上部にあるパンくずリストでそのレコードの名前をクリックします。
   ...
   パンくずリストにレコード名が表示されない場合は、**詳細**&#x200B;をクリックします。 これにより、複数の行にパンくずリストが表示されます。 「**Less**」をクリックしてパンくずリストを短くし、1行に表示します。

   >[!TIP]
   >
   >レコードの詳細ページでパンくずリストを表示するには、ワークスペースに接続されたレコードタイプ、レコード、階層が必要です。
...
1. （オプション）レコードプレビューの右上隅にある&#x200B;**新しいタブで開く** アイコン ![新しいタブアイコンで詳細ボックスを開く](assets/open-details-in-a-new-tab-icon.png) <!--check the icon; they are changing it-->をクリックして、新しいタブでレコードのページを開きます。
   ...
   <span class="preview"> レコード **詳細** ページが新しいタブで開きます。</span>
...
1. レコード **詳細** タブで、フィールド名の左側にある&#x200B;**グラブ** アイコン ![&#x200B; グラブアイコン &#x200B;](assets/grab-icon.png)をクリックし、目的の場所にドラッグ&amp;ドロップします。
   ...
   >[!TIP]
   >
   >フィールドを別のセクションにドラッグ&amp;ドロップできます。
   >セクションには少なくとも1つのフィールドが必要です。
   >

   フィールドの新しい位置は、レコードを表示しているすべてのユーザーについて、同じタイプのすべてのレコードのプレビューとページの両方で更新されます。

   レコードのプレビューまたはページのレイアウトに対するすべての変更は、自動的に保存されます。

## レコードへの接続されたレコードページの追加

接続されたレコード ページのタブをレコードの詳細領域に追加することで、接続されたレコードまたはオブジェクトからの情報を表示できます。 これにより、テーブルまたはリスト表示の接続されたレコードがタブに追加されます。

詳細については、[接続されたレコードの追加ページをレコードに追加する](/help/quicksilver/planning/records/add-a-connected-records-page-to-a-record.md)を参照してください。
