...
Adobe Workfront計画エリアでレコードタイプを選択すると、そのタイプのすべてのレコードを様々な方法で表示できます。

複数の形式のビューでレコードを表示すると、最適な方法で情報を柔軟に調べ、理解できます。 構造化された概要、時系列のストーリー、日付ベースのレイアウト、シンプルなスクロール可能なリストのいずれを使用する場合でも、各ビューは独自の視点を提供します。
...
* リスト
  ...
  接続済みレコード ページのレコードは、リスト表示で表示できます。

  >[!IMPORTANT]
  >
  >レコードタイプページのレコードのリストにリスト表示を適用することはできません。 接続されたプロジェクトのリストに適用できるのは、レコードの接続済みレコードページのリスト表示のみです。<!--this will change-->

  詳しくは、次の記事を参照してください。

   * [接続されたレコード ページをレコードに追加する](/help/quicksilver/planning/records/add-a-connected-records-page-to-a-record.md)
...
このセクションの情報は、次のビュータイプに適用されます。

* テーブル
* タイムライン
* カレンダー

リスト表示について詳しくは、「[&#x200B; リスト表示の管理 &#x200B;](/help/quicksilver/planning/views/manage-the-list-view.md)」を参照してください。
