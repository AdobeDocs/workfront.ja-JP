...
Adobe Workfront Planning のレコードの「接続されたレコード」ページでプロジェクトとそのフィールドにアクセスすると、リスト表示でプロジェクトとそのフィールドを表示できます。<!--change 'projects' to other objects when they become available and the location of the list view-->

この記事では、レコードの [ 接続されたレコード ] ページにプロジェクトを表示するリスト ビューを作成または編集する方法と、ビューでプロジェクトを編集する方法について説明します。<!--change 'projects' to other objects when they become available and the location of the list view-->
...
* リスト表示では、レコードタイプページのすべてのオブジェクトタイプを表示することはできません。

  以下のオブジェクトは、レコードの [ 接続されたレコード ] ページで表示した場合にのみ、リスト ビューで表示できます。
...
1. （オプション）次のいずれかの操作を行って、リスト表示を変更します。
   ...
   1. 既存のビューの名前にポインタを合わせて **詳細** メニュー ![&#x200B; 詳細メニュー &#x200B;](assets/more-menu.png) をクリックし、次のいずれかをクリックします。
      ...
      * **削除**：ビューを削除します。
      ...
      >* システム・ビューは変更できません。
      >
      >* <span class="preview"> 自分と共有されていた、表示する権限のみを持つビューをリセットし、元の環境設定に戻すために変更した後でリセットするか、変更内容を含むビューをコピーして、コピーを共有できます。 詳しくは、[&#x200B; 拡張リストの使用 &#x200B;](/help/quicksilver/workfront-basics/navigate-workfront/use-lists/enhanced-lists.md) を参照してください。</span>
...
リスト表示で、最初の列に表示されるプライマリフィールドは削除できません。
   ...
   1. <span class="preview"> 「セルの書式設定 **」アイコン** 「セルの書式設定 ![&#x200B; アイコン &#x200B;](assets/format-cells-icon.png) をクリックします。 **フォーマット** ボックスが開きます。</span> <!--change the name of the box when they update it-->
      <span class="preview"> 次の手順を実行します。</span>

      1. 「**条件を追加**」をクリックします。
      1. <span class="preview">**If** 行で、フィールドを選択してフィールド値を選択し、「**条件を追加**」をクリックします。</span>

         >[!TIP]
         >
         ><span class="preview"> 条件付き書式には、リスト表示に表示されているフィールドのみ使用できます。</span>

      1. <span class="preview"> （任意） **If** 行の **条件の追加** をクリックして、同じルールにさらに条件を追加します。</span>

         >[!TIP]
         >
         ><span class="preview"> 条件付きルールには最大 10 個の条件を追加でき、1 つのフィールドに最大 20 個のルールを持つことができます。</span>

      <div class="preview">

      1. 条件間の **Or** コネクタをクリックして **And** に変更し、複数の条件を同時に満たす必要があることを示します。 **または** がデフォルトのコネクタです。
      1. **フォーマット** 行で、フォーマットする列を示すフィールドを選択します。<!--edit this area, if it changes names??-->
      1. （オプション）選択したフィールドの横にある **色円** アイコン ![&#x200B; 色円アイコン &#x200B;](assets/color-circle.png) をクリックして展開し、別の色を選択します。
      1. **行に適用** 設定をオンにして、条件を満たすフィールドの行全体に書式を適用します。
      1. （省略可能） [**書式**] ボックスの [**条件の追加**] をクリックして、別のフィールドに別の動作規則を追加し、上記の手順を繰り返します。
      1. （オプション）「**すべてクリア**」をクリックすると、すべての書式設定が削除されます。
      1. **フォーマット** ボックスの外側をクリックして閉じます。

         これにより、リスト表示に戻ります。
書式設定はリスト表示に直ちに適用されます。
**セルの書式設定** アイコンの横には、ビューに特別な書式が適用されていることを示す青い点があります。
