...
1. （オプション）次のいずれかの操作を行って、リスト表示を変更します。
   ...
   1. 既存のビューの名前にポインタを合わせて **詳細** メニュー ![ 詳細メニュー ](assets/more-menu.png) をクリックし、次のいずれかをクリックします。
      ...
      * **削除**：ビューを削除します。
      ...
      >* システム・ビューは変更できません。
      >
      >* <span class="preview"> 変更した後に共有されていたビューをリセットして元の環境設定に戻したり、変更内容をコピーしてコピーを共有したりできます。 詳しくは、[ 拡張リストの使用 ](/help/quicksilver/workfront-basics/navigate-workfront/use-lists/enhanced-lists.md) を参照してください。</span>
...
リスト表示で、最初の列に表示されるプライマリフィールドは削除できません。
   ...
   1. <span class="preview"> 「セルの書式設定 **」アイコン** 「セルの書式設定 ![ アイコン ](assets/format-cells-icon.png) をクリックします。 **フォーマット** ボックスが開きます。</span> <!--change the name of the box when they update it-->
      ...
      1. **If** 行で「**条件を追加**」をクリックし、書式設定するフィールドを選択して、フィールド値を選択します。

         >[!TIP]
         >
         >リスト表示に表示されているフィールドのみが、条件付き書式に使用できます。

      1. （任意） **If** 行の **条件の追加** をクリックして、同じルールにさらに条件を追加します。

         >[!TIP]
         >
         >条件付きルールには最大 10 個の条件を追加でき、1 つのフィールドに最大 20 個のルールを設定できます。
