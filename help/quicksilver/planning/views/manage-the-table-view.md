...
1. 列ヘッダー上にポインタを合わせ、下向き矢印をクリックして、「**フィールドを非表示**」をクリックします。
   ...
   >
   ><span class="preview"> デフォルトでは、非表示のフィールドはレコードの **詳細** ページに表示されません。 詳しくは、[ レコードページレイアウトの管理 ](/help/quicksilver/planning/records/manage-the-record-page.md)</span> を参照してください
