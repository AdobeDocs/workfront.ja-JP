...
1. （条件付き）管理者がカスタム四半期を有効にしており、Workfrontがカスタム四半期の設定方法に関する問題を検出した場合、タイムラインビューを開くと警告が表示される場合があります。
   ...
   * 四半期の日付の間にギャップや重複が検出された場合、カスタム四半期が設定可能になり、編集が必要になる可能性があるという通知が届く場合があります。
     ...
     >[!TIP]
     >
     >このメッセージは、組織が Planning を購入した直後に表示され、購入前にカスタム四半期が既に有効になっています。 組織でWorkfront Planning を有効にした後は、四半期の重複やギャップは使用できません。
...
1. <span class="preview"> （オプションおよび条件付き）グループ化の基準として選択したフィールドタイプに応じて、次の並べ替えオプションのいずれかを選択して、昇順または降順でグループを並べ替えます。</span>

   <div class="preview">

   * **A -> Z** または **Z -> A** 選択フィールド、テキストフィールド、段落フィールド、接続されたレコードまたはオブジェクト、人物フィールド
   * **0 -> 9** または **9 -> 0** （数値、通貨、割合のフィールド）
   * 「日付」フィールドについて **最も早い** 最新の **最新の** または

   </div>

   <span class="preview"> タイムライン表示のグループ化は、それに応じて並べ替えられます。 グループ化内のレコードは、デフォルトの並べ替えである **開始日** の順序で並べ替えられます。</span>
