...
# スタンドアロン製品としてのAdobe Workfront Planningに必要なアクセス
...
>
>Workfront Workflow パッケージと一緒に購入した場合のAdobe Workfront Planningについて詳しくは、[Adobe Workfront Planningの基本を学ぶ](/help/quicksilver/planning/general/planning-overview.md)を参照してください。

ここでは、スタンドアロン製品としてのAdobe Workfront Planningのライセンス、アクセスレベル、およびユーザー機能について説明します。

## Workfront計画パッケージ

お客様は、次のスタンドアロンのWorkfront計画パッケージのいずれかを購入できます。

* Prime
* 選択
* Ultimate

次の表に、各パッケージに含まれる内容を示します。

| プランニング機能 | 選択 | Prime | Ultimate |
|---|---|---|---|
| プランニング、オーケストレーション、キャンペーン管理 | ✓ | ✓ | ✓ |
| 無制限のワークスペース | ✓ | ✓ | ✓ |
| ワークスペースあたりのレコード数 | 25,000 | 500,000 | 1,000,000 |
| 合計レコード数（すべてのワークスペース） | 500,000 | 2,000,000 | 制限なし |
| グローバルレコードタイプとワークスペース間の接続 | — | ✓ | ✓ |
| リリース時の今後の機能へのアクセス | 一部 | 一部 | Most |

## Workfront計画パッケージの可用性
...
プランニングライセンスは、ユーザーに割り当てられたアクセスレベルの一部です。

スタンドアロン製品としてPlanningを使用する場合、ライセンスをアクセスレベルに割り当てることはできません。ユーザーに割り当てられたアクセスレベルに含めるように既にハードコードされています。

お客様が購入するWorkfront計画パッケージの組み合わせに応じて、お客様のユーザーに対して次のいずれかのアクセスレベルが設定されている場合があります。

* **Planning Administrator**: Workfront Planning パッケージで使用できます。 ユーザーがAdmin Consoleに追加されると、計画管理者は自動的に割り当てられます。
* **Planning Standard**：すべてのWorkfront Planning パッケージで使用できます。 このアクセスレベルは、作成時にユーザーに割り当てることができます。
...
各スタンドアロンのPlanning ライセンスは、システム内の役割にマッピングされ、製品のどの領域にアクセスできるかを制御します。

次の表に、スタンドアロン製品として購入した場合のWorkfront Planningの各ライセンスの種類とその機能を示します。

| 機能/ライセンスタイプ | 計画管理者 | 計画標準 |
|---|---|---|
| アクセスレベルの説明 | 完全なシステムアクセス | ワークスペースとコンテンツを管理し |
| メインメニューのプランニングエリア | ✔ | ✔ |
| メインメニューのユーザーエリア | ✔ | 表示のみ |
| メインメニューのリクエスト領域 | ✔ | ✔ |
| メインメニューの設定領域 | ✔ |   |
| ワークスペースとそのコンテンツの管理 | ✔ | ✔ |
| プランニングデータをチームと共有する | ✔ | ✔ |
...
ユーザーのアクセスレベルの割り当てについて詳しくは、[Adobe Workfront Planningのユーザーの管理をスタンドアロン製品](/help/quicksilver/planning/planning-sta/manage-users-in-planning-sta.md)を参照してください。

## ナビゲーションエリアとアクセスレベルの依存関係

Planning ライセンスによっては、グローバルナビゲーションで使用できる領域が異なる場合があります。
...
Planning管理者のアクセス・レベルを持つユーザーには、次の機能があります。

* システムへの完全な管理アクセス権があります。
* ユーザーの作成と編集、チームの管理、カスタム四半期の設定、すべての設定サブページへのアクセスが可能です。
...
Planning Standardのアクセス・レベルを持つユーザーには、次の機能があります。
...
## スタンドアロン製品としてのPlanningのアクセス・レベルの設定

アクセス・レベルは、スタンドアロン製品としてPlanningを購入する際に組み込まれたもので、Planning ユーザー用に設定することはできません。

Planning管理者としてユーザーにアクセス・レベルを割り当てるには、次の手順に従います。

* Adobe Consoleでユーザーを作成します。
...
Workfront Planningでは、次のオブジェクトをスタンドアロン製品として共有できます。
