...
>[!IMPORTANT]
>
> この記事に記載されている情報は、Adobe Unified Experienceにまだオンボーディングされていない組織にのみ適用されます。
> 組織がAdobe Unified Experienceにオンボーディングされている場合、日付の環境設定は、Adobe Unified Shellの言語設定によって制御されます。 既定の言語設定、つまり既定の日付設定は`en-US`です。
