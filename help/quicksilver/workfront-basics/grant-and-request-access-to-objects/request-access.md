...
Workfront 内の特定のオブジェクトに権限が必要な場合は、そのオブジェクトへのアクセスをリクエストできます。 Workfrontの管理者またはオブジェクトの所有者にメールを送信してニーズを説明するのではなく、Workfront内で追加のアクセス権（または権限）をリクエストできます。

誰かがオブジェクトへのリンクを共有した場合は、オブジェクトへの初期アクセスをリクエストできます。また、既に表示権限を持っているオブジェクトへの追加アクセスをリクエストすることもできます。 例えば、プロジェクトに表示権限がある場合は、タスクをプロジェクトに追加する必要があります。 この場合、プロジェクトに投稿権限をリクエストできます。
...
次の標準的な共有ルールは、Workfront システムのデフォルトのオプションであり、自動的に有効になります。
...
オブジェクトへのアクセス権をまだ持っておらず、リンクからそのオブジェクトに移動すると、情報を表示するためのアクセス権がないことを知らせる画面が表示されます。

オブジェクトへの初期アクセスをリクエストするには：

1. 「**アクセスの要求**」をクリックします。 **アクセスを要求** ダイアログボックスが表示されます。

1. （条件付き）複数のユーザーが適切なアクセス権を持ち、追加のアクセス権を付与できる場合、ユーザー名の横にドロップダウン矢印が表示されます。 ドロップダウンリストからアクセスリクエストを受け取るユーザーを選択します。

   ドロップダウンリストには10人のユーザーのみが表示され、アルファベット順に並べ替えられます。 このドロップダウンメニューに表示されるユーザーの順序について詳しくは、[「アクセスを要求」と「さらにアクセスを要求する」のドロップダウンメニューの階層](#hierarchy-of-the-request-access-and-request-more-access-drop-down-menus)を参照してください。
...
1. （条件付き）複数のユーザーが適切なアクセス権を持ち、追加のアクセス権を付与できる場合、ユーザー名の横にドロップダウン矢印が表示されます。 ドロップダウンリストからアクセスリクエストを受け取るユーザーを選択します。
   ...
   ドロップダウンリストには10人のユーザーのみが表示され、アルファベット順に並べ替えられます。 このドロップダウンメニューに表示されるユーザーの順序について詳しくは、[「アクセスを要求」および「さらにアクセスを要求する」ドロップダウンメニューの階層](#hierarchy-of-the-request-access-and-request-more-access-drop-down-menus)を参照してください。
...
## 「アクセスを要求」および「詳細アクセスを要求」ドロップダウンメニューの階層 {#hierarchy-of-the-request-access-and-request-more-access-drop-down-menus}
...
オブジェクトに対する「アクセスをリクエスト」または「その他のアクセスをリクエスト」リストを入力する際、Workfrontは、オブジェクトにアクセス権を付与できる様々なロールを満たす最大10人のユーザーのリストを選択します。 作成されたリストは、アルファベット順に並べ替えられます。

「アクセスをリクエスト」または「その他のアクセスをリクエスト」ドロップダウンのユーザーの順序は、次のルールによって決まります。

* リストの最初のユーザーは、オブジェクト「所有者」です。詳しくは、[オブジェクトの所有者について](#understand-the-owner-of-an-object)を参照してください。
* その後、オブジェクトが個別に共有されるユーザーがリストに入力されます。 これらのユーザーはアルファベット順にリストされます。
* チーム、グループ、企業との共有を通じて必要なアクセス権を取得したユーザーが、リストにさらに追加されます。 これらのユーザーはアルファベット順にリストされます。
