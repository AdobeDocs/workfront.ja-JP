...
Workfront管理者が、アクセスレベルの特定の種類のオブジェクトへのアクセスを設定します。 詳しくは、 [アクセスレベルと権限の連携](../../administration-and-setup/add-users/access-levels-and-object-permissions/how-access-levels-permissions-work-together.md).
...
* 承認者として割り当てられたユーザーは、承認を待機しているオブジェクトに対して View アクセス権を持ちます。
...
オブジェクトに対するアクセスレベル権限を持っていない状態で、リンクからそのオブジェクトにアクセスしようとすると、Workfront管理者に問い合わせるよう通知する画面が表示されます。 
...
1. 次をクリック： **詳細** メニューをプロジェクト名でインライン表示し、「 **さらにアクセスをリクエスト**.\
...
* [アクセスを申請ドロップダウンメニューおよびアクセスをさらに申請ドロップダウンメニューに表示されるユーザーの階層を理解する](#understand-the-hierarchy-of-users-listed-in-the-request-access-and-request-more-access-drop-down-menus)
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>問題</td> 
   <td> <p>所有者は、問題のプライマリ連絡先、または問題が見つからない場合や必要なアクセス権がない場合は、上述のように、問題が存在するプロジェクトの所有者です。 </p> <p>問題の作成者と同じ人物ではない可能性があります。 </p> </td> 
  </tr> 
  <tr> 
   <td>ポートフォリオ</td> 
  ...
  <tr> 
   <td>ドキュメント</td> 
   <td>所有者は、ドキュメントの所有者（ドキュメントをアップロードしたユーザー）、またはドキュメントが見つからない場合や必要なアクセス権がない場合はドキュメントが存在するオブジェクトの所有者です。</td> 
  </tr> 
  <tr> 
   <td>レポートとダッシュボード</td> 
  ...
  <tr data-mc-conditions="QuicksilverOrClassic.Quicksilver"> 
   <td>目標</td> 
   <td> <p>所有者は、所有者に指定されたユーザーです。 彼らは目標の創造者と同じ人物ではないかもしれない。 </p> <p>追加のライセンスが必要です。 </p> <p>Workfront目標について詳しくは、 <a href="../../workfront-goals/goal-management/wf-goals-overview.md" class="MCXref xref">Adobe Workfront目標の概要</a>. </p> </td> 
  </tr> 
 </tbody> 
</table>
