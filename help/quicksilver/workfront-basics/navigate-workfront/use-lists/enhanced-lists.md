...
Adobe Workfrontの一部の領域では、拡張リストを利用できます。 これらのリストは、リスト項目を表示するために表形式を使用し、標準のリストとは異なるルックアンドフィールを持ちます。
...
* リストの下部にある「**新しい行**」をクリックします。 このオプションを選択すると、テーブルに新しい行が追加され、各セルに情報が入力されます。 セルが特定のデータセット（通貨名など）に限定されている場合は、リストから選択するか、名前を検索するかを確認するメッセージが表示される場合があります。
...
アクションバーのボタンをクリックして、リスト項目の表示、項目の削除、デフォルト項目にするなどのアクションを実行します。

選択した項目で使用できるアクションがない場合、アクションバーには「使用できるアクションがありません」と表示されます。

![ アクションバーの例 ](assets/glist-action-bar-exchange-rates.png)

**詳細** メニューは、リスト項目にカーソルを合わせたときにリスト項目の横に表示される 3 ドットメニューです。 メニューをクリックして、項目の追加のアクションを表示します。 一部のアクションは、そのリストに固有で、他のリストには表示されない場合があります。
...
1. 列を並べ替えるには、「**ドラッグ**」アイコンをクリックし、列を目的の場所に移動します。 列を移動すると、リストは自動的に変更されます。
   ...
   >[!NOTE]
   >
   >フィールドは、常に最初の位置に表示されるか固定される場合があり、列を変更することはできません。
...
1. フィルターを適用したら、「**フィルター**」オプションを再度開いて、フィルターオプションを変更したり、すべてのフィルターをクリアしたりできます。
   ...
   <span class="preview"> フィルターがリストに適用されると、インジケーターが **フィルター** ボタンに表示されます。</span>
...
>[!NOTE]
>
>リストにデフォルトの項目がある場合、リストの並べ替え方法にかかわらず、その項目は常にリストの先頭になります。
