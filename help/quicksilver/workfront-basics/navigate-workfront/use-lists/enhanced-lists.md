...
1. リストの下部にある&#x200B;**新しい行**&#x200B;をクリックします。 このオプションを選択すると、テーブルに新しい行が追加されます。 セル内をダブルクリックして、そのセルに情報を入力します。 各セルは、リスト項目のフィールドを表します。 フィールドは、リストに表示される前に存在する必要があります。
   ...
   * 1行テキスト：英数字の情報を入力できます。
   * 数値：情報は数値形式で入力する必要があります。
   * 通貨：通貨記号の前に番号形式で情報を入力する必要があります。
   * 日付：カレンダーを使用して日付を選択します。
   * 日付と時刻：カレンダーを使用して、日付と時刻を選択します。
   * 単一選択または複数選択ドロップダウン：ドロップダウンメニューから1つまたは複数のオプションを選択します。 リストでサポートされている場合は、結果が見つからないときに、ドロップダウンから新しいオプションを直接作成できます。
   * 先行入力：既存のオブジェクトの名前（グループまたはプロジェクトの名前など）を入力し、候補のリストから名前を選択します。
   * 段落：複数の英数字を入力できます。
   * 担当者（1人または複数）: リスト項目の担当者として1人以上のユーザーを選択します。
   * カラーピッカー：ピッカーからカラーを選択するか、16進数やRGBなどのコードを入力します。

   「担当者」、「単一選択」 ドロップダウンおよび「複数選択」ドロップダウンフィールドは、次のキーボードコマンドをサポートしています。

   * キーボードの上向き矢印と下向き矢印を使用して、アイテムのリスト内を移動します。
   * スペースバーを押して、項目を選択します。
   * Delete キーを押して、選択した項目を削除します。

   >[!NOTE]
   >
   >フィールドタイプごとに編集オプションがあります。 一部のフィールドは読み取り専用の場合があります。
   >
   >すべてのリストがすべてのフィールドタイプをサポートしているわけではないことに注意してください。
...
1. 列を並べ替えるには、**ドラッグ** アイコン ![&#x200B; ドラッグ アイコン &#x200B;](assets/drag-icon.png)をクリックし、列を目的の場所に移動します。 列を移動すると、リストが自動的に変更されます。
   ...
   一部のリストでは、列自体を好みの順序にドラッグ&amp;ドロップすることもできます。
...
1. <span class="preview"> （オプション）列を並べ替えるには、**Selected** セクションの列名にカーソルを合わせ、**ドラッグ** アイコン ![&#x200B; ドラッグ アイコン &#x200B;](assets/drag-icon.png)をクリックします。 列を垂直方向に移動して、列を順番に配置します。</span>
1. 「**保存**」をクリックします。
...
1. 行を好みの順序にドラッグ&amp;ドロップします。

   >[!NOTE]
   >
   >ソートが適用されている場合、行の順序を手動で変更することはできません。
