...
1. 「**プロジェクトの健全性**」セクションに含める要素を選択し、プロジェクトの健全性の基準を決定する際に含めない要素を選択解除します。
   ...
   * **必須フィールド**：必須フィールドがない場合（プロジェクトの説明など）。 これらの必須フィールドは、プロジェクトの完全性を決定します。**完全性を確認するフィールドはどれですか？**&#x200B;で指定します 以下の設定セクションを参照してください。
...
1. **では、プロジェクトの作業範囲をどのように見積もりますか？** セクションで、プロジェクトスコープの拡大に伴い増加するプロジェクト要因を選択します。
