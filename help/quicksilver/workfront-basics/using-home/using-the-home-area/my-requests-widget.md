...
1. （オプション）情報がリクエストリストに表示される方法を管理するには、リストの次のビュー要素を作成または更新します。
   ...
   * 行の高さ
   ...
   リクエストリストのビュー要素の更新について詳しくは、[拡張リストの使用](/help/quicksilver/workfront-basics/navigate-workfront/use-lists/enhanced-lists.md)を参照してください。
...
>[!NOTE]
>
>次のオブジェクトは、リクエスト領域で新しいリクエストエクスペリエンスを有効にすると、マイリクエストウィジェットのリクエストリストからリンクされます。
>
>* 「件名」フィールドでリクエストを計画およびWorkfrontします。
>* 「オブジェクト作成」フィールドのPlanning リクエストから作成されたPlanning レコード。
>* <span class="preview">Workfrontのタスクとイシューは、プレビュー環境の「オブジェクトを作成」フィールドでWorkfront リクエストから変換されました。</span>
...
1. そのリクエストの&#x200B;**作成済みオブジェクト**&#x200B;列のオブジェクト名をクリックします。
   ...
   >[!TIP]
   >
   >実稼動環境では、**作成済みオブジェクト** フィールドにリンクが含まれているのはプランニングレコードのみです。
   >   
   ><span class="preview"> プレビュー環境では、イシューから変換されたWorkfront タスクとプロジェクト、およびPlanning リクエストから作成されたプランニングレコードには、**作成済みオブジェクト** フィールドにリンクが含まれています。</span>
   >
