...
| 一括承認の参加者を追加および削除する | `approvals_bulk_update_approval_participants`<br>`approvals__submit_bulk_update_approval_participants` | 同時に複数の承認に参加者を追加したり、複数の承認から参加者を削除したりします。 現在、一括更新は1つのプロジェクトにのみ適用できます。 近い将来、複数のプロジェクトで一括アップデートが利用可能になる予定です。 | 編集 |
