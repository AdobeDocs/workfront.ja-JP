...
>[!IMPORTANT]
>
>現在、Workfront MCP サーバーは、AWSを使用しているお客様のみが利用できます。 GCPまたはAzureを使用しているお客様は、近い将来、Workfront MCP機能を使用できるようになります。
...
>[!IMPORTANT]
>
>現在、Claude Connectorは、米国リージョンのWorkfront MCP サーバーへの接続のみをサポートしています。  EU リージョンのWorkfront インスタンスに接続するには、この記事の「[URLを使用してClaudeに接続](#connect-to-claude-with-a-url)」を参照してください。
