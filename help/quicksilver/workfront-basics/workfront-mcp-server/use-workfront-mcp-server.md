...
### インサイトデータは、即座にではなく、ほぼリアルタイムで取得されます

Workfrontのデータを検索、フィルタリング、レポートするために使用されるインサイトツールは、service level agreement（SLA）で約15分の変化をほぼリアルタイムで反映します。 Workfrontで変更を加えてすぐに確認した場合、その変更が反映されない場合があります。

最新の変更を確認する必要がある場合は、数分待ってもう一度確認するか、Workfrontで直接確認してください。
...
| Workfrontで変更したデータはまだ表示されません。 | インサイトデータはほぼリアルタイムで、SLAの所要時間は約15分です。 | 数分待ってから、もう一度尋ねるか、Workfrontで直接確認してください。 |
...
特に削除の場合は、アクションを実行する前に確認するように促します。
リクエストを承認する前に、プラットフォームがこれから行うことをお読みください
...
Workfrontに、プロンプトやAI エージェンティックプラットフォームの応答が保存されることはありません。
AI エージェント型プラットフォームを提供する誰もが、会話の流れを制御できます
