...
>[!IMPORTANT]
>
>左側のパネルにデフォルトで 1 つのセクション（**[!UICONTROL プロジェクト]** エリアの **[!UICONTROL すべてのプロジェクト]** など）のみが表示される場合、[!DNL Workfront] 管理者はレイアウトテンプレートを作成する際にこのエリアに少なくとも 1 つのダッシュボードを追加し、そのテンプレートにユーザーを割り当ててから、左側のパネルがそのエリアに表示されます。
...
<table style="table-layout:auto">
    ...
    <tr>
        ...
        <td>レイアウトテンプレートの [!UICONTROL プロジェクト ] 領域の左側のパネルに、[!DNL Workfront] 管理者が追加したすべてのダッシュボードを表示できます。 これらのダッシュボードの名前は、お使いの環境用にカスタマイズされています。</td>
    </tr>
</table>
...
1. （条件付き）オブジェクトのリストを含むエリアを選択した場合（**[!UICONTROL プロジェクト]**&#x200B;または&#x200B;**[!UICONTROL ポートフォリオ]**&#x200B;など）、左側のパネルナビゲーションにアクセスするには、次の操作を行う必要があります。
   ...
   1. （オプション）左側のパネルナビゲーションでは、左側のパネルでプロジェクト内の別のオブジェクト（**[!UICONTROL タスク]** や **[!UICONTROL イシュー]** を選択してオブジェクトのリストにアクセスし、特定のオブジェクトの名前をクリックすることができます。
...
1. （オプション）ダッシュボードのクイックリンクを追加するには、左パネルの下部にある **[!UICONTROL ダッシュボードを追加]** をクリックしてから、次の手順を実行します。

   1. ダッシュボードの名前を&#x200B;**[!UICONTROL クイックリンク名]**&#x200B;フィールドに入力します。
   1. 「**[!UICONTROL ダッシュボードを選択]**」フィールドに既存のダッシュボードの名前の入力を開始し、リストに表示されたらダッシュボードをクリックします。

      >[!TIP]
      >
      >ダッシュボードがリストに表示される前に、ダッシュボードを作成する必要があります。

1. （オプション）ダッシュボードのクイックリンクを削除するには、左側のナビゲーションのダッシュボードにマウスポインターを置き、表示された **削除** アイコン ![ 削除アイコン ](assets/remove-dashboard-from-left.png) をクリックします。
