...
「[!UICONTROL チームリクエスト]」タブには、ドロップダウンリストで現在選択されているチームの割り当て待ちのリクエストが表示されます。 括弧内の数字は、作業の準備ができているアイテムの数を示します。 「[!UICONTROL  チームリクエスト ]」タブには、1つのチームに対する最大2,000件のリクエストを表示できます。
