---
name: release-notes-formatter
description: Workfront リリースノートの書式と検証を行って、一貫性、正しい構造、適切なリンクを確認します。 製品リリースディレクトリ内のリリースノートファイル、またはユーザーがリリースノート、製品リリース、または四半期リリースについて言及した場合にのみ使用します。 ハウツー記事や一般的なドキュメントには適用されません。
source-git-commit: dac869369d6d9ef32741aa0972ccf9cb25b2633c
workflow-type: tm+mt
source-wordcount: '2183'
ht-degree: 2%

---


# リリースノート形式

`help/quicksilver/product-announcements/product-releases/` ディレクトリのAdobe Workfront リリースノートをフォーマットして検証します。

## ページタイプ

ファイルパスとコンテンツからページタイプを特定します。

| ページ タイプ | ファイルパターン | テンプレート |
|-----------|-------------|----------|
| **概要** | `{YY}-q{N}-release-overview.md` | .claude/commands/_release-notes-formatter-reference.mdを参照してください#overview-page-template |
| **製品エリア** | `{YY}-q{N}-{area}.md` | .claude/commands/_release-notes-formatter-reference.mdを参照してください#product-area-page-template |
| **計画中** | `planning-release-activity-{YY}-q{N}.md` | 商品エリアと同様 |
| **ルックアンドフィール** | `look-and-feel-updates-{YY}-q{N}.md` | .claude/commands/_release-notes-formatter-reference.mdを参照してください#look-and-feel-page-template |

## 手順0：四半期を決定します（他の何よりも前にこれを行います）

>[!IMPORTANT]
>
>プレビュー日または実稼動日にカレンダー四半期の計算を使用して、機能をDOC-quarterに割り当てないようにします。 Doc-quarterは、Workfrontの内部リリースカレンダーのグループ化に従って、この機能が付属する&#x200B;**月リリース**&#x200B;の基準です。これはカレンダー四半期からオフセットされます。このファイルの末尾にある[2026 リリースカレンダー](#2026-release-calendar) テーブルを参照してください。 例えば、2026年8月13日の実稼働日を持つ機能は、8月の月間リリースが`26-q4`にマッピングされるため、`26-q3`ではなくドキュメント四半期`26-q4`に属します。
>
>さらに下の「四半期マッピング」テーブル（手書きフォーム / 月）は、タイトルに四半期名を書き出す場合（例：第3四半期）です。機能がどの四半期のファイルに属するかを決定するには、**個だけでは**&#x200B;できません。 ファイルを作成または編集する前に、必ずリリース カレンダーのテーブルを確認してください。
>
>機能の実稼働日がリリースカレンダーテーブルに表示されない場合（例えば、テーブルの日付範囲を超えている場合）、ユーザーに推測ではなく更新されたカレンダーを求めます。

## 書式設定ワークフロー

### 手順1:Frontmatterの検証

すべてのリリースノートページの必須フィールド：

```yaml
---
title: <descriptive title>
description: <matches or summarizes the title>
author: <author name>
feature: Product Announcements
recommendations: noDisplay, noCatalog
exl-id: <existing UUID — never generate or change>
---
```

ルール：
- `feature`は正確に`Product Announcements`でなければなりません
- `recommendations`は正確に`noDisplay, noCatalog`でなければなりません
- `exl-id`を発明しない – 既に存在する場合にのみ含める
- 実際のページに`draft: Probably`を追加しない（テンプレートのみ）

### 手順2：ページタイプ別の構造の検証

#### 製品エリアページ

1. **H1**: `{Written Quarter} {Area} enhancements`
   - 例：`# Second Quarter 2026 Administrator enhancements`
   - 四半期には、「第1四半期」、「第2四半期」、「第3四半期」、「第4四半期」と記載する必要があります

2. **イントロパラグラフ**：概要のエリアとリンクを説明します
   - **四半期の**&#x200B;概要ファイルにリンクする必要があります
   - 一般的なバグ：前の四半期へのリンク（例：`26-q2`ではなく`26-q1`）

3. 機能ごとの&#x200B;**H2**：見出しとしての機能タイトル
   - **最新の機能が最初に** – 最新のリリースノートは、イントロパラグラフの後の最初のH2として表示される必要があります
   - 古い機能は時系列が逆になります

4. **各H2の後の日付コールアウトブロック**:

```markdown
>[!NOTE]
>
>Preview: {Month Day, Year}
>Production fast release: {Month Day, Year}
>Production for everyone: {Month Day, Year}
```

5. **Body**：機能の説明、ヘルプドキュメントへのリンク

#### 概要ページ

1. **H1**: `{Written Quarter} release overview`

2. 予定公開月を含む&#x200B;**イントロパラグラフ**

3. リリーススケジュール表を含む&#x200B;**`>[!IMPORTANT]`ブロック**

4. アンカーリンクの箇条書きリストを含む&#x200B;**H2`Adobe Workfront enhancements`**:

```markdown
* [Administrator enhancements](#administrator-enhancements)
* [Document enhancements](#document-enhancements)
```

5. HTML機能テーブルを含む製品領域&#x200B;**ごとの** H3 （。claude/commands/_release-notes-formatter-reference.md#overview-feature-tableを参照）
   - 各テーブル内では、**最新の機能が最初に** – 最新の行がテーブルの上部（ヘッダー行の後）に表示されます

6. **後続セクション** （H2）：その他の領域のリリースノート、デスクトップ校正ビューアの更新、お知らせ、API バージョン、メンテナンスの更新、トレーニングの更新

### 手順3：リンクの検証

- **製品領域ページの概要リンク**：同じ四半期を指す必要があります
  - 正解：`26-q2-release-activity/26-q2-release-overview.md`
  - 間違っています：`26-q1-release-activity/26-q1-release-overview.md`
- **概要**&#x200B;のアンカーリンク：H3 ID （小文字、ハイフン）と一致する必要があります
- **概要テーブルの機能リンク**: `class="MCXref xref" xrefformat="{para}"`を使用する必要があります
- **ヘルプドキュメントのリンク**: `/help/quicksilver/`で始める必要があります

### 手順4：日付の検証

- 形式：`{Month} {Day}, {Year}` （例：「2026年3月12日」）
- 不明な日付に`TBD`を使用
- 製品領域ページ `>[!NOTE]` ブロックの日付は、対応する概要テーブル行と一致する必要があります
- プレビュー日は生産日より前にする必要があります

### 手順5：一般的な修正

書式設定時に次の修正を適用します。

| イシュー | 修正 |
|-------|-----|
| 不適切な概要リンク四半期 | ファイルの独自の四半期に一致するように更新 |
| `>[!NOTE]`日付ブロックがありません | H2機能見出しの後にブロックを追加 |
| 日付形式に一貫性がない | `Month Day, Year`に標準化 |
| `>[!NOTE]`の前に空白行がありません | 空白行を追加 |
| 吹き出し線内の余分なスペース | 末尾の空白をトリミング |
| HTMLの商品エリアページ | マークダウンとして保持（HTMLは概要テーブル専用） |
| `exl-id`がありません | そのままにしておきます。生成しないでください |

### 手順6：概要ページの同期

製品領域ページに&#x200B;**新機能**&#x200B;を追加するたびに、同じ変更でその四半期の`{YY}-q{N}-release-overview.md`の一致する行を追加または更新します。 製品領域ページにのみ存在し、概要テーブルには存在しない機能は、リリース概要インデックスには表示されません。

- その製品領域（例：`### Reporting enhancements`）のH3 セクションを見つけ、既存の行形式に一致する新しい`<tr>`行をテーブルの&#x200B;**top**&#x200B;に追加します（ヘッダー行の後）（。claude/commands/_release-notes-formatter-reference.md#overview-feature-tableを参照）。
- この行の日付は、その機能の製品領域ページの`>[!NOTE]` ブロックと一致する必要があります（手順4）。
- 機能が別の製品領域に再分類された場合（例：レポートから管理者に移動された場合）、その行を新しい領域のH3 セクションに移動します。古いコピーを古い領域に残さないでください。
- Planningのみの機能は概要テーブルには追加されません。Planningには独自のリリースアクティビティページがあり、「その他の領域のリリースノート」の下に1回リンクされています（機能ごとの行は必要ありません）。

機能に既に行があり、その内容/日付が変更されていない場合は、概要ページに触れないでください。

回避すべき主な間違い：

- 一致する行を概要テーブルに追加せずに、機能のH2を製品領域ページに追加します。
- コンテンツを別の領域のページに移動した後、古い製品領域セクションに古い概要行を残す。
- 製品領域ページの`>[!NOTE]` ブロックと一致しない概要行の日付。

### 手順7：目次の更新

**new** リリースノート ページ （概要または製品領域）を作成するたびに、同じ変更で`help/quicksilver/TOC.md`に追加します。 目次にないページは、概要テーブル内のリンクがそのページを指している場合でも、公開されたナビゲーションには表示されません。

追加先：

- 目次には、四半期ごとに`* 2026 Q3 Release {#release-26-q3}`のような見出しの下にセクションがあります。 四半期の見出しがまだ存在しない場合（新しい四半期の最初のページ）、前の四半期の上に追加して、新しい四半期が上に表示されるようにします。
- その四半期の見出しの下に、次の順序でページをリストします。
  1. **概要**&#x200B;最初（`Third Quarter 2026 release overview`）。
  2. **製品領域ページ**&#x200B;のアルファベット順の地域名（管理者、文書、エンタープライズ操作、プロジェクト、レポート、リクエスト）。
  3. **その他の機能強化**&#x200B;は最後に（常にアルファベット順の製品領域の後に）。

各目次エントリは、ページタイトルと絶対リポジトパスを使用したマークダウンリンクです。

```markdown
      * [Third Quarter 2026 Documents enhancements](/help/quicksilver/product-announcements/product-releases/26-q3-release-activity/26-q3-documents.md)
```

周囲のエントリにインデント（6つのスペース）を一致させます。 ページ H1のverbatimをリンクテキストとして使用します（例：`Documents enhancements`、`Requesting enhancements` （`Requests`ではなく））。したがって、目次ラベルは前四半期と一致します。

回避すべき主な間違い：

- 目次に追加せずに製品領域ページを作成します。
- 新しい製品領域ページから別の四半期の概要にリンクする（手順3）。
- 前四半期の見出しに新しい四半期のページを挿入します。

### 手順8：ホームページの更新

**新しい四半期の概要ページ**&#x200B;を作成する場合（つまり、これは新しい四半期の最初のページで、既存の四半期に追加された新しい製品領域ページだけでなく）、同じ変更で`help/quicksilver/home.md`を更新します。

- `>[!TAB Latest release]` セクションで、リリースの概要リンクを新しい四半期の概要リンクに置き換えます。
- このセクションでも、新しい四半期の計画ファイル （`planning-release-activity-{YY}-q{N}.md`）が存在する場合は、Adobe Workfront計画リリースアクティビティリンクを更新して、その計画ファイルを指すようにします。
- 現在の年の`>[!TAB {YYYY} releases]` タブで、新しい四半期の概要リンクをリストの上部にある、前四半期のエントリの上に追加します。

既に概要ページが表示されている四半期に製品領域ページのみを追加する場合は、`home.md`をタッチしないでください。

回避すべき主な間違い：

- `home.md`の「最新リリース」タブを更新せずに、新しい四半期の概要ページを作成します（古い四半期を指し続けます）。
- また、新しい四半期を現在のタブ リストに追加するのを忘れています。

&lt;&lt;&lt;&lt;&lt;&lt;&lt; アップストリームを更新
### 手順8：製品リリースインデックスページの更新
=======
### 手順9：製品リリースインデックスページの更新
>>>>>>>>>>スタッシュされた変更
> 
**新四半期の概要ページ**&#x200B;を作成するたびに、同じ変更で`help/quicksilver/product-announcements/product-releases/product-releases.md`も更新します。

- 「Workfront リリース」列で、現在の年の`<p>Releases in {year}</p>` ブロックを検索します。
- その年の&#x200B;**top**&#x200B;に、新しい四半期の概要ページにリンクする新しい`<li>`を、既存のエントリと同じ形式で追加します。

  ```html
  <li><a href="/help/quicksilver/product-announcements/product-releases/26-q4-release-activity/26-q4-release-overview.md" class="MCXref xref" xrefformat="{para}">Fourth Quarter 2026 release overview</a></li>
  ```

- 新しい四半期のPlanning リリースアクティビティページ （`planning-release-activity-{YY}-q{N}.md`）が存在する場合は、同じ行の「その他の製品リリース」列の先頭に一致する`<li>`も追加します。
- 現在の年にまだ行がない場合（新しい年の最初の四半期）、既存の行構造に従って、前年の行の上に新しい`<tr data-mc-conditions="">`を追加します。

既に概要ページが表示されている四半期に製品領域ページのみを追加する場合は、`product-releases.md`をタッチしないでください。

回避すべき主な間違い：

- `product-releases.md`に追加せずに新しい四半期の概要ページを作成します（このページは前四半期のみに表示されます）。
- 概要リンクは追加されますが、対応するPlanningのリリース活動リンクは忘れられます。

## ファイル命名規則

| タイプ | パターン | 例 |
|------|---------|---------|
| 概要 | `{YY}-q{N}-release-overview.md` | `26-q2-release-overview.md` |
| 製品エリア | `{YY}-q{N}-{area-slug}.md` | `26-q2-admin-and-setup.md` |
| ディレクトリ | `{YY}-q{N}-release-activity/` | `26-q2-release-activity/` |

標準領域スラグ：`admin-and-setup`、`documents`、`projects`、`reports`、`requests`、`other`

## 四半期マッピング

>[!NOTE]
>
>この表は、四半期の名前（H1やタイトルなど）を書き出すためのものです。 機能がどの四半期のファイルに属しているかは決定されません。doc-quarterはcalendar-quarterからオフセットされているため、以下の[2026 リリースカレンダー](#2026-release-calendar) テーブルを使用してください。

| 四半期 | 手書きフォーム | 月 |
|---------|-------------|--------|
| Q1 | 第1四半期 | ヤン=マール |
| Q2 | 第2四半期 | 4月 |
| Q3 | 第3四半期 | 7月 |
| Q4 | 第4四半期 | Oct-Dec |

**重要 – ファイル名で使用されるドキュメント四半期（`26-q3`、`26-q4`など） は、このカレンダーマッピングから1か月オフセットされています。** 代わりに、Workfrontの内部リリースカレンダーのグループ化に従います。各ドキュメント四半期= 2つの先行する月次リリース +四半期ごとのリリース月です。 例えば、ドキュメント四半期`26-q3`は2026年5月/6月/7月の月間リリース（四半期リリース `2026.07`）をカバーし、ドキュメント四半期`26-q4`は2026年8月/9月/10月の月間リリース（四半期リリース `2026.10`）をカバーします。 上記のカレンダークォーターテーブルに基づいてファイルのクォーターを想定する前に、必ず以下のリリースカレンダーを確認してください（または更新を求める）。

## 2026 リリースカレンダー

Source:「2026月次リリースカレンダー」（Adobe株式会社wiki、AWF スペース — `wiki.corp.adobe.com`、スペースキーAWF、タイトル「2026月次リリースカレンダー」）。 WebFetchはこのページに到達できません（Adobe SSOが必要です）。ここでキャプチャしたものよりも日付が必要な場合は、更新されたPDF/テーブルを貼り付けるようにユーザーに依頼します。

| リリース月 | 最終プレビュー | 実稼動 | 月次リリース | 四半期ごとのリリース | 文書（四半期） |
|---|---|---|---|---|---|
| 2025年11月 | 2025年10月30日 | 2025年11月13日 | 2025.11 | 2026.01 | 26-q1 |
| 2025年12月 | 2025年11月27日 | 2025年12月11日 | 2025.12 | 2026.01 | 26-q1 |
| 2026年1月 | 2025年12月23日 | 2026年1月15日 | 2026.01 | 2026.01 | 26-q1 |
| 2026年2月 | 2026年1月29日 | 2026年2月12日 | 2026.02 | 2026.04 | 26-q2 |
| 2026年3月 | 2026年2月26日 | 2026年3月12日 | 2026.03 | 2026.04 | 26-q2 |
| 2026年4月 | 2026年4月2日 | 2026年4月16日 | 2026.04 | 2026.04 | 26-q2 |
| 2026年5月 | 2026年4月30日 | 2026年5月14日 | 2026.05 | 2026.07 | 26-q3 |
| 2026年6月 | 2026年5月28日 | 2026年6月11日 | 2026.06 | 2026.07 | 26-q3 |
| 2026年7月 | 2026年7月7日 | 2026年7月16日 | 2026.07 | 2026.07 | 26-q3 |
| 2026年8月 | 2026年7月30日 | 2026年8月13日 | 2026.08 | 2026.10 | 26-q4 |
| 2026年9月 | 2026年9月3日 | 2026年9月17日 | 2026.09 | 2026.10 | 26-q4 |
| 2026年10月 | 2026年10月1日 | 2026年10月15日 | 2026.10 | 2026.10 | 26-q4 |
| 2026年11月 | 2026年10月29日 | 2026年11月12日 | 2026.11 | 2027.01 | 27-q1 |
| 2026年12月 | 2026年11月26日 | 2026年12月10日 | 2026.12 | 2027.01 | 27-q1 |
| 2027年1月 | 2027年1月5日 | 2027年1月14日 | 2027.01 | 2027.01 | 27-q1 |

この表を使用する際の注意事項：

- **最終プレビュー**&#x200B;は、その月次リリースのプレビューに表示される機能の最終日です。概要ページの「機能がプレビュー環境に表示される最終日」の箇条書き（四半期末の月のみ）に使用してください。
- **Production**&#x200B;は、その月次リリースの正式なProduction-for-everyoneの日付です。
- 四半期の終わる月（四半期リリース列に一致する月）の場合、概要ページのスケジュール表には、その月のリリース **twice**&#x200B;が一覧表示されます。生産日の&#x200B;**前日**&#x200B;の「月間リリース」列に1回、実際の生産日の「四半期リリース」列に1回が表示されます。 四半期の最終以外の月は、月次リストと「迅速リリース」の両方で同じ生産日を使用します。調整は必要ありません。
- この表は2027年1月を通過します。 後で日付が必要な場合は、推測するのではなく、更新されたカレンダーをユーザーに依頼します。

四半期ごとの生産リリースは、通常、四半期の最後の月の第2週の木曜日にリリースされます。

## 検証チェックリスト

リリースノートファイルを確認する際は、次の点を確認します。

- [ ] Frontmatterには、すべての必須フィールドが正しい値で含まれています
- [ ] H1がページタイプ形式と一致しています
- [ ]概要リンクが正しい四半期を指しています
- [ ]各機能には`>[!NOTE]`個の日付ブロック （製品領域ページ）があります
- [ ]日付形式が一貫しています（`Month Day, Year`）
- [ 概要内の]機能テーブル行が製品領域ページのコンテンツに一致
- [ ]壊れた内部リンクはありません
- [ 概要の]個のアンカーリンクがH3 セクション IDと一致
- [ ]機能は、最新の順に並べられます（製品エリアページと概要テーブルの両方）
- [ ]新しいリリースノートのページが`help/quicksilver/TOC.md`に正しい四半期の下に一覧表示され、概要が最初に、製品領域がアルファベット順に表示されます（その他の最後）
- [ ]新しい四半期の概要ページが作成された場合、`help/quicksilver/home.md` 「最新リリース」タブと現在のタブがそれにポイントされます
- [ ]新しい四半期の概要ページが作成された場合、`help/quicksilver/product-announcements/product-releases/product-releases.md`は現在の「Workfront リリース」リストの先頭にリストします（存在する場合は計画リンクを含む）

## その他のリソース

- HTMLのテンプレートと例については、.claude/commands/_release-notes-formatter-reference.mdを参照してください。
